...
const ALLOWLIST = [
  ...
  { file: 'lib/completion-gate.js', match: /store\.updateItem\(item\.id, \{ status: 'complete' \}\)/, why: 'completionGate({intent:\'backfill\'}) §5.10 step 6.4 for tracksFeatureJson:false modes (fix/plan) — the twin of the /lifecycle/complete write above, inside the gate. updateLifecycle does not touch item.status, so without it a backfilled fix/plan item finalizes still reading its old status (COMP-LIFECYCLE-BACKFILL R2B-9)' },
