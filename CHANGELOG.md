...
- **Build summary shows lessons awaiting the owner (STRAT-LEARN-INLINE-TS-1 §A5, STRAT-LEARN-DELIVER-1 D6).** At every completed, failed and killed build exit, Compose runs `stratum learn list --unreviewed` and `--reviews` (`--if-enabled --json --root <cwd>`) through the existing Stratum bin resolution and prints staged-but-unreviewed lessons and open lesson reviews. Silent when the workspace's `[learn]` switches are off, fail-open on any error. `test/learn-summary-golden.test.js` drives real builds to each exit against a real Stratum, and also asserts an applied lesson's guidance reaches the Compose-built `## Intent`. **Deploy order:** needs a Stratum release that has these CLI flags; an older CLI ignores them and would list raw candidate rows, so bump the `@smartmemory/stratum` dependency in the same release. A build that throws, including during setup (bad pipeline YAML, MCP connect failure), prints the summary too, at most once per build, after the build's teardown drain is released; cancelling while the summary queries are pending kills them and prints nothing. Raw `PatchCandidate` rows from an older CLI (they carry `rendered`) are filtered out of the unreviewed list.
...
- **Model tiers move to current models (owner directive 2026-09-23).** `server/model-tiers.js`: Claude `critical` → `claude-opus-5-5` (xhigh, unchanged effort); Claude `standard` → `claude-opus-5-5` at `medium` (was `claude-sonnet-5` at `high`); Codex `standard` → `gpt-6-sol` at `medium` (was `gpt-5.6-terra` at `high`); Codex `fast` → `gpt-6-luna` (was `gpt-5.3-codex-spark`, retired upstream 2026-09-16 and now refused by stratum's model validation, so every fast-tier Codex step was failing); Codex `budget` → `gpt-6-luna` (was `gpt-5.6-luna`). Fast and budget now resolve to the same model but stay distinct tier names; the "spark draws a separate quota" rationale that separated them is gone. Claude `critical` and `standard` are both Opus 5.5 and differ only by effort (xhigh vs medium), so the review triage/lenses/merge steps now run on Opus instead of Sonnet. Tests that pinned tier values follow the map (build-wave-golden now derives the standard effort from it); test helpers/fixtures that DISPATCHED made-up Codex ids (`gpt-5.4`) move to `gpt-6-sol`, since stratum now rejects ids outside its priced allowlist (STRAT-AGENT-RUN-MODEL-VALIDATE). Recorded historical fixtures that are only priced or displayed keep their original ids. Also restored `node_modules/@smartmemory/stratum` to the local-checkout symlink (an `npm install` on 2026-09-19 had replaced it with the published 0.6.0 copy). Verified: node 7215 pass / 1 fail / 1 cancelled (both load flakes, pass standalone: `build-model-route-outcomes` 34/34, packaged-cockpit install 1/1); UI 624/624.

- **COMP-GATE-REENTRY-RESUME and COMP-GATE-MERGE-FINGERPRINT: merge-gate retry guards now survive `--resume` and recognize a repeat conflict without needing byte-identical failure text.** Reported from an external build that spent $176.57 over 61 generations of an 18-lane fan-out without landing a single file, stuck on the same `tokens.json`/`tokens.ts`/`tokens.css` merge conflict every round. Root cause, two parts: (1) `gateReentries` and `consumerMergeFailures` (`lib/build.js:4885-4886`) were plain in-memory `Map`s scoped to one `runBuild()` call, so a `--resume` after the `MAX_GATE_REENTRIES = 20` cap tripped — which its own error message explicitly invites — reset both counters to zero instead of continuing where the prior process left off; (2) `decideMergeRepairOutcome`'s same-failure kill switch (`lib/build.js:7782`) required exact string equality of the full failure message, which an LLM-regenerated diff on the same conflicting file essentially never reproduces byte-for-byte, so it never fired and only the unrelated 20-round cap eventually stopped a non-convergent build. Fixed both: retry state (round count + last-failure fingerprint) is now persisted per gate in the consumer merge journal (`ConsumerFanoutArtifacts.gateRetryState`/`recordGateRetryState`, `lib/consumer-fanout.js:1209`) and seeded from it on resume instead of starting empty; the same-failure check now compares a stable fingerprint of error code + normalized conflicting file paths (`mergeFailureFingerprint`, `lib/build.js:7781`), falling back to raw message equality when no file evidence is extractable from the git failure text. File-ownership enforcement (`filesOwnedConflict` decompose-time check, `checkOwnership` runtime per-lane check) was checked first and ruled out as the gap — it already existed and worked correctly; this was specifically about a legitimately single-owned file whose own regenerated content never merged cleanly. Implemented and independently reviewed via Codex (`gpt-5.6-sol/high`, two passes). Verified: 7353/7353 (`node --test`), 624/624 (vitest UI), 100/100 (tracker) — full suite, plus 41/41 on `test/build-cancel-detect.test.js` specifically for the `tx.failure`/`tx.failureCode` shape change. See `docs/bugs/COMP-GATE-REENTRY-RESUME/description.md`, `docs/bugs/COMP-GATE-MERGE-FINGERPRINT/description.md`.

- **Two COMP-TRACKER-FORGEJO drift fixes: an outdated assertion and an undiscovered test file.** `de908e2` registered Forgejo as a first-class tracker provider and widened `feature-writer.js`'s `expect_labels` refusal message from "github-only" to "github/forgejo-only" (`:1049`, `:1072`), but `test/feature-linker.test.js:375` still asserted the old wording, so the full suite failed on an assertion the production change had already made correct -- the regex now matches the live message. Separately, `test/tracker/forgejo-api.test.js` (12 `node:test` cases) was added but never reached by any runner: it lives outside every glob in `npm test`'s `node --test` invocation and outside `vitest.tracker.config.js`'s `tests/**` include, so `test/test-discovery.test.js`'s own coverage guard correctly flagged it as omitted. Added `test/tracker/*.test.js` to the `node --test` glob (it is a `node:test` file, not a vitest one, so it does not belong in `test:tracker`'s vitest config despite the similar name). Verified at node 7347/7347 across the full `node --test` glob, including the now-included file.

- **COMP-REVIEW-BUDGET-2: the review prompt budget now covers the path it was mostly likely to be needed on.** `COMP-REVIEW-BUDGET-1` (`7e637db`) shipped a size budget so an over-long review prompt fails locally and cheaply instead of being paid for and rejected by the provider. It was wired to one of the two dispatch sites: consumer review items pass `promptBudget` (`lib/build.js:1923-1928`, gated on `reviewOpts.reviewMode`), while the ordinary step/review dispatch (`lib/build.js:5350-5372`) passed none. The failure that feature exists to prevent — the one that cost $14.04 and 208,080 tokens of completed work — therefore remained reachable on the more common path. The ordinary site now builds an ambient-free `requiredPrompt` the same way the consumer site does (re-calling `buildStepPrompt` with `contextDir: null`, precedent at `:1791-1794`) and reuses the same `reviewPromptBudgetChars(context)` helper, so `COMPOSE_REVIEW_PROMPT_MAX_CHARS` keeps one meaning across both sites. **Scoped to review steps only** (`isReviewMain`, including reducer-style `ReviewResult` steps), argued from the consumer precedent rather than widened silently: for a non-review step `requiredPrompt` degenerates to "the prompt minus ambient context", so budgeting there would introduce context-dropping and local refusal to every build step, which is a behaviour change this fix does not want. A scaffolded review reuses one scaffold instance across both the assembled and the ambient-free prompt; a reducer gets the budget with no scaffold, matching its existing semantics. Two regressions added to `COMP-REVIEW-BUDGET-1`'s own test file so both call sites are pinned in one place, each seen RED first: before the fix an oversized ordinary review dispatched 22,813 characters against an 8,000-character budget, and the required-content case dispatched and completed instead of refusing. Verified at node 7/7 (`test/review-provider-resilience.test.js`), 34/34 (`test/build-model-route-outcomes.test.js`), 12/12 and 8/8 (`test/ts-cutover-e3-round{3,4}.test.js`). Found while inventorying what already inspects prompt content for `COMP-PROMPT-SECRETS-1`, not by looking for it.
...
- **COMP-REVIEW-BUDGET-1 (gap G3): a review no longer pays twice to fail the same way.** COMP-HOST-PORTABILITY-1 host B lost its whole lifecycle flow here: two `require: all` lenses had their prompts rejected as too long, the automatic retries hit a provider rate limit, and the run died after $14.04 / 208,080 tokens of completed work. Three defects compounded. (1) No size budget existed anywhere on the path: `lib/step-prompt.js:17-48` concatenates every `docs/context/**.md` in full with no truncation, and the fanout interpolates the whole serialized triage item. (2) The retry made the prompt BIGGER — Stratum retries by attempt count only, reusing the same prompt plus an added "Previous Attempt Failed" section (`lib/step-prompt.js:87-100`), so a length rejection was resent longer and could not succeed. (3) The connector mapped every non-success result to a generic `Error` (`lib/local-claude-connector.js:46-53`), so a deterministic length rejection and a transient rate limit were indistinguishable and both retried immediately, with no backoff. Now: `providerFailureClass()` tags failures as `prompt-too-long`, `rate-limited` or `other` from both yielded results and thrown SDK errors, preserving the original provider message, and honours a provider-supplied `retry_after_ms`; the classification survives the normalizer and reaches the consumer adapter through `previousFailure`, where a prompt-too-long reissue is settled locally with `PROVIDER_RETRY_SUPPRESSED` and never reaches the provider a second time; a rate-limited reissue waits on exponential backoff (2s base, 30s cap, `COMPOSE_REVIEW_RATE_LIMIT_BASE_MS` / `_MAX_MS`) and reports the delay. The assembled prompt is measured after schema injection against `COMPOSE_REVIEW_PROMPT_MAX_CHARS` (default 120,000 chars): over budget drops ambient `docs/context` material first, with a warning, and never cuts the task, review contract or output schema — if required content alone still exceeds the budget the dispatch fails locally with `REVIEW_PROMPT_BUDGET_EXCEEDED` rather than being paid for. Two honest limits: the provider's exact status/code for the measured failures was never captured, so classification matches recorded message shapes and an unrecognized future wording degrades to `other`, which is today's behaviour, not something worse; and the 120,000-char default is a conservative estimate (~30K English tokens), not a measured provider ceiling, which is why it is configurable. Unchanged by design: `require: all` semantics, the lens list, cost-ceiling behaviour, resume semantics, and Stratum. Five regressions in `test/review-provider-resilience.test.js`, each seen RED first, using real-shaped 400/429 payloads — every pre-existing test on this path mocks the provider so thoroughly that no test had ever seen either failure. Verified at node 25/25 across `test/review-provider-resilience.test.js:116`, `test/ts-cutover-e3-round3.test.js` and `test/ts-cutover-e3-round4.test.js`.

- **COMP-GATE-HEADLESS-1 (regression fix): a lost gate response is adopted against proof where proof exists, and the one place it cannot exist is now named as such.** `84eb338` made authoritative-event reconciliation unconditional in the `gateResolve` catch block, removing a `waveProfilesEnabled(pipelineProfiles)` guard that two independent readers judged redundant. Without it, any failed `gateResolve` whose ordinal lookup found a `gate_resolved` event adopted that event's decision as the build's outcome, so a lost response could be upgraded to acceptance with no proof that the recorded decision was ours. The discriminator now has four arms. Three are adoption: an explicit engine refusal (`gate is not awaiting a decision`) adopts, because the engine itself is the authority for "already resolved"; a reconciliation that produced a validated `token-engine-witness` adopts, because the witness carries the consumed gate token; and wave profiles adopt. Otherwise the original error is rethrown and the outcome stays `unknown`. The witness was already computed and returned by `acknowledgeRoutingGate` (`lib/routing-gates.js:216`) and the catch block was discarding the return value. **The wave-profile arm is a scope exception, not proof, and is documented as one in the code.** An adversarial dispatch verified at runtime that in that case `context.routing`, `captured` and `disposition` are all null and no consumed token exists anywhere to prove the engine recorded our decision; the engine records only `{ decision, target }` on a `gate_resolved` event (`stratum/ts/src/engine/engine.ts:1342`), so the proof is not merely missing from a fixture, it does not exist for the approve path. That arm therefore preserves pre-`84eb338` behaviour for its scope rather than satisfying the proof requirement, and closing it properly needs engine-side evidence that is not filed yet. Verified at node 8/8 (`test/build-wave-ship.test.js`, including `:103`, the case that dies without the wave arm), 34/34 (`test/build-model-route-outcomes.test.js`, both discriminator tests — `:68` the ordinal-refusal safety case, which uses a non-wave `simpleSpec` and so cannot enter the new arm, and `:94` the witness case), 4/4 (`test/build-gate-headless.test.js`, including C(ii)) and 6/6 (`test/gate-log-emit.test.js`, re-run unsandboxed after the Codex sandbox cancelled all six on loopback `listen()` EPERM). Procedural note: the invariant test lives in a file about model routing and the wave case in a file about ship commits, so the nine hand-picked gate/workspace suites that were 89/89 green on `84eb338` could not have caught either.

- **COMP-GATE-HEADLESS-1 + COMP-LIFECYCLE-BRIDGE-1 (gaps G1, G4): a build no longer hands its gate to a server that cannot address its workspace.** Filed as two gaps, fixed as one, because grounding showed one root cause with two faces. `runBuild` chose the web-UI gate path on any successful `probeServer()`, and `/api/health` is deliberately exempt from workspace resolution (`server/workspace-middleware.js:30`) — so a healthy server that had never heard of the workspace still captured the gate; host C then waited with no client deadline until it was killed. The same build constructed `new VisionWriter(dataDir)` with no `workspaceId` (`lib/build.js:3814`), so tracker writes carried no `X-Compose-Workspace-Id` and the middleware fell back to the server's own active root — which is why host B's tracker truthfully reported `count:0` gates during a LIVE `design_gate` that Stratum could see (SD06). Three changes: the build now derives the canonical workspace id and threads it into its `VisionWriter`; a new `probeWorkspace()` replaces the health probe at the delegation decision, and a healthy-but-unaddressable server now prints the server URL and workspace id and falls through to the existing headless path rather than capturing the gate; `pollGateResolution()` gains an independent deadline sharing one `COMPOSE_GATE_TIMEOUT` contract with the server's lazy expiry via `lib/gate-timeout.js` (30-minute default, so client and server cannot drift), failing with `GATE_POLL_TIMEOUT` naming the gate id and the headless command. The already-resolved-gate race is now adopted rather than rethrown: the authoritative-event reconciliation at `lib/build.js:5945` was live only under wave profiles, so a non-wave build that had its gate resolved out-of-band died with `gate is not awaiting a decision` — host B's loud failure. **That change was too broad and is corrected above: making reconciliation unconditional also let a bare ordinal lookup upgrade a lost response to acceptance.** Adoption is now gated on the engine's own refusal or a validated token witness. Consumer-merge checkpoint work stays guarded by `consumerMergeArtifacts`, cancellation still rethrows, and a missing authoritative decision still rethrows the original error. One consequence not in the brief and adopted deliberately: once the header is sent, a globally healthy but workspace-unaware server makes the writer fail on `400 Unknown workspaceId` BEFORE the gate is reached, so `VisionWriter`'s REST-versus-direct transport choice is workspace-aware too. No middleware, workspace-registration, session-binding or Stratum semantics changed. Four regressions in `test/build-gate-headless.test.js`, each seen RED first; the exact production error `MCP error -32603: gate is not awaiting a decision` was reproduced before the fix. Verified locally at 89/89 across the new file and nine existing gate/workspace suites — four of those (34 cases) could not run inside the Codex sandbox, which denies loopback `listen()`, and were re-run unsandboxed by the controller.

- **COMP-SUPERVISOR-OWNER-1 (gap G8): `compose start` can no longer silently kill another project's server.** `killExistingSupervisor()` read the install-wide PID file at `COMPOSE_HOME/.compose-supervisor.pid` and SIGTERMed whatever process it named, unconditionally, before binding. The PID file is keyed on the Compose INSTALL rather than the target project, so starting Compose from any project replaced the supervisor serving a different one. This removed the owner's running :4001 server during the COMP-HOST-PORTABILITY-1 audit. The file now holds an ownership record (`pid`, `targetRoot`, `startedAt`) at the same path: a same-project restart behaves exactly as before, a stale record proceeds silently, and a cross-project start is REFUSED with the owning project and PID named, unless `compose start --takeover` is passed. Residual, accepted: a legacy bare-PID file carries no project identity, so the first restart after upgrading keeps the old kill behavior once.

- **COMP-VERIFY-POLICY-1 (gap G9): triage can raise a requirement but can no longer silently lower one.** Measured on host B (COMP-HOST-PORTABILITY-1, SD08): a feature whose stored profile carried `needs_verification: true` ran with it false, Stratum recorded the verification step `skipped`, no verifier ran, and nothing explained the downgrade. Root cause: `deriveProfile()` computes a profile purely from repo signals and returns a fresh object (tier 0, "config-only", returns `needs_verification: false`), so an explicit requirement recorded earlier was overwritten rather than honored. A stored `needs_*: true` now forms a floor over every newly derived profile, applied at the effective-profile persistence seam in `lib/lane-gate.js` and at the cached-profile read in `lib/build.js` (feature.json is excluded from the mtime staleness scan, so an override added to an otherwise-fresh feature would never be reconciled otherwise). Preservation is recorded in `triageRationale`, and `profileOverrides` is the explicit human escape hatch: a boolean there wins and is recorded as an override, so "deliberately lowered" stays distinguishable from "never requested".

- **COMP-CLI-HONEST-EXIT-1 (gap G7): four CLI commands no longer exit 0 while doing less than they claim.** COMP-HOST-PORTABILITY-1 probed 88 standalone CLI surfaces on host C and found four silent partial successes (SD01-SD04), which shell automation and quota planning cannot distinguish from real success. All four reproduced and each now reports the incomplete state on stderr and exits 1: `roadmap` printed two newline bytes for a populated roadmap when no named rows parsed (the code column header must read "Feature" or "ID"); `experiment` reported `complete` with `nCompleted: 0, nTotal: 1`, hiding failed authenticated work; `remote status` omitted the paired-device query entirely when `COMPOSE_API_TOKEN` was unset; `migrate-anon --non-interactive` found six anonymous rows, promoted none, and returned 0. A deliberate no-op stays successful: `--dry-run` remains an exit-0 listing with unchanged output.

- **COMP-EXPLORER-EVIDENCE-1 (gap G10): the design phase no longer accepts a design that skipped exploration.** `explore_design` asked in prose for 2-3 explorer subagents while its only acceptance checks were the outcome enum and `file_exists(result.artifact)`, so a requirement that existed only in prose was never enforced: COMP-HOST-PORTABILITY-1 measured host C running zero explorers ("no explorer subagents were needed") and host B running one, both designs ACCEPTED with no cardinality warning (SD05). `PhaseResult` now carries optional `explorations: ExplorationEvidence[]` (`focus`, `findings`, `files_examined`), the step prompt asks for one entry per explorer, and a third `ensure` expression requires at least two explorations for a `complete` result while leaving a legal skipped-with-artifact result untouched. Scope is the audit's S half (count/evidence validation) only; portable subagent delegation (the M half) remains unbuilt. The frozen `model-route-off-*` prompt fixtures are unaffected.

- **COMP-OUTCOME-ENUM-1: repair the dead expected-output prompt section and enum-less flat schema.** The live callers pass `output_fields` as `Record<string,string>`, so agents never saw the object-gated field list, while `complete|skipped|failed` degraded to unconstrained `{}` in the injected JSON Schema; together these defects preceded 23 measured invalid-outcome occurrences across Compose and Stratum. Object fields now render as `name (type)`, and bare pipe types become string enums consistently in flat and closure-root schemas. The synonym adapter remains deferred behind the fresh-build `learn harvest` falsifier. The three frozen model-route baselines were re-captured from `ed8e333` because the prompt bytes legitimately changed.

- **COMP-HOST-PORTABILITY-1: host portability audit measured, not reasoned — report at `docs/features/COMP-HOST-PORTABILITY-1/report.md`.** Three hosts (Claude Code reference, Codex CLI driving Compose over MCP, standalone shell) and the Stratum connector pair. Measured: 55 of 88 standalone CLI command surfaces work, 4 exit 0 while doing less; both lifecycle runs dispatched ONLY Claude via `@anthropic-ai/claude-agent-sdk` regardless of host — host B failed loudly at review ($14.04, unresumable: MCP resume, `--resume` "Nothing to resume", `stratum_resume` all failed), host C blocked forever at a web-UI gate that could not resolve the scratch workspace. Stratum connector neutrality REFUTED (17 one-sided capability groups, 3 failure-surfacing asymmetries). 14-row silent-degradation inventory incl. explorer subagents skipped, `depends_on` ignored by the fanout, verification silently downgraded by triage, Compose tracker returning empty at a live gate, and the `outcome: "success"` vs `complete` contract retry that costs a full extra dispatch on every host. Operability finding: `compose start` SIGTERMs whatever supervisor the machine-wide PID file names (`server/supervisor.js:80-93`), so starting the app from any project replaces another project's server — this took down the owner's :4001 server during the audit. Recommendation: declare limits and fix G1–G4/G6–G10 first, then a bounded Codex pilot before any port. Audit only; no source changed. Stratum flow `05e46af9` (5 Codex dispatches, 0 retries).

- **COMP-COMPLETION-GATE-1: malformed completion input is refused before irreversible guard work.** A git-resolvable short SHA could previously pass evidence, be written into the intent and guard ledger, then fail the writer's full-SHA rule and become unrecoverable by retry. The writer now exports the single 40-hex normalizer and shared pure-shape validation; the live and backfill gates canonicalize SHA case before evidence/recovery, hoist writer-side request shapes other than `tests_pass` (which remains evidence-attested), and validate restored backfill intents before guard replay or lifecycle-history writes.

- **COMP-TUI-4 COMPLETE through the completion gate; COMP-COMPLETION-GATE-1 filed.** Recording the completion surfaced an ordering defect in `lib/completion-gate.js`: the evidence step accepts any ref git resolves, so a 7-char SHA prefix passed, the guard ledgered the transition, and only then did `completion-writer.js` refuse the short SHA (Decision 9), leaving an intent that no retry could recover (same prefix → same writer refusal; full SHA → refused as "different evidence"). Operator workaround: correct the write-ahead intent to the full SHA of the same commit and re-drive the same operation. The fix belongs before the guard write and is filed as COMP-COMPLETION-GATE-1.

- **COMP-TUI-4: live parallel task grid in the CLI.** During a fanout `compose build` now renders a per-task grid (row index, status icon, agent, task id, elapsed) at the bottom of the terminal instead of one scrolling banner per task. Every redraw path (toggle, resume, heartbeat, warnings, findings) routes through one grid-aware dispatcher so the grid survives keypresses and messages; erase accounting counts physical rows after ANSI stripping so narrow terminals do not leave stale fragments; non-TTY streams get one plain-text line per task start and completion; the heartbeat timer stays referenced (an earlier draft unref'd it for tests, which could let the process exit mid-step). `lib/build.js` forwards the agent name and the succeeded/failed status at the two fanout call sites. Reviewed by Codex in two rounds (4 should-fix → fixed → 1 missing regression test → added). Known nit: wide/CJK glyphs are measured by string length, not cell width.

- **COMP-MCP-ARTIFACT-LINKER-1 filed: `link_artifact` advertises artifact types the schema rejects.** The tool description (`server/mcp-tool-defs.js`) lists `finding`, `report-supplement`, `link`, `external`, but `contracts/feature-json.schema.json` only allows `design|prd|architecture|blueprint|plan|report|journal|snapshot`, so a linked `finding` writes successfully and then fails the pre-push schema gate (bit COMP-MODEL-ROUTE-1 on 2026-09-15; worked around in `8f5e33f`). Until fixed, use only `report`, `journal`, or `snapshot`.
...
- **COMP-COST-OWNER: two open residues closed by trace, no code.** (1) The ~15x compose/stratum token disagreement on flow `4122e695` reconciles exactly and the cached-input double-count hypothesis is REFUTED: 425,284 is nine model requests' input+output for one codex dispatch (recovered from the raw session under `~/.codex/sessions/`), 28,279 is the sum of the only four usage reports stratum received, because at `a48c40a9` compose's ordinary step-completion envelopes carried no `usage`. (2) `13fd190e`'s ledger-only `aborted` row was written by `abortBuild` — the session transcript pins the `--abort` command around the row's timestamp; the "flow still `running` rules it out" inference applied the `d0a07c1` (2026-09-10) settle guard to an August event. `evidence/token-tally-15x-reconciled-2026-09-14.md`, `evidence/13fd190e-writer-attributed-2026-09-14.md`.

- **COMP-COST-OWNER: local-Claude dispatch telemetry now labels provider-reported dollars.** The omission did **not** reach `aggregateUsdSource` or drop money from the build stream or Surface-15 receipts: dispatch-ledger rows are a separate telemetry path read by `collectDispatchMetrics`. The cost was incomplete raw telemetry provenance, not missing downstream spend. `lib/local-claude-connector.js` now stamps `usd_source: reported` on priced success and `SDKResultError` usage and on their dispatch rows; a missing cost remains `null` with no source instead of becoming a synthetic `$0`. `lib/result-normalizer.js` now preserves that connector-owned provenance unchanged on both success and failure instead of inventing `reported` for an unlabelled local result, so an unknown SDK failure remains null and unlabelled through normalization, while a genuine provider-reported `$0` remains a reported zero.

- **COMP-COST-OWNER: a dispatch row now carries the token split and the cost's provenance.** `#recordAgentDispatch` hardcoded `tokens_in: null, tokens_out: null` and ignored `usdSource`, while the SAME FILE ~600 lines later reads `value.split` off the same result envelope and prefers it, with a comment (`STRAT-USAGE-SPLIT`) noting the legacy reconstruction "cannot tell input from output and filed the whole aggregate as output for every record ever written". So every stratum-routed dispatch recorded a bare total. Measured 2026-09-14: `tokens_in` null on 36 of 39 stratum-routed claude rows and 6 of 6 codex rows — the 3 populated ones come from `lib/local-claude-connector.js`, which reads its own capture. **Consequence, and the reason this matters:** a row with only a total cannot be repriced, because input and output differ by 6x on terra — so the 449,116 historical unpriced tokens (56% of this project's ledger) are unrecoverable, and stratum's flow file is no fallback (`receipts: null`, zero token-split keys).

  **`usd_source` is the second half and arrives for a live reason.** stratum `00ff4db` (2026-09-12) began ESTIMATING codex costs from token counts, because Codex reports none of its own. Without a provenance field the ledger cannot tell an estimate from a provider figure, and a consumer may reasonably refuse to reconcile spend against an estimate. `null` means unknown, never a claim; the value is never derived from whether `usd` is present, which is the inference that turned an honest estimate into a false "provider reported" figure once already.

  Registering the field in `lib/dispatch-ledger.js` was **required, not cosmetic**: the event shape is an allow-list and an unknown field drops the whole event — reverting that file alone fails 10 of 15 dispatch-capture tests, not 2. `tokens_total` is deliberately NOT recomputed from the split: it is the connector's own total and the two can legitimately differ, since codex keeps OpenAI's raw dialect where `input_tokens` INCLUDES `cached_input_tokens`. Deriving it would change the field's meaning silently.

  **Scoped out, deliberately:** `lib/local-claude-connector.js` was a second, independent writer and was left emitting no `usd_source`, so its rows read `null` although their cost is always provider-reported. It remained a separate follow-up rather than a silent ride-along.

  Falsifier: `test/dispatch-capture.test.js` — "a dispatch row carries the connector split and the cost provenance" and "a dispatch with no split or provenance still records nulls, not zeros". Negative control RED against **both** `lib/stratum-mcp-client.js` and `lib/dispatch-ledger.js` on a GREEN 15/15 baseline.

- **COMP-COST-OWNER 0b Part B: the cost oracle was reading the wrong file, and the money was never missing.** S5 reported three flows where compose recorded less than an independent lower bound, the worst retaining 15-28%. It was measuring `build-history.jsonl`. Compose's real cost record is `dispatch-ledger.jsonl`: `13fd190e`'s resumed segment is there in full (`aborted`, **$4.5037144**, **36354** tokens, the same 36354 recorded elsewhere as a three-way census PASS on that flow), and only its history row is missing. Two terminal paths write a `build-actuals` ledger row and never append a history row — `abortBuild` (`lib/build.js:7745-7750`) calls `appendBuildHistory` nowhere at all, and `terminalizeThrownBuild` bails at `if (!flowId) return false` (`:3365`, `flowId = response?.runId`) before both its `active-build` write and its history append while the enclosing `finally` still emits the ledger actuals. Which one produced `13fd190e` is **not** determined; the flow file still reads `status: running`, which argues against the first. Part B was pre-registered as a controlled repro with a forced kill; it cost **$0** and no build was run, because the writer set plus data already on disk settle it.

  **`scripts/cost-oracle.mjs` now reads the ledger.** Its `readLedger()` opened `build-history.jsonl` — a function named for a file it never touched. Aggregation is keyed by FEATURE, because ledger rows carry `build_id` and `feature_code` and no flow id, so a per-flow join does not exist for legacy rows. The rule is **last `build-actuals` row per `build_id`, summed across `build_id`s**, which needs no heuristic since `build_id` is on every row — unlike the history surface's `accumulator_build_id`, which only shipped in `10f46ea` and is absent from all existing data. The flow inventory now comes from stratum flow files filtered by `workspaceRoot`, so it still sees builds whose history row was never written. `build-history` is still read, but only to MEASURE the gap between the two surfaces rather than to be silently preferred.

  **CORRECTED same day, before anyone quotes it: "zero UNDER findings" is a ROLL-UP ARTIFACT.** The feature total masks a per-build under-record that the ledger makes *worse*. Pairing builds to flows by exact cost match against each flow's own `flowSpent`: `f9309dc4`/`af922492` $18.9301 vs $16.4574 = 1.15x (resolved); `678e6a58`/`44c575e7` $6.9342 vs $4.0447 = **1.71x over**, up from 1.08x on the history surface; `3e95eb77`/`4122e695` $0.6974 vs $2.5118 = **0.28x UNDER**, down from 0.92x. So of S5's three UNDER flows, `13fd190e` and `af922492` resolved and **`4122e695` did not** — the COMP-GUARD-CLAIM-1 roll-up reads OK only because `f9309dc4`'s $18.93 hides it. The tool now emits `build_rows` so the masking is visible; the verdict stays per-feature, because build-to-flow pairing rests on a cost-match heuristic rather than a key. **Open question 0d widens rather than dissolving**, and the gloss "compose counts spend stratum never sees" was shape reasoning, never traced, and is withdrawn. **The ledger is also not a superset of history** — history carries $1.6044701, $2.4726621 and $2.8894888 with no ledger counterpart, so *both* surfaces leak and the defect is non-atomic terminal writes across two surfaces, not "history is the bad one"; the tool's message is now `SURFACE-DISAGREEMENT` and names no mechanism. **The surviving defect is `3e95eb77`**, which both signals point at: 0.28x under its flow, and 436,736 tokens for $0.6974 — more tokens than every flow's `flowSpent` combined, at 1/26th the feature rate. Either `tokens_total` is inflated or most steps carry no dollars. Next step is to trace `tokens_total` accumulation for that build.

  **Raw result: zero UNDER findings at FEATURE level, exit 0.** COMP-GUARD-CLAIM-1 $30.5862 ledger vs $23.0139 flowSpent / $20.6084 ccusage (1.33x); COMP-SEMVER-STRICT $4.5037 vs $3.0870 (1.46x). `13fd190e`, previously the worst at 0.28x, is now 1.46x above its lower bound. **The under-recording defect does not survive contact with the real ledger.** Two things do, and the tool now reports both: a **HISTORY-GAP of $14.79** across the two features (the Part B defect, quantified — a surface gap, not lost money), and an **UNPRICED-SUSPECT** on build `3e95eb77`, **$0.6974 for 436,736 tokens** — $0.0016/1k against a feature mean of $0.0413/1k. That one cannot be adjudicated from the ledger at all, because the `build-actuals` row carries no `usd_unknown_count`, so "cheap" and "unpriced" are indistinguishable on it; it is flagged as a labelled heuristic that does not set the exit code, and it is now the most likely remaining real defect. Open question 0d does not dissolve: at feature level the ledger sits *above* `flowSpent`, not below, and the two disagree on tokens (741k vs 217k), so they are not measuring the same scope.

  Falsifier: `test/cost-oracle.test.js` (15 tests; negative control RED 15/15 on a GREEN 15/15 baseline against `scripts/cost-oracle.mjs`).

- **COMP-MODEL-ROUTE: ordinary builds can now record a routing shadow corpus — `routing.mode` in `.compose/compose.json`.** Two pieces of this feature were waiting on evidence that nothing was collecting. Q3 (does the repair floor measure the right thing?) was deferred on the record with "the shadow corpus should settle it before S3 builds enforcement", and S2's hindsight report reads the same corpus. **The corpus was empty** — `.compose/routing/` did not exist, and nothing about ordinary use would ever have created it: `route_mode` defaults to `off`, `bin/compose.js` had no `--route-mode` flag at all, and the only shipped config setting `_routing` was `presets/team-fable-astra.profiles.json`, the preset whose live-fire is held. Both open items were therefore blocked behind a paused run.

  Mode precedence is now `--route-mode` flag > `.compose/compose.json#routing.mode` > preset `_routing.mode` > `off`. That is one `??` term in `routingOptionsFor`, the single resolver both Build and GSD already call, so both entry points are covered by the one change; `--route-mode=<off|shadow>` is added to `compose build`. This repo enables it, so evidence accumulates during ordinary work at no extra model spend.

  **Checked before building rather than after:** shadow on an ordinary preset is an already-tested path — `test/helpers/routing-runtime-fixture.js` defaults to `shadow`, drives `runBuild` with `template: 'bug-fix'` rather than the team preset, and asserts ledger rows. Had it been team-preset-only, this would have been a flag that turns on a refusal, and the right slice would have been a different one.

  **Refusal semantics are deliberately unchanged.** A `ROUTING_*` error still fails the run, including an ordinary-only one, because the design's acceptance criteria say it must. Degrading to "record what we can" was considered and rejected: a censored sample silently corrupts the corpus the open questions depend on, which is the same failure class as a `?? 0` turning "unknown" into "free". The escape hatch is the switch rather than a downgrade, so a routing refusal now prints its `ROUTING_*` code alongside both `--route-mode=off` and the config key — previously the user saw only the refusal text, with nothing connecting it to a setting.

  Eight precedence cases plus three end-to-end through the real `runBuild`: the project setting alone produces rows on an ordinary build, `off` stays byte-silent, and the flag overrides a shadow-configured project. Negative control RED against `lib/build.js`. This does not settle Q3, it makes Q3 answerable.

- **COMP-COST-OWNER Open Question `0`: the cost row finally records cache tokens.** `input_tokens` was 0 on 9/9 history rows. With 1h caching on, genuinely uncached input really is 1-3 tokens per turn, so the near-zero was not the bug — the bug was that **cache tokens, 95%+ of billed input, had no field on the row at all**, so a fully cached build was indistinguishable from one that did no work. Rows and the accumulator now carry `cache_read_tokens` and `cache_creation_tokens`, summed from the producer's own report through `recordBuildUsage`.

  **It was filed as "additive across four layers" and it was not — it needed a version bump.** The build accumulator is strict-validated in both directions (an unknown field and a missing field are each "corrupt", and `v` must match exactly), so adding two fields to a v3 record would have made every in-flight sidecar on disk unreadable mid-build. `BUILD_ACCUMULATOR_VERSION` is now **4**, with a v3→v4 migration that sets both fields to **null**: a v3 record cannot recover its cache totals, and a 0 would read downstream as a measured "nothing was cached" — on a real build, the opposite of the truth. That is the same refusal v2→v3 made about the input/output split. A rotation writes 0 rather than null, because a fresh record's cache total is a measured nothing.

  **The first implementation read the wrong object, and the end-to-end test is what caught it.** Cache is summed per DISPATCH ENTRY, selected exactly as the unpriced-step count already is. In the non-event path `result-normalizer` fills the cache fields on the RECORD (`usageRecordFromRaw:306-310`) while the aggregate `usageTotals` keeps the 0 it was seeded with at `:409-410` — so reading the aggregate reports **no caching on a fully cached build**. **And the non-event path is the production path**, which was traced rather than assumed after the fact: `usageTotals` is filled only by `step_usage` events (`:484-492`), and `result-normalizer:697-699` states that the TS `agent_run` route "returns a synchronous `complete` envelope with aggregate usage and streams NO `step_usage` progress events" — only the retired python/factory-shim route streamed them. Since the TS cutover that is every real dispatch, so the aggregate would have carried a 0 for cache on every build, not merely on an edge case. The comment beside `unknownEntries` warns about this precise trap ("same trap as `usd_source` riding `usages[0]`") and the first draft walked into it regardless. A schema-only test would have passed; the test that drives a real connector usage record through the normalizer, `recordBuildUsage` and the accumulator onto the row went red.

  **A cache-only usage is reachable, so it joins the write guard** — settled by tracing rather than assumed. `hasReportedUsage` (`result-normalizer:282-283`) returns true on cache tokens alone, so `usageRecordFromRaw` builds a record with `input_tokens: 0`, `output_tokens: 0` and a cache field, and `:521` already treats that case as real when counting an unpriced step. Without the guard term such a usage would be dropped in silence.

  `tokens_total` still means input+output — cache is billed at a different rate and the S1 ledger/accumulator reconciliation depends on that field keeping its meaning. All three `lastOwnerCost` mirror sites carry the new fields, the rotation site included, which is the site that nearly went wrong in `10f46ea`. Scoped out deliberately: the stream's `build_end` event still emits input/output/cost only (it cherry-picks rather than spreading, so it was unaffected); adding cache totals there is a schema change with no current reader. Negative control RED against `lib/build.js` on both the cost and the history-mirror suites.

- **COMP-COST-OWNER `0d-1`/`0d-2`: a failed build now fails — `runBuild` has a terminal result.** `runBuild` reaches its terminal FAILURE state without throwing: it writes the failed history row, prints "Build failed", and falls through its cleanup blocks. It had no `return` on that path at all, so it resolved `undefined`, and **every caller read a failed build as a success** — `compose build`, `compose fix` and `compose plan` all exited **0**, and `lib/build-all.js` pushed the feature onto `built`. Any CI running `compose build` has been passing silently through failures.

  **Correction to the entry as first published (`8832eda`), traced 2026-09-13.** That entry claimed `lib/experiment.js` was "fixed for free, since it gates on that same exit code." **False.** The A/B orchestrator never reads the exit code: `lib/experiment-metrics.js:231` derives its outcome from the build-history row (`historyRecord?.status === 'complete'`), and `exitCode` appears only in `collect`'s JSDoc (`:177`) — the sole field taken from `buildResult` is `wallMs`. So experiment scoring was already correct and is untouched by this change; the blast radius is the CLI exit code and `build --all`, not experiment runs. The claim was asserted from the SHAPE of `realHeadlessBuild` (which *returns* `exitCode`) without tracing to a read-site — the same failure mode as the five claims corrected during S5, and caught the same way, by tracing rather than re-reading.

  **Why the obvious fix was inert.** The previous session dropped the `abort &&` guard at `bin/compose.js:2885` and an integration test caught it doing nothing (`0 !== 1`); it was reverted rather than shipped as a dead path. Nothing ever resolved `{ok:false}` to gate on, so the CLI branch was unreachable. The real fix is the return contract: `runBuild` now returns `{ ok, status, featureCode, flowId, failureReason }`, assembled beside the `build-history.jsonl` append so it reuses that row's already computed `failureReason` and is read **after** the health gate may have downgraded the build. A build that throws still rejects.

  **`ok` is true only for `complete`.** `killed` (the gate rejected the work) and `aborted` (the flow was cancelled) did not ship, so a caller gating CI must see them as failures. The subtle case is a flow that COMPLETES and is then downgraded by the health gate — the build has already printed "Build complete." by then, so an `ok` derived from the flow's own status would be wrong there and nowhere else; it is pinned by a test.

  **A failure now also blocks its dependents** in `compose build --all`. Same-phase roadmap entries are implicitly chained, so a failed feature previously landed in `built` and its dependents ran on top of it.

  Three edits, three independent negative controls, all **RED**: `lib/build.js` (via the pass/fail build goldens and the health-downgrade case), `bin/compose.js` and `lib/build-all.js` (via the new `test/build-exit-code.test.js`, which drives the real CLI to a real process exit code and the real `runBuildAll`, with a loader stub at the `lib/build.js` boundary so no engine, agent or paid dispatch starts). `POST /api/build/start` now returns `{ok:false, ...}` with HTTP 200 for a failed build instead of `{ok:true}`; no UI consumer branches on that body field (they read the HTTP `Response.ok`), so the body is strictly more informative.

- **COMP-COST-OWNER: `build-history.jsonl` is aggregatable again — rows carry the accumulator identity.** A row's `cost_usd` is a CUMULATIVE snapshot within one accumulator lifetime and DISJOINT across lifetimes, and nothing on the row said which, so no consumer could total a flow. Neither rule worked: flow `44c575e7`'s three rows SUM to $11.6007 while the flow itself spent $4.0447 (impossible), and flow `4122e695`'s LAST row ($0.6974) sits below its earlier row ($1.6045) so cannot be a running total. Rows now carry `accumulator_build_id`, making the rule exact: **group by `(flowId, accumulator_build_id)`, take the LAST row within a group, SUM across groups.** Purely additive; no existing key changed.

  **The field is written at THREE sites, and the third is the one that mattered.** The first brief for this named two — the `lastOwnerCost` initialiser and `buildCostSnapshot` — and was **wrong**: the rotation path (`lib/build.js:3885`) replaces the whole mirror with a zeroed record, so a two-site change would have dropped the field precisely when a build **auto-resumed**, which is the case the feature exists to measure. Auto-resume of a terminal flow rotates the accumulator (`lib/build.js:4151`), and that rotation is **correct** for the grouping rule rather than a problem for it — a new accumulator lifetime is exactly a new group — so the comment says so, to stop a later reader "fixing" it. Negative control **RED 3/3** against a 3/3 green baseline, with a test dedicated to the rotation site. `scripts/cost-oracle.mjs` applies the exact rule when every row carries the field and keeps its conservative `max(sum, last)` fallback for legacy rows, which is all existing data; verdicts on real data are unchanged (three flows UNDER, exit 1).

  **One fix was attempted, proved inert, and reverted rather than shipped.** A failed build exits 0 — `bin/compose.js:2885` gates on `result?.ok === false`, so a non-abort failure reports success and any CI running `compose build` has been passing silently through failures. Dropping the `abort &&` guard looked like the fix and **is not**: `runBuild()` never returns `{ok:false}` on a terminal failure, it writes the failed row, prints "Build failed" and falls through its cleanup blocks resolving `undefined` (`lib/build.js:6531`). An integration test caught it (`0 !== 1`). The real fix is a return-contract change to `runBuild` with several callers, filed as `0d-1` with its falsifier. Also filed as `0d-2`: `lib/build-all.js:128` counts every resolved result as success including `{ok:false}`, independently of the CLI exit code. Cost-surface suite 123/123.

- **COMP-COST-OWNER: the controlled repro ran — the accounting is exact, and the checking tool was the thing that was wrong.** Correction and completion of the S5 entry below; that entry's verdict table and coverage claims are superseded by this one. A build was killed mid-flight and resumed to completion in a throwaway project, so for once the transcript oracle is a **full** accounting rather than a lower bound. **Three independent sources agree to the cent on the resumed row:** ccusage over transcripts `$7.1940` ($4.0210 main-source + $3.1730 fanout), stratum `flowSpent.usd` `$7.1940315`, compose's ledger row `$7.194031`. Attempt 1 corroborates separately ($0.5738 vs row 1's $0.573726). So resume seeding works, and **per-flow, per-segment cost accounting is exact even across a kill and a resume.** This was pre-registered (`7dd155f`) as the outcome that must NOT read as a clean bill of health: it relocates `13fd190e`'s cause to the resumed run being **killed before its terminal write**, rather than the resume computing a wrong number.

  **The experiment falsified `scripts/cost-oracle.mjs`.** History rows of one flow are cumulative, so summing `cost_usd` across them double-counted every resumed segment — here the sum ($7.455723) overstates by exactly the killed segment. Correcting it **removed the large "ledger is high" artifacts entirely**: both flows previously reported as high were products of the summing bug, and one of them (`4122e695`) is in fact UNDER at 0.92x. The corrected count is **three of four real flows under-recording** (0.28x, 0.60x, 0.92x), the worst retaining 15-28% of a spend two independent sources agree on. One ~8% over-count survives on `44c575e7` and is **unexplained** — the correction removed the 2.88x and 4.00x artifacts, not every over-count.

  **The deeper defect the repro exposed: `build-history.jsonl` cannot be aggregated per flow at all.** Rows are cumulative *within* one accumulator lifetime and disjoint *across* lifetimes (`clearBuildAccumulator`), and **nothing on the row says which**. Neither rule can be right for both cases: `44c575e7`'s row sum ($11.6007) exceeds its own flow's total spend ($4.0447), which is impossible, while `4122e695`'s last row ($0.6974) is below its earlier row ($1.6045) and so cannot be a running total. The script now takes the **most generous** reading (`max(sum, last)`) and flags UNDER only when even that falls short, so it under-reports rather than manufacturing findings. It also gains **stratum `flowSpent.usd` as a second oracle** — it needs no transcripts, so it fixes the thin-coverage blind spot that made two flows unjudgeable, and where both sources exist they agree to 0.3-2.8%.

  Two non-cost defects found on the way, filed as open questions rather than fixed: **a retried step cannot satisfy its contract if the failed attempt already committed** (attempt 1 returned an out-of-enum `outcome: "success"` but had committed `ffab7f2`; attempt 2 returned a valid outcome with `commit_hash: null` because nothing remained to commit, and the strict contract rejected it, failing the flow terminally — `13fd190e` hit the same enum error and survived only because its attempt 1 had not committed), and **`compose build` exits 0 while printing "Build failed."** Also withdrawn: the earlier claim that `input_tokens: 0` proves a broken field — these runs recorded 14 and 7, so with caching the zeros may be honest; the defect that survives is the absence of any cache-token field on the row — though the site named for that was **wrong and is corrected in `design.md`**: `lib/build.js:2238` is a repair-failure aggregation, not the accumulator, and `buildCostSnapshot` cannot drop what it never receives. The loss is upstream in `recordBuildUsage` (`lib/build.js:4412`), whose reduce collapses each entry to three fields and whose accumulator update writes no cache key; the persisted accumulator has none. Measurement cost $7.77. Full suite 7101/7101.

- **COMP-COST-OWNER S5: an outside source now checks the cost ledger, and it says we under-record.** Every cost check we owned compared our numbers to our own numbers — `scripts/cost-census.mjs` included, since both sides descend from the same dispatch records — so a mistake made in two places passed clean. S5 adds `scripts/cost-oracle.mjs`, which prices nothing itself: it shells out to `npx ccusage session --json` over `~/.claude/projects` and sums that tool's own totals. Using ccusage rather than reimplementing its shape is deliberate — a recompute needs a rate table, and a rate table is exactly what S3 deleted.

  **Calibrated before it was trusted.** Claude Code writes its own `type: "cost-state"` record carrying `totalCostUSD` in 176 of 10,091 sessions, which is a free ground truth. Against it, ccusage agrees to **2.57% in aggregate (n=160 single-segment sessions)**. The apparent 600% outliers were not oracle errors: those sessions were resumed, and the first-party record covers only the final segment, so ccusage is the *more* complete number there. A residual ~2.5% systematic bias is real and is recorded as **unattributed** — the leading hypothesis (1h cache writes priced at 2x, not 1.25x) could not be tested because every session in the corpus is ~100% 1h-cache so there is no contrast group, and a least-squares derivation of the implied rate table returned negative rates (`input_tokens` is 1-3 per turn once caching is on, leaving the design matrix near-degenerate). Tolerance was written down before the comparison ran, not after seeing the gap.

  **The finding: on the two flows with real transcript coverage, the ledger records less than a strict lower bound** — `af922492` $4.56 against $16.00 (**0.28x**) and `13fd190e` $1.84 against $3.09 (**0.60x**). The mechanism is pinned to the millisecond: a row's `output_tokens` is *exactly* the cumulative receipt total at the instant the row is written, and `13fd190e`'s row closed **25 ms** after the last counted receipt, leaving 30,691 of 36,354 tokens (84% of the flow) settled afterwards with no second row ever written. That it is a *resume* rather than orphaned workers is settled by the next missing receipt being `decompose`, a **`main`-source** step, 75 s later — orphaned fanout workers cannot run a main-source step. `af922492` shows the same shape (20.3% of tokens recorded against 28% of cost), so this is **one defect, not two**; an earlier draft called them distinct on a timestamp comparison and the token accounting overturned it.

  **Two flows are reported as INSUFFICIENT-COVERAGE, not as counter-evidence.** They joined 1 and 2 sessions against 9 for the others, so the oracle is missing most of their work and "ledger higher" is the expected artifact of thin coverage. The script therefore never sets a non-zero exit on them, labels the oracle a strict **lower bound** on every line (main-source transcripts cannot be joined by path), counts files with no ccusage entry as `unjoined` with unknown cost rather than zero, and **prints no cross-flow aggregate at all** — real findings cancel against coverage artifacts, and the resulting "14.3% low" reads as a mild systematic bias when per-flow error actually ranges 0.28x to 4.00x.

  Also traced, and filed rather than fixed: **the history row drops cache tokens the accumulator already maintains.** `input_tokens` is 0 on 9/9 rows, but with 1h caching genuinely uncached input really is 1-3 tokens per turn, so the near-zero is not the bug — the bug is that cache tokens, 95%+ of billed input, have no field on the row. `lib/build.js:2238-2246` accumulates them and `buildCostSnapshot()` (`lib/build.js:3825`) drops them, copying only `usd`, `input_tokens`, `output_tokens` and `usd_unknown_count`. Corrects the prior session's note that the transcripts carry "no cost field at all": false in general (176 sessions carry one), true for compose's own build sessions (0/21 fanout, 0/26 cwd). Landmine paid for: every directory under `~/.claude/projects` begins with `-`, so a bare `grep -q PAT "$f"` reads the filename as flags and silently reported **0 sessions with cost data when the answer was 176** — the script reads via node `fs` for this reason. Evidence: `docs/features/COMP-COST-OWNER/evidence/s5-oracle-calibration-2026-09-13.md`. Full suite 7101/7101.

- **COMP-OBS-GATELOG: an unreadable gate log no longer 500s the status route.** `readGateLog` guarded `readFileSync` with nothing but an earlier `existsSync` — a TOCTOU, because the file can go away between the two calls. It does: a full-suite run tears down `.compose/data` under other tests while the server is still serving, `readFileSync` throws ENOENT, and the throw escaped through `computeStatusSnapshot` as a **500 from GET /api/lifecycle/status**, failing `test/status-route.test.js` with `Cannot read properties of undefined (reading 'drift_alerts')`. Caught on the COMP-COST-OWNER S3 push gate 2026-09-12. **Not a flake:** that test had passed in all 228 previous logged full-suite runs, and the single failure had a mechanism, so it was traced rather than retried away.

  **The first fix was wrong and the suite caught it — that is the part worth keeping.** Making `readGateLog` itself treat an unreadable log as an empty one looked obviously right ("every caller is an observability read where no data is survivable"). It is not: `lib/smartmemory-sync.js:66` counts a surface it cannot read as SKIPPED, which is exactly how an unreadable gate log stays VISIBLE instead of masquerading as an empty one. Swallowing the error there dropped `result.skipped` from 2 to 1 and was caught by `test/smartmemory-sync.test.js:475` — a test that already encoded the requirement, using the same directory-in-place-of-file trick. The generalisation was made from the callers that were convenient to think of, not from the caller list.

  So `readGateLog` stays **strict** and the tolerance moves to the caller that needs it: `gate_load_24h` is a display counter on a snapshot whose route turns any throw into a 500, so it degrades to 0 and the rest of the snapshot survives — mirroring what `decision-events-snapshot.js:146` already did at its own call site, and leaving the sync's fault reporting intact. The **append path** fails open separately, because there the throw escapes BEFORE the append and loses a gate decision outright; the outcomes are asymmetric (skipping loses a decision, proceeding can at worst duplicate one) and in the race that actually happens the file vanished, so there is nothing to duplicate.

  Three tests, each modelling its own failure rather than reusing one trick: `readGateLog` must THROW on an unreadable log (pinning the contract the first attempt broke); the status route must serve a 200 with `gate_load_24h: 0`; and the append path uses a **write-only file**, because a directory would block the append too and would test something else — the race makes the SCAN fail while the write stays fine. That one asserts the entry is on disk afterwards, not merely that nothing threw, and SKIPS rather than passing vacuously if the filesystem ignores the mode bit. **Negative control matrix RED on both production files against their own suites, GREEN against each other's** (`server/status-snapshot.js` → status-route RED; `server/gate-log-store.js` → gate-log-store RED), against green baselines.

- **COMP-COST-OWNER S3: compose stops pricing anything, and the last short-total defect goes with it.** S3 was filed as "build one pricer with `dialect` as data on the rate row, closing the `gpt-5` prefix defect that mispriced luna 35.7x". Both things it was meant to serve were read-verified only, so both were probed before any code was written and **both came back against the filed plan** (`docs/features/COMP-COST-OWNER/evidence/s3-reachability-probes-2026-09-12.md`). **Probe 1:** `deriveUsd` is still dead after S1, for a second independent reason — both `appendBuildHistory` sites (`lib/build.js:3336`, `:6438`) spread `buildCostSnapshot()`, whose amount is the accumulator's `usd` and therefore always a finite number (S1 routes unpriced spend to `usd_unknown_count`, not to the amount), so the `typeof cost_usd === 'number'` branch always wins with a record present; and with NO record present the token inputs read from that same absent record via `?? 0`, so it could only ever be called as `deriveUsd(model, 0, 0)`. There is no input under which it returns a wrong number. **Probe 2:** the `result-normalizer` fallback is REACHABLE but could only ever return 0 — `codex.ts:492` omits `cost_usd` exactly when stratum cannot price the model, and both tables carried the identical Codex key set with rates now agreeing (stratum's terra/sol are no longer the stale 2.5/15 and 5/30 this design recorded); Claude never reaches it at all, since `claude.ts:171` and `local-claude-connector.js:276` emit `cost_usd` unconditionally. So the filed pricer would have shipped with **zero live callers**. **Not built.** `lib/model-pricing.js` and `lib/experiment-pricing.js` are DELETED instead, along with the consumer-side fallback — compose now prices nothing, every producer states its own cost, and stratum is the sole authority for the one case that needs a table. The prefix defect dissolves rather than being fixed: stratum's lookup is exact after `baseModel()` strips `/effort`, with no `gpt-5` catch-all to fall through to.

  **The one live defect found was not about pricing.** An unpriced step used to add 0 to the run total and label the run `estimated` — a sum SHORT by that step, presented as the total with a soft label. That is the S1 defect one layer up, and it was the last instance. Now a step that moved tokens with no stated cost is COUNTED, the primary usage record OMITS `cost_usd` entirely, and the omission is exactly the signal `recordBuildUsage` (`build.js:4445`) counts as unpriced and `reportUsageReceipts` (`build.js:2300`) refuses to stamp provenance on. The connector-result adoption guard widened from "the events totalled 0" to "the event total is not trustworthy" so that removing the fallback never LOSES a figure the connector actually knew.

  The "every routable model is priced" invariant survives the deletion, retargeted at stratum's table through the test-only deep import this design blessed (stratum publishes no `exports` field, so a break fails compose's suite loudly instead of degrading production silently), plus a control that fails if either compose table comes back. **Two existing assertions were FAKE-PRODUCER tests and were corrected, not weakened:** `cost-tracking.test.js` asserted $0.0105 priced here from a Claude event carrying no cost, and `usage-receipts.test.js` asserted an `estimated` label on the same shape — no real Claude producer can emit it. New `test/unpriced-step-cost.test.js` (5 tests). **Negative control RED, 3 of 5 failing** against a 5/5 green baseline; run by hand rather than through `scripts/negative-control.sh`, because reverting one production file at a time would here break an import and prove nothing — the deleted-file mirror of the new-file case the script's own header calls out. **A SECOND negative control** covers the `experiment-metrics.js` half: restoring it and `lib/experiment-pricing.js` to HEAD reports RED with `actual: 18` where the new contract requires `null` — the empirical half the original "2.58x over" claim never had, confirming the arithmetic was real and unreachable at the same time. Also closes the doc's own untraced lead: the bare `type: 'usage'` stream event is WRITE-ONLY — `result-normalizer.js:526` is its only writer, nothing in `lib/`, `server/` or `src/` reads it, and `build-stream-schema.js` does not define it, so it is never validated. Recorded as an open question (give it a reader or delete it) rather than acted on further. `docs/features/COMP-COST-OWNER/design.md` amended in place: Decision 4's analytics half no longer says "estimate" (as written it instructed a future reader to restore `deriveUsd`), the Files table now records what shipped instead of two files never built, and S4 is retargeted as a stratum-side slice since compose holds no table to keep fresh. Two residual holes filed, not fixed: stratum's `claude.ts:171` emits `cost_usd` unconditionally, so an SDK turn omitting `total_cost_usd` becomes a provider-reported $0 — the S2 defect still live one repo over. Targeted: node 147/147 across unpriced-step-cost, cost-tracking, usage-receipts, build-cost-owner, build-stream-usage-provenance, result-normalizer, model-tiers, plus experiment-model-ab 32/32 and stream-validate/writer-integration 43/43. UI vitest 624/624.

- **COMP-COST-OWNER S2: unknown cost stays unknown all the way to the screen.** A step nobody could price was converted to the number 0 twice on its way out — by the stream writer (`cost_usd: usage.cost_usd ?? 0`) and again by the cockpit bridge (`event.cost_usd ?? 0`) — while `usd_source` was dropped entirely by the bridge's projection. The cockpit then rendered **`<$0.001`** for a call that may have cost dollars, in a formatter that already had a correct `usd == null` branch it could never reach. Now: `writeUsage` emits the amount only when something can state HOW it is known, omitting both keys otherwise; `build.js` derives step-level provenance with a new `aggregateUsdSource` that is sticky and where unknown dominates (any unpriced dispatch makes the step's total not a total, any estimated one makes the sum an estimate, an unlabelled dollar refuses) — the same rule `result-normalizer` applies per run, applied per step, and the same rule `reportUsageReceipts` (`build.js:2280-2283`) already applied to receipts, now applied to the stream. The bridge carries `usd_source` and stops coercing the cost, while tokens still default to 0 because a missing token count genuinely IS zero tokens. **`ContextStepDetail.jsx:280`'s `cost_usd != null` filter starts working with no change to it** — it was always asking the right question, and the bridge was destroying the answer. Also collapses **four** divergent `formatCost` implementations (not the two first counted) into `src/lib/format-cost.js`: they disagreed about exactly the case that matters, rendering the same unknown as `$0.00`, `<$0.001`, `''` and nothing at all. The shared one distinguishes "we do not know" (`—`, or a per-surface marker) from "it genuinely cost nothing" (`$0.00`), keeps per-surface precision, and treats a negative amount as unknown rather than letting it subtract from a displayed total. One existing test updated at the fixture level: `cost-tracking.test.js` called `writeUsage` with a bare `cost_usd` and no provenance, which is now dropped rather than emitted unlabelled. New `test/build-stream-usage-provenance.test.js` (10 tests); **negative control RED on both production files** (4 and 2 failures on revert) against a 10/10 green baseline. Targeted: node 137/137, UI vitest 624/624.

- **COMP-COST-OWNER S1: one owner for the build cost number.** The persisted build accumulator is now the sole owner of what a build cost and every surface reads it. `buildCostTotals` is deleted. It was a second in-memory tally incremented from exactly ONE site (the main step) while `recordBuildUsage` fed the accumulator from nine, including every fix, revise, gate-fix and error-carried dispatch — so repair spend reached the accumulator and the routing ledger and never the history record. Measured on the retained live-fire run: ledger and accumulator agreed to the cent at $1.4257732 while `build-history.jsonl` reported $1.38900475 with 589,373 of 808,002 tokens. Accumulator goes to **v3** with `input_tokens`, `output_tokens` and `usd_unknown_count`; a v2 record migrates with a **null** split rather than a fabricated one, because it knows its combined total but not its split — and null is STICKY, never later topped up into a partial presented as a total. That also removes the old `output_tokens`-seeded-from-`tokens_total` conflation which made a resumed build's split wrong on top of its total. **Unpriced spend is now COUNTED, not added as a zero**, so a reader can tell a free build from one with spend we could not price. Two implementation traps worth recording: (1) the accumulator is DELETED on a complete or aborted terminal (`clearBuildAccumulator`, `build.js:2820`), so `buildCostSnapshot()` keeps a mirror of the owner — load-bearing, not an optimisation, since a bare disk read would report a completed build as free; the mirror only ever holds what the owner last said, so unlike the old tally it cannot diverge. (2) Unknown-cost detection has to read the per-dispatch `usages[]` entries, selected exactly as `reportUsageReceipts` does — the aggregate object seeds its merged `cost_usd` at 0 and only adds, so an unpriced run arrives carrying a numeric `cost_usd: 0` indistinguishable from a measured free call; the honest signal is the OMITTED key on `usages[i]`. Same trap as `usd_source` riding `usages[0]`. The receipt path is deliberately unchanged: it already strips an unprovenanced dollar rather than inventing provenance, and was the reference for all of this. New `test/build-cost-owner.test.js` (10 tests); **negative control RED, 7 of 10 failing when `lib/build.js` is reverted** against a 10/10 green baseline. Targeted suites: 133 pass, 0 fail across build-cost-owner, build-history, usage-receipts, build-output-gate, build-wave-ship, gsd-budget, dispatch-capture. The full suite then caught three stale fixtures no targeted run covered, in `test/dispatch-build.test.js`: two asserted the accumulator version constant directly, and the third seeded `v: 3` as its "unknown future version" probe — a version this change makes CURRENT. All three were updated at the fixture level with their intent intact, not weakened: the v1 migration test now also asserts the null split, and the future-version probe moves to `v: 4` seeded with the full v3 field set, because without those fields the validator would refuse for a missing field and the test would pass without ever reaching the version check it exists to exercise.

- **COMP-PRICING superseded by COMP-COST-OWNER, with the defect now MEASURED.** Tracing every cost value from its set-site to its read-site (rather than from the import graph) found the real defect, and the retained live-fire run measured it. **Three independent totals for one real run: the routing ledger's 13 non-null receipt rows and the two build accumulators agree to the cent at $1.4257732 (delta 0.0000000000), while `build-history.jsonl` reports $1.38900475 — short $0.0367685, 2.6% of spend — and 589,373 tokens against the accumulator's 808,002, short 218,629 (27.1%).** Evidence: `docs/features/COMP-COST-OWNER/evidence/three-totals-2026-09-12.md`. Cause: one measurement fans out to FIVE independent running totals over different event populations with nothing reconciling them. `recordBuildUsage` feeds the accumulator from nine sites including every fix, revise, gate-fix and error-carried usage; `buildCostTotals` — which becomes the history record — is incremented from exactly one (`build.js:5273-5275`, main step only), and is SEEDED from the accumulator at `:3746-3749` on every invocation including resume, so the history cost depends on whether the build was resumed (that half remains read-verified; the measurement owed is a forced repair wave, read, resume across it, read again). Each sink arrived with a different feature (COMP-OBS-COST, COMP-MODEL-AB, COMP-MODEL-ROUTE, resume support), none found an owner to read from, and each added a defensive `?? 0` — five conversions of "unknown" into "free" between the connector and the screen, ending at `<$0.001` on the cockpit for a call of unknown cost, in a `formatCost` that already has a correct null branch it can never reach. The receipt path (`build.js:2280-2283`) is the exception and the model: it strips an unprovenanced dollar rather than inventing provenance. Re-sliced around ownership: S1 one owner distinguishing unknown from zero, S2 stop destroying provenance in transit, S3 the one pricer, S4 the freshness check, S5 the independent transcript oracle. COMP-PRICING recorded as PARKED, not SUPERSEDED — the guard reserves KILLED for the lifecycle path and this feature never entered one, and the transition policy allows only IN_PROGRESS/KILLED/PARKED from PLANNED. Nothing forced. No code changed.

- **COMP-PRICING design amended again — and a CORRECTION to the entry below.** Owner asked why anything would reprice months later given the routing ledger is immutable. It would not, and it is. Two errors found by that one question. **(a) The claim in the previous entry that a live price feed would make `evidence/livefire-2026-09-12/reconcile.mjs` "yield a different total while still exiting 0" is FALSE.** That reconciler reads `r.cost.usd` from persisted ledger rows (`:35`, `:38`, `:40`) and never recomputes a price, so no upstream rate change can move its output — the ledger's immutability already protects it. Decision 3's conclusion (pin the rates, never fetch live) stands on a corrected WRITE-time argument: a live feed prices the same call differently depending on when it was dispatched and leaves no record of which rate was in effect, so the receipt freezes a figure that can never be re-derived from its token counts or audited. Immutability freezes whatever the feed said; it does not make it reproducible. **(b) Effective dating was speculative and is now DEFERRED.** Nothing in either repo reprices anything: `routing-ledger.js` / `routing-runtime.js` / `consumer-fanout.js` contain no pricing call and only read a persisted `usd`, and `experiment-metrics.collect()` is invoked exactly once, inline, at `lib/experiment.js:404` — immediately after the build it measures and before the run record is written at `:431`, with no path that re-reads old artifacts. Every pricing event happens at the moment of the call, so `at` is always "now"; the pricer loses its `at` parameter and `KNOWN_DIVERGENT` resolves to one correct current rate rather than dated rows. "Historical repricing" was the wrong name for what `experiment-pricing.js` does — it prices records that never carried a cost, in the same run that produced them. What remains of the feature is two measured bugs (cache-blind `deriveUsd` at 2.58x over; `gpt-5` prefix precedence), one freshness check, and deleting an unreachable fallback. No code changed.

- **COMP-PRICING design AMENDED same day — the cross-repo question dissolves.** Owner asked why the price table could not simply be exported from stratum. The filed answer ("copies plus a drift test, because stratum's package.json exports nothing") was correct on its narrow point and answered the wrong question. Investigation: NO live path in either repo prices a Claude call from a table — stratum's `claude.ts:171` and compose's `lib/local-claude-connector.js:276` both read the SDK's own `total_cost_usd`. Codex is the only provider needing a computed price, and stratum's connector now always states it (`codex.ts:488`, `:623`), omitting both keys only for a model it cannot price. Compose's consumer-side fallback (`result-normalizer.js:489`) is therefore reachable only for an unpriced model — and compose cannot price it either: the two tables hold the IDENTICAL key set (gpt-5.3-codex-spark, gpt-5.6-luna, gpt-5.6-terra, gpt-5.6-sol, gpt-6-astra) with every rate in agreement, verified by direct diff 2026-09-12. The fallback cannot fire, and its only possible effect is masking drift between the two tables. So Decision 2 reverses from "unify the table" to **delete the live consumer-side pricing path**: stratum owns live Codex pricing (prices at "now", needs no dates), compose owns historical repricing (needs dates, Claude and Codex, retired models included). Nothing is exported because nothing needs importing. The dialect complexity does NOT leave compose, it moves: `deriveUsd` is cache-blind and prices the same live-fire call at **$0.46037775 against the authoritative $0.17813775, 2.58x over** (measured 2026-09-12) — the identical class of error just fixed on the live path, still unfixed on the historical one. New Decision 4 states the two opposite contracts for "unknown cost": live receipts REFUSE and record `missing-usd` (evidence), experiment metrics ESTIMATE at the record's timestamp and label it (analytics). S1 changes from "both tables become callers" to "the live path is removed"; S2 narrows to a single drift test deep-importing stratum's table (verified reachable today, unstable by design — acceptable in a test, not in production); S3 retargets at stratum's table, now the only table on a live path. Recommended order is S3 → S1 → S2, since the live-path bug is dormant while the staleness bug is active. No code changed; `stratum/ts/src/judge/pricing.ts` is explicitly left alone.

- **COMP-PRICING filed (design, PLANNED)**: three hand-maintained price tables collapsed to one effective-dated rates contract. `lib/model-pricing.js`, `lib/experiment-pricing.js` and `stratum/ts/src/judge/pricing.ts` are not three copies of one table but three different functions over two token dialects and three cache models, which is the class defect behind the 2.76x overcharge fixed above. Design carries four findings. (1) The defect splits in two: consolidation fixes the MECHANISM and does nothing for FRESHNESS — terra/sol were stale by two upstream price cuts for ~6 weeks, and one table instead of three is still one stale table. The component is internal; only the data origin is external. (2) A live price feed is ruled out categorically, not on cost: receipts are tamper-evident evidence and `evidence/livefire-2026-09-12/reconcile.mjs` would yield a different total after any upstream change while still exiting 0 — the falsifier would stop falsifying without failing. The external touch is therefore a scheduled CI diff of a pinned snapshot that never reaches runtime. (3) The `KNOWN_DIVERGENT` deferral at `test/model-tiers.test.js:293` rests on a misread and is a bug, not an owner decision: `deriveUsd` takes no timestamp and its only call site (`lib/experiment-metrics.js:224`) supplies none, so that table cannot price historically at all — what it legitimately retains is old model KEYS, not stale rates on live models. (4) "Export the table from stratum and import it" is unreachable: stratum's `ts/package.json` has `main`/`types`/`exports` all null and compose spawns its bin over MCP rather than importing it, so that option means establishing stratum as a dual CLI-plus-library package. Modeling core is effective dating (`model -> [{from, until, ...}]`, priced at the receipt's own timestamp), which turns the terra/sol divergence into two query dates against one history. Three slices, each with a stated negative control. Out of scope: new npm package, stratum-as-library, runtime feed, spark (subscription-billed, no external source) and COMP-MODEL-ROUTE Q3. No code changed. See `docs/features/COMP-PRICING/design.md`.

- **Codex usage events were silently dropped; the stream event now carries STATED provenance.** Two defects with one cause. `lib/build-stream-schema.js` marked `cost_usd` REQUIRED on `step_usage` — a rule inherited from the RETIRED python producer the file still names (`events.py`) — so `lib/stratum-mcp-client.js:477` warn-and-DROPPED every Codex usage event before `lib/result-normalizer.js:481` ever saw it (**6x in the retained 2026-09-12 live-fire run**, `dropping invalid BuildStreamEvent kind=step_usage ... data must have required property 'cost_usd'`). And the producer could not simply send its cost, because `:491` inferred provenance from field PRESENCE — so an honest estimate would have been relabelled as provider-reported spend. Omitting was the only safe option, and omitting got the event dropped.

  Fixed by adding the field that was missing: `usd_source` (`'reported' | 'estimated'`) on the `step_usage` metadata, stated by the producer and read explicitly by the consumer. `cost_usd` becomes OPTIONAL; the three token fields stay required. Both stratum connectors now say how they know (stratum `feat(connectors): step_usage states the amount and its provenance`). `estimated` is sticky across a run: one estimated step makes the whole total an estimate, because a mixed sum cannot honestly be called reported.

  **This also removes a 2.76x overcharge before it could land.** OpenAI reports `input_tokens` INCLUDING `cached_input_tokens`; Anthropic — and therefore `calculateCost` ("Standard prompt tokens", cacheRead billed separately) and stratum's own `claude.ts` — excludes them. Pricing raw Codex numbers double-bills the cached portion: on the live-fire review call (216,385 input of which 179,200 cached, 5,836 output) that is **$0.49173775 against the connector's authoritative $0.17813775**, and `result-normalizer.js:709` only adopts the connector's figure while the event-derived total is zero — so un-dropping the events alone would have let the inflated number OVERRULE the correct one. Because the producer now states the amount, the consumer no longer prices tokens on the happy path and the dialect never enters the cost math. A dialect-aware `calculateEventCost` remains as the fallback for a producer that sends tokens with no cost, pinned by tests against both dialects.

  Two dead ends are recorded rather than repeated. Normalizing the tokens at the producer was tried (stratum `d006278`) and REVERTED (`1c2646c`): it failed 15 tests here, all `ROUTING_CALL_EVIDENCE_CONFLICT: Forwarded tokens differs from original connector evidence` (9 in `test/usage-receipts.test.js:1127`, 5 in `test/integration/build-wave-golden.test.js`, 1 in `test/build-model-route-outcomes.test.js`). `lib/routing-runtime.js:187-196` compares every forwarded field against the connector's own evidence and refuses on any difference — that identity is what makes the routing ledger tamper-evident, and it is the property gate 5 rests on. Token counts therefore stay RAW end to end. Caught by the pre-push gate; it never reached a consumer.

  New tests, each proven non-vacuous by reverting what it guards: the codex-shaped event with `usd_source: 'estimated'` validates (restoring `cost_usd` to `required` fails it with the live-fire error string verbatim); a `usd_source` outside `reported|estimated` is rejected; an estimated cost survives to `usages[0].usd_source` and a mixed run stays `estimated` (making the consumer ignore the stated label fails both); and `calculateEventCost` reproduces the connector's figure for OpenAI-dialect models while leaving Anthropic ones byte-identical.

- **COMP-MODEL-ROUTE gate-5 reconciliation recorded (`evidence/livefire-2026-09-12/reconcile.mjs`).** Complete-plus-excluded now reconciles against unique receipt totals in one command, exiting non-zero on any violation rather than asserting: 16 rows partition **13 complete / 3 excluded**, exhaustive and disjoint; all 16 paid receipt refs unique by BOTH `payloadDigest` and `dispatchId` with exactly one per row, so no row can double-count another's receipt; excluded rows keep `usd: null` rather than a coerced 0; and the complete sum equals the all-non-null sum at **$1.4257732** (reported n=10 $1.0815650, estimated n=3 $0.3442082). Verified as a real falsifier by tampering — duplicating a `payloadDigest` exits 1 with `duplicate payloadDigest — double-count possible`, and flipping an excluded `usd` from null to 0.0 exits 1 with `an excluded row carries a non-null usd`. **Gate 5 is still NOT ticked:** the run produced no repair wave, so the uncredited and failed-repair exclusion paths have no evidence here either way and need a run that actually produces one. The README's zero-based row numbering is now stated explicitly (its rows 1/2/5 are file lines 2/3/6).

- **COMP-MODEL-ROUTE S1b dispatch 3**: extend real-engine goldens with independent GSD off/shadow connector traces, paid bundled-preset evidence, adversarial same-profile A/B receipt ownership across carry reset and late delivery, primary/repair/failure/unknown-cost reconciliation, and GSD original-owner continuation/terminal coverage. All three frozen off fixtures remain unchanged. Final golden verification: **37 passed, 0 failed, 0 cancelled**; observation reversion and evidence-check-removal controls are recorded in `docs/features/COMP-MODEL-ROUTE/reports/slice1b-d3-impl.md`. Document shadow joins, outcomes, unsupported rows and incomplete ledger evidence. No production behavior changed. **Live-fire completion evidence remains OUTSTANDING**; no provider or full npm suite was run, and the parent feature remains incomplete pending S2/S3. No report/calibration/feedback/learned-selection surface is claimed.

  Host adjudication found two TEST defects the dispatch could not observe, neither in production code. The first HOST run of the three golden files was **36 / 31 pass / fail 0 / cancelled 5 at `duration_ms 900008`** — the whole-file `--test-timeout` cap, i.e. a hang wearing the mask of four failures. Cause: these goldens reach a real `execute_merge` gate and `lib/build.js:5915` delegates a gate to the web UI whenever `probeServer()` answers on `resolvePort()` (`COMPOSE_PORT > PORT > 4001`). Only `npm test` escapes it, via the `--import ./test/suppress-expected-drift.js` preload (`package.json:23`) that sets `COMPOSE_PORT=19997` — so the targeted golden command documented in the blueprint was itself the trap on any machine running the dev server, while the dispatch sandbox could not reproduce it because it cannot bind ports. Fixed as a control rather than a note: `test/helpers/build-wave-golden-fixture.js` sets `COMPOSE_PORT=19997` when unset, all three golden files import that helper, and an explicit value is never overridden. With the hang gone, two tests then failed for real (`fail 2`, `cancelled 0`): `build-wave-golden.test.js:344` pinned the LAUNCH order of A0/B0, which the latch at `build-wave-golden-fixture.js:413` deliberately leaves free — it waits until both have arrived and forces only the RETURN order — and which contradicts this gate's own requirement that ownership survive adversarial A/B ordering. Launch order is now compared as a multiset; the forced `returned` sequence stays exact; mutating one expected member gives 1/0/1/0, so the relaxed assertion is not vacuous. **Final host tally with the server UP on 4001 and `env -u COMPOSE_PORT -u PORT`: 37 / 37 / 0 / 0 in 271s.**

- **COMP-ROADMAP-SILENT slice-1 blueprint (design only, no implementation)**: the design's V1 crossed migration, local-mutation, graph, projection, read, xref and transport boundaries behind 14 acceptance gates, which makes the first useful correction depend on all of them. Carved into seven sequential slices, each with a standalone pass gate and a stated negative control, and a coverage table assigning every one of the 14 original gates to a named closure slice — S1 closes **AC6 only**, contributing bounded evidence to AC1/2/3/5/10/12/13/14 and explicitly satisfying none of the rest. S1 is truthful migration through the real CLI and direct library entry: contract, strict parser observation, a migration-only registered executor with a guarded write sink, receipt-derived finalization, in three sequential dispatches. Two corrections to the design verified against source: the parser initializes `position` at 0 and post-increments (`lib/roadmap-parser.js:60,178`) while `contracts/roadmap-row.schema.json:21` requires `minimum: 1`, so validating raw parser rows against the shared row schema would reject the first normal row — S1 therefore defines `MigrationParsedRow` with the producer's zero-based ordinal and files shared-schema convergence as a follow-up; and the graph check is misleading readiness advice rather than a literal exit-0 regression, since `bin/compose.js:1611-1619` already exits 1 on byte difference. Instance C reproduces exactly as reported (anonymous skip `lib/migrate-roadmap.js:47-49`, write at `:102`, unconditional `process.exit(0)` at `bin/compose.js:1509`). Four owner questions all ruled 2026-09-12 and recorded in §8 of the blueprint: migration REFUSES on mixed/unclassified prose rather than migrating a partial subset (availability traded for truthful coverage until PROSE-READ ships); legacy `created`/`updated` collection names survive one documented minor-version interval but carry only verified applied receipts; no shared row-position schema change in S1; and the future `inspect` command performs zero filesystem writes including temporary stores. No contract, code, test or feature status changed; no tests run.

- **Roadmap migration follow-ups filed (3 features, PLANNED)**: surfaced by a SmartMemory investigation that re-scoped its own `ROADMAP-MIGRATE-1` and found most of the mechanism was compose's. `COMP-ROADMAP-PROSE-READ` — `parseRoadmap` (`lib/roadmap-parser.js:54`) recognizes only `## `/`### ` headings and `|…|` table rows; everything else hits `continue` at `:125-131`, so a `### CODE — Title` section becomes a phaseId string and never a feature. On SmartMemory's 6,917-line roadmap it parsed 114 entries, 0 real codes, all `_anon_`, wrote 0 files, exited 0. `COMP-ROADMAP-ORDINAL` — 336 force-rank ordinals (188 distinct) exist in no other artifact and are cited as IDENTIFIERS by documents that survive migration (`CORE-VEC-DOMAINS-1/plan.md:9,:174` cite `25f6`), so dropping them breaks inbound references, not just sort order; design found the proposed single `legacyOrdinal` field insufficient because one feature carries different ordinals in different sections, and specifies a retained import ledger instead. `COMP-ROADMAP-SILENT` — operations that match nothing report success; design verified the three reported instances, qualified one (the schema permits the cited status/phase pair and startup seeding assigns it), found a fourth (`changed:false` on a status update with no supported row) plus swallowed scan/read errors and xref "nothing to reconcile" over wholly unresolved targets, and corrected the drift census to 141 disagreements across 503 overlapping records. Named after compose's flat-code convention rather than the proposed `COMP-ROADMAP-MIGRATE-2`, since `COMP-ROADMAP-MIGRATE` has no `-1` sibling and `ROADMAP-MIGRATE-1` is SmartMemory's code; lineage carried on `surfaced_by` plus an external `local` link. 32 acceptance gates across the three designs, each with a stated negative control. Also records, without new tickets: graph label parity on `COMP-ROADMAP-GRAPH-3` (labels hard-coded to folder name at `server/feature-scan.js:538`, so generated nodes render `CODE\nCODE` and 370 curated labels have no source field), and round-trip safety on `COMP-ROADMAP-MIGRATE` (6,915 → 6,995 lines, 80 additions, zero deletions with guards bypassed — preservation is structural, removing the migration's main data-loss risk).

- **COMP-MODEL-ROUTE S1b dispatch 2**: runtime observation ENABLED — the dispatch-1 primitives now record. Call intents are minted at the connector boundary alone (`lib/stratum-mcp-client.js` `#dispatchAgentRun`, `lib/local-claude-connector.js` `runLocalClaudeAgent`); normalizer forks and `runAgentText` only pass bindings, so one dispatch cannot be counted twice. Adds `lib/routing-runtime.js` and `lib/routing-gates.js` over the dispatch-1 APIs: `context.routingCalls` sink, receipt `detail.routing` join, pre-reset wave capture at the top of `resolveGateWithConsumerMerge` (before prepareMerge, not the post-reset accepted read), gate acknowledgement distinguishing durable token response / validated engine witness / ordinal / unconfirmed, the token-absent FAILURE settlement path, cancellation settled on run-cancellation audit plus per-call termination (never an acknowledged `stepDone`), and RECORDING-only partition/lineage checks in `lib/output-gate.js`. All five journal-guard hardening sites fixed (seven dereferences). Engine receipts are read in their persisted `ReceiptRecord.amount` shape with nested `detail.item` fanout fields. New `test/routing-calls.test.js`, `test/build-model-route-outcomes.test.js`, `test/gsd-model-route-outcomes.test.js`, `test/helpers/real-codex-tool.js`; `test/usage-receipts.test.js` and ten adapter call sites extended.

  Astra review r1 found 6 (1 P1: an unresolved billable child still settled its parent accepted/positive; a second repair lost lineage; receipt forwarding compared only USD and total tokens; plus three test gaps, one where deleting the production recovery callback at `lib/build.js:4157` left all 18 cancellation tests green). Fix round closed them and correctly REJECTED part of finding 4 with evidence. r2 then found that two of those fixes had introduced regressions: the new literal model comparison rejected ordinary unchanged Codex output (`gpt-5.4/high` in the usage event vs `{model:'gpt-5.4', effort:'high'}` in telemetry — a real $0.20 call lost its settlement), and per-issuance retirement erased residual ownership a successor never assumed, bypassing the legitimate multiple-owner refusal. Both fixed: the comparison follows the producer's first-slash grammar (`../stratum/ts/src/connectors/base.ts:73`) without hardcoding model names, and retirement is now per-file, dropping an item only when every file it owned was assumed by an acknowledged successor. Narrow r3 over just those two fixes: CLEAN.

  Verification is negative-control based, not pass-count based. `scripts/negative-control.sh` (new) reverts one production file at a time and classifies by node's own TAP tally — RED / GREEN / CANCELLED / DID-NOT-RUN — because a missing file, a timeout and a real assertion failure all exit non-zero and mean different things. Host: all ten modified production files go RED when reverted; 259 tests across 11 files, 0 fail, 0 cancelled. Both "did not weaken it" controls reproduce exactly: removing the added evidence comparisons gives `# fail 17`, removing the multiple-owner refusal gives `# fail 1`, so neither fix bought green by disabling the check it was meant to preserve. The strengthened receipt tests now drive the real `CodexConnector` through `test/helpers/real-codex-tool.js`; against the previous synthetic fixture the unchanged-input population was 9 pass / 9 fail, which is the seventh instance of the fake-producer pattern on this feature and the reason the r2 P1 escaped. Three frozen 0.5.1 off-mode fixtures byte-identical. No report product, learned selection or repair-floor enforcement (d3, S2, S3). **The pre-push full suite then blocked the push** (7072 tests, 2 fail) on a regression no targeted run could see: the new unconfirmed-termination guard in `runConsumerIssuance` landed AHEAD of the pre-existing `AgentAbortedError` -> stuck-halt conversion, so a stuck detector aborted the run with exit 1 instead of writing `stuck.json` and exiting 0 (`test/integration/gsd-route-continuation-golden.test.js:280`). Fixed by exempting only `AgentAbortedError` with `reason.stuck === true`, ordered AFTER the integrity check so an integrity refusal carrying that same type and reason still escapes. Bisect confirmed the regression predated the round-2 fixes. Using `scripts/negative-control.sh` in anger also exposed a defect in it: it reverted via `git checkout`, which stages; it now reads historical bytes with `git show` and leaves the index untouched.

- **COMP-MODEL-ROUTE S1b dispatch 1**: routing join, outcome and ledger primitives with runtime observation still DISABLED. Adds `contracts/routing-join.schema.json` (call intent/resolution, a dedicated `receiptRouting` definition for `detail.routing`, engine-receipt evidence) and `contracts/routing-outcome.schema.json` (pre-reset wave snapshot, gate disposition and acknowledgement, lineage link, one evidence-derived outcome per issuance); extends `routing-record.schema.json` with versioned ledger rows, receipt-owner and evidence-checkpoint records. `lib/routing-ledger.js` gains the validators, per-issuance observer factory, append-only evidence chains, the project-locked `.compose/routing/ledger.jsonl` writer and latest-validated-version reader, and the pure label, context and eligibility functions; `lib/consumer-fanout.js` gains atomic issuance-plus-metadata, routing receipt spooling and owner-journal locking. Ledger rows carry a `context` repair stratum (`waveKind`, `ancestryUnknown`, `repairOfRecordId`, `repairDepth`) recomputed from persisted admission and lineage records, so a repair-wave sample is distinguishable from a fresh one; unknown ancestry stays censored across retries. Journal immutability and ledger revision remain separate contracts. Astra impl review r1 (3 HIGH / 1 MEDIUM: a seeded lineage relation overrode contradictory gate evidence, engine-receipt spend was discarded, a retry erased unknown-ancestry censoring, and the embedded ledger outcome skipped the label/binary invariant), fix run, r2 (2 HIGH: the engine fix projected the submission shape rather than the persisted `ReceiptRecord.amount`, and refused legitimate `legacy` provenance), second fix run which also caught a third instance in partial estimated receipts. Off mode is unchanged: the three frozen 0.5.1 fixtures are byte-identical and no off-mode path allocates routing storage, a ledger or journal routing fields. Host: node 6983/6983, UI 624/624, tracker 100/100. No Build/GSD, connector or normalizer wiring yet (dispatch 2); no report, learned selection or repair-floor enforcement.

- **COMP-MODEL-ROUTE S1b blueprint (review CLEAN at r2)**: implementation plan for attributable shadow outcomes — per-call intent/resolution observation at the connector boundary for primary, failure and normalization-repair calls on both transports; `compose:route` metadata receipts and a validated `detail.routing` join spooled independently of `_costCeiling`; unsupported observations recorded with a null issuance rather than an invented one; pre-reset wave capture at gate-closure entry selected from the gate’s reset dependency closure; gate disposition with token-bound-or-ordinal reconciliation; the six D6 acceptance labels over one acceptance-eligible population; and an append-only `.compose/routing/ledger.jsonl` whose revision semantics are deliberately distinct from the journal’s immutable-bytes rule. Three dispatches, five journal-guard hardening sites carried in, two owner questions open (executed-tier evidence on the local SDK path, failure-settlement scope). A read-only evidence pass first found 5 of the 14 code anchors cited by design Decisions 5/6/7 outright stale and one misdescribed (normalization repair has one transport, not two). Astra blueprint review r1 (5 HIGH / 6 MEDIUM / 1 LOW, all accepted), fix run, r2 CLEAN. No implementation yet; live-fire shadow evidence remains outstanding. See `docs/features/COMP-MODEL-ROUTE/blueprint-slice1b.md` and `reports/slice1b-evidence.md`.

- **COMP-MODEL-ROUTE S1a dispatch 3**: real-engine goldens consume the frozen 0.5.1 bundled Build, carry and GSD-input fixtures; compare complete serialized calls with cwd/workspace, UUID and TAP-duration substitutions and order tolerance within concurrent waves; assert shadow records, ordinary epochs, GSD continuation and recovery through real halt/crash writers. The goldens found two production defects, both fixed in this change: D3-1 shadow admission re-validated concurrently settled siblings in a ready batch (`ROUTING_BINDING_DRIFT` at concurrency 3); D3-2 a stale GSD `pause.json` outranked a newer crashed continuation, losing lineage; participating GSD resume now follows recorded run lineage while non-participating off-mode resume is unchanged (see `docs/features/COMP-MODEL-ROUTE/reports/slice1a-d3-impl.md`, section "Defect fixes (D3-1, D3-2)"). S1a ships static shadow routing only: no receipts, ledger, report, learned or active selection.

- **COMP-MODEL-ROUTE S1a dispatch 2**: Build and GSD carry an immutable routing start record and root digest in their fresh plan inputs (optional declarations in both bundled specs, no default-valued keys injected); recorded-role resume validates the persisted root and binding before any model call and never re-resolves from current flags; fresh-after-terminal-old-flow re-resolves the current invocation's routing options; per-epoch ordinary admission entries; route-only fanouts (no `tier_from`, no `_consumer`) enter whole-wave admission in both runners with the source binding independent of consumer epoch (GSD merge-revise admits); deferred multi-stage/engine fanouts keep legacy bindings under a persisted deferred-step identity; stage-aware route map consulted at admission (static baseline in S1a); GSD continuation intent before plan with old→new run binding and three-run logical identity; reserved routing transport refused in model-facing interpolation (`ROUTING_TRANSPORT_EXPOSED`); customised GSD ordinary profile refused (`ROUTING_STATIC_DISPATCH_MISMATCH`); active/trials/exploration/feedback refused (`ROUTING_SLICE_UNAVAILABLE`); routing storage excluded from consumer-journal snapshots and never recreated when lost. `team-fable-astra` ships `_routing:{mode:"shadow"}` with `plan`/`execute` `route.learn`; bundled GSD stays off. Off mode reproduces the frozen 0.5.1 bundled/carry traces; shadow makes identical model calls. Astra impl review r1 (4 HIGH / 1 MEDIUM), r2 (1 HIGH), r3 gate; all fixed.
...
- **COMP-MODEL-ROUTE filed (design, PLANNED)**: dynamic model routing framework for agent dispatches — one resolver per dispatch key with an ordered value chain (manual fallback, opt-in learned, preset, spec default), immutable start record and root-bound route journal, receipt joins with downstream acceptance labels, executed-tier repair floor as an admission rail, recorded cold-start trials, separately gated planner calibration feedback. Slices S1a/S1b/S2/S3; bundled preset ships shadow. Supersedes COMP-FABLE-CALIBRATE, killed 2026-09-11 through the guarded lifecycle path (`explore_design` → `killed`) with the rationale in `docs/features/COMP-FABLE-CALIBRATE/killed.md`. Design gate: three Codex gpt-6-astra review rounds, REVIEW CLEAN at round 3; reports under `docs/features/COMP-MODEL-ROUTE/reports/`.
