...
- **`budget` tier added (Codex-only), routing `gpt-5.6-luna`**: luna was PRICED but UNROUTABLE — present in `lib/model-pricing.js` and `stratum/ts/src/judge/pricing.ts`, absent from `server/model-tiers.js`, and profile strings are a closed `provider:template:tier` set (`lib/agent-string.js`), so no spec could name it. Added as a fifth tier rather than as a model name: tier names denote INTENT, and the vocabulary is frozen into persisted routing records, so a tier called `luna` would either lie or force a rewrite of every spec once luna is superseded. `budget` is the exact mirror of `coordinator` — a shared tier name that is null for the other provider — so `claude::budget` fails with the existing "not available for provider" message rather than "unknown tier", and `codex::budget` resolves to `gpt-5.6-luna` at medium effort (the fast-tier convention: the tier picks a cheap MODEL, it does not floor reasoning). `fast` deliberately stays `gpt-5.3-codex-spark` despite luna's lower sticker price, because spark draws a SEPARATE upstream quota; swapping them would lower the nominal rate and raise real spend.

  **Scope: `budget` is ADDRESSABLE, not LADDERED.** It is deliberately absent from the cost ladder in `lib/routing-ledger.js:242,261,266` and the item-tier list in `lib/pipeline-profiles.js:100,154,194`, so it is nameable in a profile sidecar but is not an auto-escalation candidate and does not enter floor computation. Ladder membership is an S2/S3 decision entangled with open question Q3 and is NOT answered here.

  Three latent drift sites were found and closed, two of which the tier vocabulary had already outgrown. (1) The tier names are pinned in **strict JSON Schema enums** — four sites across `contracts/routing-record.schema.json` and `contracts/routing-start.schema.json` — so a tier added to the table but not the schemas makes every record carrying it fail validation at the ledger boundary; a new drift control in `test/model-tiers.test.js` walks both schemas and asserts every tier enum equals `[null, ...Object.keys(MODEL_TIERS)]`, and goes 1 RED when one enum entry is removed. (2) `lib/model-router.js:94` validated `prior` against a hand-kept copy of the tier list; it now DERIVES from `MODEL_TIERS` and cannot drift (model-tiers.js has no imports, so the module stays pure). (3) `test/agent-string.test.js` asserted every tier in the table validates for Claude — an invariant `budget` legitimately breaks as the first Claude-null tier; corrected at the intent level (skip provider-unavailable tiers) plus a mirrored Codex loop and an explicit `claude::budget` rejection test, rather than by weakening the assertion.

  **A silent 35.7x mispricing was made reachable and then closed.** `lib/experiment-pricing.js` falls back to the first key that PREFIXES the model ID, and the legacy `gpt-5` key prefixes every gpt-5.x model. `gpt-5.6-luna` had no exact key, so it fell through to `gpt-5` and priced at 10/40 instead of 0.2/1.2 — $50 vs $1.40 per 1M in / 1M out — with no null to flag it (`gpt-6-astra` returned null honestly, having no prefix match at all). Both are now exact keys placed ABOVE the `gpt-5` catch-all. This was filed as a follow-up before it was reachable; making `budget` addressable is what put an experiment on that path, so it is fixed here rather than re-filed. A second new control pins every routable tier model to an exact entry in BOTH pricing tables and goes RED on a prefix fallback; terra and sol are carried as a named `KNOWN_DIVERGENT` allowlist because reconciling their pre-cut rates against the experiment table's documented historical-receipt role is still an owner decision.

  Host verification, classified by node's TAP tally rather than exit code. Targeted: `model-tiers` + `agent-string` + `routing-ledger` + `pipeline-profiles` + `profile-preflight` + `model-pricing` = **164 tests, 164 pass, 0 fail, 0 cancelled**; with `experiment-model-ab` and the extended tier file, 73/73. Wide (16 routing/model files) = **319 tests, 319 pass, 0 fail, 0 cancelled in 199.8s**. The FIRST wide run reported `309 pass / fail 0 / cancelled 1` at `duration_ms 900010` — the whole-file `--test-timeout` cap, i.e. the known `execute_merge` hang (`lib/build.js:5915` delegates to the web UI and polls for a human whenever a server answers on `resolvePort()`), not a defect in this change: 15 of those 16 files do not import the `test/helpers/build-wave-golden-fixture.js` guard, and the owner's dev server was up on 4001. Re-running the identical set under `COMPOSE_PORT=19997` gave 0 cancelled and cut wall time from the 900s cap to 200s; the test COUNT rose 310 -> 319 because the hung file aborted before registering its remaining subtests. Node exited 0 on the cancelled run, so the exit code was not evidence either way.

  Both new controls were proven non-vacuous by reverting exactly what they guard: deleting one `"budget"` entry from a contract enum gives `fail 1` ("routing-record tier enum drifted from MODEL_TIERS"), and removing the `gpt-5.6-luna` key from experiment-pricing gives `fail 1` ("prices differ between the two tables"). Nothing routes to `budget` by default; no existing tier resolution changed.

- **Codex spend was unattributable; pricing corrected against external sources**: Codex reports no cost at all (its usage events carry token counts only, verified against two real astra runs; the CLI has no built-in cost tracking and OpenAI's billing views aggregate by day with no per-session attribution), so a Codex call's USD must be ESTIMATED from tokens. `lib/model-pricing.js` — the table feeding routing usage evidence via `lib/result-normalizer.js:492` — contained **zero `gpt-` entries**, so no Codex call could carry a cost. That was never a false charge: a zero total makes `result-normalizer.js:733-735` omit `cost_usd`, `routing-runtime.js:55` yields null, and `routing-ledger.js:1286` marks the sample incomplete with `missing-usd` (verified by direct probe: `usd = null, presence.usd = false`). Honest, but it meant no Codex call could ever be a COMPLETE attributable sample, which is exactly what COMP-MODEL-ROUTE gate 5 requires. Adds astra, sol, terra, spark and luna. Prices verified against the LiteLLM registry (3,889 keys, downloaded and grepped — a WebFetch of that file returns a truncated fragment and wrongly reports these models absent) and OpenAI's rate card: **terra and sol were STALE in every table in the repo** — terra cut to 2/12 on 2026-07-30, sol to a promotional 4/20 on 2026-08-21 — overstating spend by ~20-33%. `lib/experiment-pricing.js` and `stratum/ts/src/judge/pricing.ts` still hold the stale figures and are filed as follow-ups, along with the class defect (three hand-maintained price tables drifting independently) and sol's promotional expiry (re-check on 2026-11-21). `calculateCost`'s 0-for-unknown return was deliberately LEFT ALONE after verifying it is safe. Pinned by `test/model-pricing.test.js` (30/30; the codex-coverage assertions go 4/0/4/0 RED when the entries are removed, 23 pre-existing green). No live-provider run was made; gate 5 remains OUTSTANDING.
