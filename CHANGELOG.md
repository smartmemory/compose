...
- `findStaleArtifacts(featureDir)` — the changed-file-free form of the reachability query (an artifact is stale when any upstream it wasDerivedFrom is newer than it). The build's `doc_freshness` health signal now uses it instead of the phase-marker `checkStaleness`, which read a `<!-- phase: -->` marker no production writer ever emitted (a dead signal that always scored fresh). `findStaleArtifacts` works off the canonical chain + mtimes, so the build's freshness score now reflects real derivation staleness. `staleness.js` is retained but is now consumed only by the gate-context warning in `step-prompt.js` (still the dead reader there — a separate follow-up).
