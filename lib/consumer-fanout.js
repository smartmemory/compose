...
import { validateRoutingRecord, validateRoutingJournal, initializeRoutingJournal, routingIssuanceState } from './routing-ledger.js';
import { canonicalRoutingJson, routingRefuse } from './model-router.js';
...
export class ConsumerFanoutArtifacts {
  constructor({ runId, targetCwd, artifactRoot, hooks = {}, revisionDigest, specDigest, profilesDigest, routingBinding, routingAncestry }) {
  ...
  #reload() {
    ...
    if (this._routingBinding || journal.routing) {
      ...
      for (const [token, binding] of Object.entries(journal.dispatchBindings ?? {})) {
        if (!binding.routing) routingRefuse('ROUTING_BINDING_MISSING', 'Participating dispatch routing link missing');
  ...
  #load() {
    if (!existsSync(this.journalPath)) {
      ...
      if (journal.routing) this.hooks.beforeRoutingWrite?.();
      this.#writeGuarded(journal);
      if (journal.routing) this.hooks.afterRoutingWrite?.();
      return journal;
    }
  ...
  #routingPut(record) {
    ...
    if (previous && canonicalRoutingJson(previous) !== canonicalRoutingJson(record)) routingRefuse('ROUTING_BINDING_DRIFT', 'Immutable routing record changed');
  ...
  recordRoutingIssuance({ issuance }) {
    return this.#mutate(() => {
      const routing = this.journal.routing;
  ...
  recordDispatchBinding({ dispatchToken, itemBinding, resolvedProfile, routing }) {
    return this.#mutate(() => {
      ...
      } else if (this.journal.routing) routingRefuse('ROUTING_BINDING_MISSING', 'Participating dispatch requires a routing link');
