...
export function buildStepPrompt(stepDispatch, context) {
  ...
  if ((Array.isArray(outputFields) && outputFields.length > 0)
      || (outputFields && typeof outputFields === 'object' && Object.keys(outputFields).length > 0)) {
    const fieldLines = Array.isArray(outputFields)
      ? outputFields.map(f => `- ${f.name} (${f.type})`).join('\n')
      : Object.entries(outputFields).map(([name, type]) => `- ${name} (${type})`).join('\n');
