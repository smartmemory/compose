...
// COMP-COST-OWNER: how a row's `usd` was arrived at. A connector that reports its
// own cost is `reported`; one priced from token counts is `estimated`. The
// distinction is load-bearing — a consumer may reasonably refuse to reconcile
// spend against an estimate, and stratum 00ff4db began estimating codex costs
// (codex reports none of its own), so without this field an estimate is
// indistinguishable from a provider figure. null means the cost is unknown.
export const USD_SOURCES = Object.freeze(['reported', 'estimated']);
...
const EVENT_FIELDS = {
  dispatch: {
    ...
    optional: [
      ...
      'tokens_total', 'usd', 'usd_source', 'duration_ms', 'note',
...
function validateEvent(event, { allowExtra = false } = {}) {
  ...
  switch (event.kind) {
    case 'dispatch':
      ...
      optionalNullableEnum(event, 'usd_source', USD_SOURCES);
...
function optionalNullableEnum(event, field, values) {
  if (Object.hasOwn(event, field) && event[field] !== null && !isEnum(event[field], values)) {
    invalid(`${field} must be one of ${values.join(', ')} or null`);
  }
}
