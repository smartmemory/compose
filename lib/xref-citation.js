 ...
 *   provider   = "github" | "forgejo" | "local" | "url" ; resolvable
...
const RESOLVABLE_PROVIDERS = ['github', 'forgejo', 'local', 'url'];
 ...
 * @property {string|null} repo     github/forgejo "owner/name" | local repo token | null
 * @property {number|null} issue    github/forgejo only
...
function parseOne(raw) {
  ...
  if (provider === 'github') {
    ...
    // No `#` in either repo half — `#` delimits the issue (owner/name#issue)
    // to keep the citation equivalent to the feature.json-link writer.
    const fj = target.match(/^([^\s/#]+\/[^\s/#]+)#(\d+)$/);
    if (!fj) {
      throw new ParseError(raw, `forgejo target must be "owner/name#issue", got "${target}"`);
    }
    ref.repo = fj[1];
    ref.issue = Number(fj[2]);
    if (expect !== null) {
      if (!GITHUB_EXPECT.has(expect)) {
        throw new ParseError(
          raw, `forgejo expect must be open|closed, got "${expect}"`,
        );
      }
      ref.expect = expect;
    }
