...
const RESOLVABLE_PROVIDERS = new Set(['github', 'local', 'url']);
const RESERVED_PROVIDERS = new Set(['jira', 'linear', 'notion', 'obsidian']);
// url-class = `url` + every reserved provider (carry a url_target, never resolved in v1).
const URL_CLASS_PROVIDERS = new Set(['url', ...RESERVED_PROVIDERS]);
