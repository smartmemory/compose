...
async function listJson(cwd, args, deps) {
  ...
  const { stdout } = await execFileAsync(
    ...
    { cwd, env, timeout: TIMEOUT_MS, maxBuffer: 4 * 1024 * 1024 },
  );
...
export async function learnSummaryLines(cwd, deps = {}) {
  ...
  // slices 4–5 + INLINE-TS-1). An older CLI ignores those flags and lists raw candidate rows,
  // so the Stratum dependency must be bumped before this ships. Review rows are checked for
  // a known kind as a partial guard; unreviewed rows cannot be told apart from raw ones.
...
export async function printLearnSummary(cwd, deps = {}) {
  ...
  try {
    const lines = await learnSummaryLines(cwd, { ...deps, warn: deps.warn ?? ((message) => console.warn(message)) });
