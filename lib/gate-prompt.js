...
// COMP-PLAN-GATE-LOOP (secondary): default deadline for a gate prompt that is
// reading the real stdin in a non-interactive context. Long enough for a piped
// producer to answer, short enough that a backgrounded/no-input runner fails
// fast instead of blocking forever.
export const GATE_NONINTERACTIVE_TIMEOUT_MS = 120_000;

const GATE_NONINTERACTIVE_HELP =
  'stdin is not a TTY and the Compose server is unreachable, so the gate cannot ' +
  'be answered. Start the :4001 server, run the gate interactively, or pass --all ' +
  'to auto-approve. The flow state is preserved — re-run with --resume.';

export class GateInputUnavailableError extends Error {
  constructor(reason) {
    super(`Gate decision required but ${reason}. ${GATE_NONINTERACTIVE_HELP}`);
    this.name = 'GateInputUnavailableError';
  }
}

/**
 * Read one line from the readline interface.
 *
 * When `armGuard` is set (real stdin, no TTY — see promptGate) the read is
 * bounded: it rejects with GateInputUnavailableError if the input stream ends
 * (EOF, e.g. stdin is /dev/null) or no line arrives within `deadlineMs`, so a
 * non-interactive runner fails fast instead of blocking forever. Piped input
 * still resolves normally; an unguarded read keeps the original behavior.
 *
...
export async function promptGate(gateDispatch, { input, output, artifact, askAgent, gateExtras, nonInteractive, timeoutMs } = {}) {
  ...
  // COMP-PLAN-GATE-LOOP (secondary): only bound the read when there is no
  // interactive terminal AND we're on the real stdin. A real TTY (a human who
  // may take their time) and an injected `input` (tests, piped/programmatic
  // callers) keep the original block-until-answered behavior.
  const askGuard = {
  ...
  try {
    ...
    if (notes.length > 0) {
      ...
      const addMore = await ask(rl, '  Additional rationale (or Enter to use notes): ', askGuard);
      if (addMore.trim()) {
        rationale += '\n' + addMore.trim();
      }
      ...
      while (!rationale) {
        const raw = await ask(rl, 'Rationale: ', askGuard);
        const trimmed = raw.trim();
        if (trimmed) {
          rationale = trimmed;
        } else {
