...
/**
 * Assert every same-project link target (non-external to_code) exists.
 * No-op (zero I/O) when the feature carries no such links — the common write.
 * Throws FeatureWriteValidationError('DANGLING_LINK_FEATURES_TARGET') for any
 * missing target unless opts.allowForwardRefs is set (intentional forward refs,
 * e.g. linking A→B before B is scaffolded).
 *
 * @param {string} cwd
 * @param {object} feature
 * @param {{allowForwardRefs?: boolean}} [opts]
 */
