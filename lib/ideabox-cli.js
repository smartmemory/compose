...
export async function runIdeaboxCommand(cwd, args, opts = {}) {
  ...
  switch (sub) {
    ...
    case 'kill': {
      ...
      // Killing an already-killed idea is a no-op, matching the legacy helper.
      // Not politeness: every command writes its record before re-rendering, so
      // "record committed, render failed" invites a retry — and an unconditional
      // write would replace the original date and reason, most likely with
      // "(no reason given)". A kill is dated evidence; a retry must not rewrite
      // it. The render still runs, so the retry does finish the job it failed at.
      if (idea.status === 'killed') {
        await render();
        console.log(`${idea.handle} was already killed on ${toMarkdownDate(idea.killed?.at)}: ${idea.killed?.reason ?? ''}`);
        return 0;
      }
      await provider.updateRecord(idea.handle, {
        status: 'killed',
