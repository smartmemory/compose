...
export const ROUTING_TRANSPORT_KEYS = Object.freeze([
  'route_mode', 'routing_start', 'routing_root', 'routing_plan_intent', 'routing_continuation',
]);
const transport = ROUTING_TRANSPORT_KEYS;

/**
 * Explicit routing-observation decision for every shipped lifecycle spec.
 * The shipped-spec oracle requires this inventory to stay exact, so a new spec
 * cannot silently opt in or out. Reasons make participation a reviewed product
 * decision instead of a filename convention.
 */
export const ROUTING_SPEC_PARTICIPATION = Object.freeze({
  'pipelines/bug-fix.stratum.yaml': Object.freeze({ participates: true, reason: 'diagnosis, repair, verification, and review dispatch model work' }),
  'pipelines/build-quick.stratum.yaml': Object.freeze({ participates: true, reason: 'the ordinary quick build path dispatches implementation and review model work' }),
  'pipelines/build.stratum.yaml': Object.freeze({ participates: true, reason: 'the ordinary full build path dispatches design, implementation, and review model work' }),
  'pipelines/content.stratum.yaml': Object.freeze({ participates: true, reason: 'content drafting and review dispatch model work' }),
  'pipelines/coverage-sweep.stratum.yaml': Object.freeze({ participates: true, reason: 'coverage analysis and implementation dispatch model work' }),
  'pipelines/gsd.stratum.yaml': Object.freeze({ participates: true, reason: 'decomposition, task execution, and review dispatch model work' }),
  'pipelines/new.stratum.yaml': Object.freeze({ participates: true, reason: 'project kickoff research, brainstorming, roadmap, and scaffold dispatch model work' }),
  'pipelines/plan.stratum.yaml': Object.freeze({ participates: true, reason: 'planning, convergence, and handoff dispatch model work' }),
  'pipelines/refactor.stratum.yaml': Object.freeze({ participates: true, reason: 'multi-stage refactoring and cross-model review dispatch model work' }),
  'pipelines/research.stratum.yaml': Object.freeze({ participates: true, reason: 'research and synthesis dispatch model work' }),
  'pipelines/review-fix.stratum.yaml': Object.freeze({ participates: true, reason: 'independent review and corrective implementation dispatch model work' }),
  'presets/team-fable-astra.stratum.yaml': Object.freeze({ participates: true, reason: 'coordination, fanout implementation, verification, and review dispatch model work' }),
  'presets/team-feature.stratum.yaml': Object.freeze({ participates: true, reason: 'team planning, fanout implementation, and verification dispatch model work' }),
  'presets/team-research.stratum.yaml': Object.freeze({ participates: true, reason: 'team planning, research fanout, and synthesis dispatch model work' }),
  'presets/team-review.stratum.yaml': Object.freeze({ participates: true, reason: 'review triage, lens fanout, and synthesis dispatch model work' }),
});
...
export function routingExecutedTier(start, intent, resolution) {
  // Keep nullable tier semantics for eligibility; explicitly label unknown
  // execution without filling it from the selected profile.
  const unavailable = reason => ({ value: null, status: 'unknown', source: reason, evidenceRefs: resolution ? [resolution.id] : [] });
  ...
  return tiers.length === 1 ? { value: tiers[0], status: 'known', source: 'reported-primary-pinned-mapping', evidenceRefs: [resolution.id] } : unavailable('unmapped-or-conflicting-execution');
}
...
export function validateRoutingLedgerRow(row) {
  ...
  for (const call of row.calls) {
    ...
    if (call.executedTier.status !== undefined && call.executedTier.status !== (call.executedTier.value === null ? 'unknown' : 'known')) evidenceConflict('Executed tier knowledge differs from value');
