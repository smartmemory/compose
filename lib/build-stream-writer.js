...
export class BuildStreamWriter {
  ...
  /**
   * Emit a policy_violation event for a pattern match on a step's response
   * (COMP-POLICY-CHECK-5). Suppressed matches are emitted too — the trace
   * measures both arms. Never blocking.
   *
   * @param {string}  stepId    Step whose response matched
   * @param {object}  record    Scan record from lib/policy-check.js#scanResponse
   * @param {string}  userMode  'AUTONOMOUS' | 'PACED' | 'SKILL_GATED'
   * @param {string|null} [featureCode] Feature this build is for
   * @param {string|null} [buildId]     Build correlation id
   */
  writePolicyViolation(stepId, record, userMode, featureCode = null, buildId = null) {
    this.write({
      ...
      userMode,
      featureCode: featureCode ?? this.#featureCode ?? null,
      buildId,
    });
  }
   ...
   * @param {{ usdSource?: 'reported'|'estimated'|null }} [provenance]
   *   How the producer knows the amount. COMP-COST-OWNER S2: the cost and its
   *   provenance travel together or not at all.
   */
  writeUsage(stepId, usage, { usdSource = null } = {}) {
    // An amount is only emitted when something can say HOW it is known. Previously
    // this wrote `cost_usd: usage.cost_usd ?? 0` with no provenance at all, so a step
    // nobody could price arrived downstream as a hard 0 — indistinguishable from a
    // provider-reported free call, and rendered by the cockpit as `<$0.001`. Omitting
    // is what keeps unknown unknown; the consumer's schema makes `cost_usd` optional
    // precisely so this is expressible.
    const stated = (usdSource === 'reported' || usdSource === 'estimated')
    ...
    this.write({
      ...
      ...(stated ? { cost_usd: usage.cost_usd, usd_source: usdSource } : {}),
