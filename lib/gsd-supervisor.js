...
import { buildGsdQuery } from './gsd-state.js';
...
export async function runGsdHeadless(feature, opts = {}) {
  ...
  while (attempt < maxTotalAttempts) {
    ...
    const snap = buildGsdQuery(cwd, feature, { staleMs: cfg.heartbeatStaleMs });
    const outcome = classifyOutcome(snap.status, snap, cfg, counts);
