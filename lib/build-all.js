...
export async function runBuildAll(opts = {}) {
  ...
  for (const code of buildOrder) {
    ...
    try {
      ...
      // COMP-COST-OWNER 0d-2: a resolved result is not a successful build. runBuild
      // resolves an explicit terminal result; only `ok` means the feature shipped. A
      // terminal failure used to land in `built` and be reported as a success here.
      // No deleteActiveBuild on this path — writeFailedBuildTerminalState already
      // wrote terminal state; the catch branch's delete is for the crash case.
      const result = await runBuild(code, buildOpts);
      if (result?.ok === false) {
        console.error(`\n  ✗ ${code} failed: ${result.failureReason ?? result.status ?? 'build failed'}`);
        failed.add(code);
      } else {
        built.push(code);
        console.log(`\n  ✓ ${code} complete`);
