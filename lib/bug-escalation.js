...
import { routingCallOptions, routingArtifactsFor, callsForRouting } from './routing-runtime.js';
...
export async function tier1CodexReview(stratum, context, bugDescription, reproTest, currentDiff, hypotheses) {
  ...
  const onUsage = (usages) => context.onUsage?.(usages, {
    stepId: context.step_id,
    source: 'escalation',
  });
  const rawText = await stratum.runAgentText('codex', codexPrompt, routingCallOptions({
    ...(routingArtifactsFor(context) ? { routingCalls: callsForRouting(context, context.routingParent ?? null, 'bug-escalation', { id: context.step_id }) } : {}),
    cwd: context.cwd,
...
export async function tier2FreshAgent(stratum, context, codexReview, hypotheses, checkpointPath) {
  ...
  try {
    ...
    const agent_reasoning = await stratum.runAgentText('claude', prompt, routingCallOptions({
      ...(routingArtifactsFor(context) ? { routingCalls: callsForRouting(context, context.routingParent ?? null, 'bug-escalation', { id: context.step_id }) } : {}),
      cwd: wtPath,
      telemetry: escalationTelemetry(context),
      onUsage: (usages) => context.onUsage?.(usages, {
        stepId: context.step_id,
        source: 'escalation',
      }),
    }));
