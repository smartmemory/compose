...
export async function linkFeatures(cwd, args) {
  ...
  return maybeIdempotent({ ...args, cwd }, async () => {
    ...
    // COMP-MCP-VALIDATE-1: reject a dangling target before persisting. Checked
    // after the source-existence guard (a missing source is the more
    // fundamental error). `force` overrides for intentional forward-references
