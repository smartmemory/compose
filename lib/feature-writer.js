...
export async function addRoadmapEntry(cwd, args) {
  ...
  return maybeIdempotent({ ...args, cwd }, async () => {
    ...
    if (args.promoted_from !== undefined) feature.promoted_from = args.promoted_from;
...
const XREF_PROVIDERS = new Set(['github', 'forgejo', 'local', 'url', 'jira', 'linear', 'notion', 'obsidian']);
...
function validateExternalArgs(args) {
  ...
  if (args.derive_expect !== undefined) {
    if (p !== 'github' && p !== 'forgejo') {
      throw new Error('feature-writer: derive_expect is only supported for github/forgejo links');
    }
    if (typeof args.derive_expect !== 'boolean') {
      throw new Error('feature-writer: derive_expect must be boolean');
    }
  }
  if (p === 'github') {
    ...
    if (!args.repo || !Number.isInteger(args.issue) || args.issue < 1) {
      throw new Error('feature-writer: external forgejo link requires repo + integer issue ≥ 1');
    }
    if (!XREF_GH_REPO_RE.test(args.repo)) {
      throw new Error(`feature-writer: external forgejo repo "${args.repo}" must be "owner/name"`);
    }
    if (args.expect != null && !XREF_GITHUB_EXPECT.has(args.expect)) {
      throw new Error(`feature-writer: external forgejo expect must be open|closed, got "${args.expect}"`);
    }
    if (args.push != null && typeof args.push !== 'boolean') {
      throw new Error(`feature-writer: external forgejo push must be boolean, got "${args.push}"`);
    }
    if (args.expect_labels != null
        && (!Array.isArray(args.expect_labels)
            || !args.expect_labels.every((l) => typeof l === 'string' && l.length > 0))) {
      throw new Error('feature-writer: external forgejo expect_labels must be an array of non-empty strings');
    }
  } else if (p === 'local') {
    if (args.expect_labels != null) {
      throw new Error('feature-writer: expect_labels is github/forgejo-only (no labels on a local ref)');
    }
    ...
    if (args.expect_labels != null) {
      throw new Error('feature-writer: expect_labels is github/forgejo-only (no labels on a url-class ref)');
    }
    if (!args.url) {
...
async function linkFeatureExternal(cwd, args) {
  ...
  return maybeIdempotent({ ...args, cwd }, async () => {
    ...
    const targetKey = args.provider === 'github'
      ? String(args.issue)
      : args.provider === 'forgejo'
        ? `forgejo:${args.repo}#${args.issue}`
        : args.provider === 'local'
          ? args.to_code
          : args.url;
    const matchIdx = links.findIndex(
      l => l.kind === 'external'
        ...
        && (
          l.provider === 'github' ? String(l.issue) === targetKey
            : l.provider === 'forgejo' ? `forgejo:${l.repo}#${l.issue}` === targetKey
    ...
    if (args.derive_expect !== undefined) entry.derive_expect = args.derive_expect;
    if (args.note) entry.note = args.note;
