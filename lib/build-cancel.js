...
export async function runCancelTeardown({
  ...
  claimOwnership = () => ({ ok: true }), finalizeEvidence, killVision, writeTerminal, removeListeners, exit, log,
}) {
  ...
  // Share the existing 1000ms local-cleanup allowance; evidence recovery must
  // never add an unbounded wait to signal teardown or certify a deadline as proof.
  const cleanupStarted = Date.now();
  if (finalizeEvidence) {
    try { await withDeadline(finalizeEvidence(), Math.min(timeoutMs, 1000)); }
    catch (error) {
      error.code ??= 'ROUTING_RECOVERY_INCOMPLETE';
      removeListeners();
      throw error;
    }
  }
  ...
  if (claim.ok) {
    ...
    try { await withDeadline(killVision(), Math.max(1, Math.min(timeoutMs, 1000 - (Date.now() - cleanupStarted)))); } catch { /* best-effort */ }
