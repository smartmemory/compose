...
import { routingCallOptions, routingCallsTerminated, routingArtifactsFor, callsForRouting, installRoutingCalls, recoverRoutingEvidence, routingIntegrityError } from './routing-runtime.js';
...
import { createContinuationIntent, pendingRoutingPlans } from './routing-ledger.js';
import { readRoutingSnapshot } from './flow-state.js';
import { resolveConsumerProfile } from './pipeline-profiles.js';
...
import { executeShipStep, toPhaseResultOutput, runConsumerIssuance, ConsumerStuckError, filesOwnedConflict, toEngineUsage, reportUsageReceipts, loadPipelineProfiles, preflightPipelineProfiles, admitConsumerWave, publishConsumerCheckpoint, evaluateConfiguredGate, reportWaveEvidence, waveProfilesEnabled, routingOptionsFor, planWithRouting, resumeRouting, sealRoutingEpochs, admitOrdinaryRoute, prepareRoutingIssuance, launchRoutingIssuance, reportRoutingStep, routingEvent } from './build.js';
import {
...
export async function runGsd(featureCode, opts = {}) {
  ...
  let gateCommands = resolveGateCommands(cwd, opts.gateCommands);
  // COMP-PAR-MERGE-QUEUE: the fast per-task pre-merge gate (lint+build).
  let preMergeGate = resolvePreMergeGate(cwd, opts.preMergeGate);
  ...
  const originalProfiles = loadPipelineProfiles(specPath);
  // COMP-MODEL-ROUTE: GSD is an ordinary run path too, so it reads the same
  // .compose/compose.json#routing.mode the Build path does. Raw read, matching
  // resolveGateCommands below (loadProjectConfig does not merge defaults).
  let gsdProjectConfig = {};
  try { gsdProjectConfig = JSON.parse(readFileSync(join(cwd, '.compose', 'compose.json'), 'utf-8')); } catch { /* defaults */ }
  const routeOptions = opts.resume ? { mode: 'off' } : routingOptionsFor(originalProfiles, opts, gsdProjectConfig);
  const profileCheck = preflightPipelineProfiles(originalProfiles, localSpec, specPath, undefined, routeOptions);
  if (routeOptions.mode === 'shadow') assertGsdRoutingBaseline(localSpec, profileCheck);
  ...
  try {
    ...
    let priorRouting = null;
    let continuation = null;
    if (opts.resume) {
      const details = loadResumeTaskGraph(cwd, featureCode, { claim: false, details: 'identity' });
      if (canonicalRoutingJson(details.graph) !== canonicalRoutingJson(resumeTaskGraph)) routingRefuse('ROUTING_CONTINUATION_AMBIGUOUS', 'Resume graph changed while acquiring locks');
      if (details.previousRunId) priorRouting = await resumeRouting({ runId: details.previousRunId, cwd,
        artifactRoot: opts.consumerArtifactsRoot, localSpec, profiles: originalProfiles, stratum, featureCode });
      if (!priorRouting && pendingRoutingPlans({ cwd, featureCode }).some(p => !details.previousRunId || p.binding?.runId === details.previousRunId)) {
        routingRefuse('ROUTING_BINDING_MISSING', 'Continuation lost its recorded run identity');
      }
      if (priorRouting) {
        resumeDetails = loadResumeTaskGraph(cwd, featureCode, { claim: false, details: true });
        resumeTaskGraph = resumeDetails.graph;
        gateCommands = priorRouting.start.originalInput.gateCommands;
        preMergeGate = priorRouting.start.originalInput.pre_merge_gate;
        assertGsdRoutingBaseline(localSpec, profileCheck);
        sealRoutingEpochs(priorRouting);
        continuation = createContinuationIntent({ start: priorRouting.start, oldBinding: priorRouting.binding, resumeDetails,
          priorJournal: { routing: priorRouting.artifacts.exportRoutingJournal() }, snapshot: readRoutingSnapshot(details.previousRunId) });
        priorRouting.artifacts.recordRoutingRecord(continuation);
      }
    }
    ...
    const resumeTasks = opts.resume ? (priorRouting ? resumeDetails.originalGraph.tasks : resumeTaskGraph?.tasks ?? []).map((t) => ({ ...t })) : [];
    const initialState = {
      feature: featureCode,
      flowId: priorRouting?.binding.runId ?? null,
      pid: process.pid,
      mode: 'gsd',
      phase: 'planning',
      status: 'running',
    ...
    stepCtx = {
      ...
      consumerArtifactsRoot: opts.consumerArtifactsRoot,
      filesChanged: [],
      stuckDetector,
    ...
    const planned = await planWithRouting({ stratum, specYaml, flowName: 'gsd', input: {
      featureCode, gateCommands, pre_merge_gate: preMergeGate,
    }, cwd, featureCode, profiles: originalProfiles, options: routeOptions, artifactRoot: opts.consumerArtifactsRoot,
      continuation, priorRouting });
    let response = planned.response;
    stepCtx.routing = planned.routing;
    if (planned.routing) {
      stepCtx.consumerArtifacts = planned.routing.artifacts;
      stepCtx.artifacts = planned.routing.artifacts;
    }
    const flowId = response.runId;
    stepCtx.flowId = flowId;
    installRoutingCalls(stepCtx);
    await recoverRoutingEvidence(stepCtx);
    if (waveProfilesEnabled(pipelineProfiles) && !stepCtx.consumerArtifacts) {
      stepCtx.consumerArtifacts = new ConsumerFanoutArtifacts({ runId: flowId, targetCwd: cwd, artifactRoot: opts.consumerArtifactsRoot,
    ...
    flushState(stepCtx, { flowId, phase: 'decompose', ...(priorRouting ? { decomposedTasks: resumeTaskGraph.tasks } : {}) });
    ...
    while (
      ...
      !(stepCtx.routing && response.status === 'cancelled')
    ) {
      response = await runOneStep(response, stepCtx);
    }

    await recoverRoutingEvidence(stepCtx);
    if (stepCtx.routing && response.status === 'cancelled') {
      flushState(stepCtx, { status: 'failed', reason: 'cancelled' });
    ...
    let routingError;
    try { await recoverRoutingEvidence(stepCtx); } catch (error) { routingError = error; }
    ...
    if (routingError && !routingIntegrityError(routingPrimaryError)) throw routingError;
  }
}
...
export async function runOneStep(response, ctx) {
  routingArtifactsFor(ctx);
  ...
  if (response.status === 'ready') {
    ...
    if (descriptor) {
      if (!ctx.consumerArtifacts) {
        ctx.consumerArtifacts = new ConsumerFanoutArtifacts({
          runId: flowId,
          targetCwd: cwd,
          artifactRoot: ctx.consumerArtifactsRoot,
      ...
      const configured = ctx.pipelineProfiles?.[descriptor.step]?.tier_from || ctx.pipelineProfiles?._consumer?.[descriptor.step] || ctx.routing;
      const admission = configured ? await admitConsumerWave({ descriptor, descriptors: ready.filter(isConsumerDescriptor), audit,
        localSpec, profiles: ctx.pipelineProfiles, artifacts: ctx.consumerArtifacts, stratum, flowId, routing: ctx.routing }) : null;
      ...
      try {
        return await runConsumerIssuance({
          ...
          context: { routing: ctx.routing, pipelineProfiles: ctx.pipelineProfiles, artifacts: ctx.consumerArtifacts, cwd, projectCwd: cwd, featureCode, flowId, receiptsMode: ctx.receiptsMode, gsd: true, gsdTaskId: consumerItem?.id, filesChanged: ctx.filesChanged, onUsage: (usage, meta) => recordTsAgentUsage(ctx, usage, meta), taskResultPath },
    ...
    const admission = admitOrdinaryRoute({ descriptor: step, localSpec, context: ctx });
    const issuance = prepareRoutingIssuance({ descriptor: step, admission, context: ctx });
    await launchRoutingIssuance(ctx, issuance);
    const routingCalls = callsForRouting(ctx, issuance, issuance ? null : 'engine-judged', step);
    let runOut;
    try {
      runOut = await stratum.agentRun(step.agent ?? 'claude', step.do ?? '', routingCallOptions({
      ...
      if (issuance) routingEvent(ctx, issuance, 'uncertain', { reason: error.message });
      if (ctx.routing && routingIntegrityError(error)) throw error;
      if (error?.usage) {
      ...
      if (issuance && routingCallsTerminated(ctx, issuance)) await reportRoutingStep(ctx, step, issuance, { failure: error.message });
      throw error;
    }
    ...
    try {
      ...
      if (issuance) await reportRoutingStep(ctx, step, issuance, { failure: `Unparseable JSON: ${err.message}` });
    ...
    if (step.id === 'decompose_gsd') {
      try {
        ...
        return reportRoutingStep(ctx, step, issuance,
  ...
  if (response.status === 'running') {
    ...
    const resolveGateWithConsumerMerge = async (requestedOutcome, requestedRationale) => {
      const captured = await captureRoutingGate(ctx, { gateStepId, gateToken: gateState.gateToken, localSpec });
      ...
      const disposition = prepareRoutingGate(ctx, captured, { audit, mergeGate: Boolean(fanoutStep), mergeEvidence: transaction ? structuredClone(transaction) : null,
        requestedDecision: { decision: requestedOutcome, rationale: requestedRationale ?? '', resolver: 'system' },
        finalProposedDecision: { decision: outcome, rationale: rationale ?? '', resolver: 'system' } });
      let next;
      try {
        next = await stratum.gateResolve(flowId, gateStepId, outcome, rationale, 'system', gateState.gateToken);
        acknowledgeRoutingGate(ctx, disposition, next);
      } catch (error) {
        if (ctx.routing && routingIntegrityError(error)) throw error;
        const current = await stratum.audit(flowId);
        acknowledgeRoutingGate(ctx, disposition, null, current);
      ...
      await recoverRoutingEvidence(ctx);
      return next;
    };
 ...
 */
export function loadResumeTaskGraph(cwd, featureCode, { claim = true, details = false } = {}) {
  const pausePath = join(gsdDir(cwd, featureCode), 'pause.json');
  const hasPause = existsSync(pausePath);
  let pause;
  if (hasPause) {
    try {
  ...
  const state = readGsdState(cwd, featureCode);
  // state.json records the latest run that persisted a checkpoint. A pause
  // from a different run cannot supersede that run's routing lineage, even
  // when the continuation died before it could replace/remove the old pause.
  // Use recorded participation, not current route flags; unindexed legacy
  // runs retain their pause-first resume behavior.
  const newerRoutingRun = hasPause && state?.flowId && state.flowId !== pause.flowId
    && pendingRoutingPlans({ cwd, featureCode }).some(p => p.binding?.runId === state.flowId);
  if (!hasPause || newerRoutingRun) {
  ...
  if (details === 'identity') return { graph: { tasks: remaining }, previousRunId: pause.flowId ?? null };
  if (details) {
    const verifiedCompletedTaskIds = verifiedGsdCompletions(cwd, featureCode);
    const completedTaskIds = [...new Set([...completed, ...verifiedCompletedTaskIds])];
    const removedTaskIds = tasks.filter(t => completedTaskIds.includes(t.id)).map(t => t.id);
    const removedDependencies = [];
    const filtered = tasks.filter(t => !removedTaskIds.includes(t.id)).map(t => ({ ...t, depends_on: (t.depends_on ?? []).filter(dependency => {
      if (!removedTaskIds.includes(dependency)) return true;
      removedDependencies.push({ taskId: t.id, dependency }); return false;
    }) }));
    return { graph: { tasks: filtered }, originalGraph: { tasks }, completedTaskIds, verifiedCompletedTaskIds, removedTaskIds, removedDependencies,
      indexMap: tasks.map((t, oldIndex) => ({ taskId: t.id, oldIndex, newIndex: removedTaskIds.includes(t.id) ? null : filtered.findIndex(r => r.id === t.id) })),
      previousRunId: pause.flowId ?? null };
  }
...
/** Q1: participating ordinary calls must equal the existing bare GSD dispatch. */
export function assertGsdRoutingBaseline(spec, preflight) {
  for (const flow of Object.values(spec.flows)) for (const step of flow?.steps ?? []) {
    if (!step.agent || step.fanout || step.id === 'ship_gsd') continue;
    const baseline = resolveConsumerProfile(preflight.normalized[step.id] ?? step.agent, {});
    const bare = resolveConsumerProfile(step.agent, {});
    const options = value => Object.fromEntries(Object.entries(value).filter(([key]) => key !== 'profile'));
    if (canonicalRoutingJson(options(baseline)) !== canonicalRoutingJson(options(bare))) {
      routingRefuse('ROUTING_STATIC_DISPATCH_MISMATCH', `GSD ordinary profile differs from bare call: ${step.id}`);
    }
  }
}
function verifiedGsdCompletions(cwd, featureCode) {
  const results = new Map();
  const add = (id, result) => {
    if (!id || !validateTaskResult(result).ok) routingRefuse('ROUTING_CONTINUATION_AMBIGUOUS', 'Invalid completed TaskResult');
    if (results.has(id) && canonicalRoutingJson(results.get(id)) !== canonicalRoutingJson(result)) routingRefuse('ROUTING_CONTINUATION_AMBIGUOUS', 'Conflicting completion results');
    results.set(id, result);
  };
  for (const [id, result] of Object.entries(readBlackboard(featureCode, { cwd }))) add(id, result);
  const dir = join(gsdDir(cwd, featureCode), 'results');
  if (existsSync(dir)) for (const file of readdirSync(dir).filter(f => f.endsWith('.json'))) {
    let result;
    try { result = JSON.parse(readFileSync(join(dir, file), 'utf8')); }
    catch { routingRefuse('ROUTING_CONTINUATION_AMBIGUOUS', 'Unreadable completion result'); }
    add(file.slice(0, -5), result);
  }
  return [...results.keys()];
}
