...
import { writeGsdState, readGsdState, gsdStatePath, pidAlive } from './gsd-state.js';
...
export async function runGsd(featureCode, opts = {}) {
  ...
  try {
    ...
    // COMP-GSD-6: terminal state.json flush (complete/killed). On a clean
    // complete the state is observable by `compose gsd query` until the next run.
    flushState(stepCtx, { status: response.status === 'complete' ? 'complete' : response.status, phase: 'done' });
...
async function runOneStep(response, ctx) {
  ...
  if (response.status === 'execute_step') {
    ...
    if (stepType === 'parallel_dispatch' || response.tasks) {
      ...
      if (ctx.runState) {
        flushState(ctx, { phase: 'execute', completedTaskIds: collectCompletedTaskIds(cwd, featureCode) });
      }
...
export function claimRunLock(cwd, featureCode) {
  ...
  // TOCTOU-safe takeover: remove + re-attempt the atomic mkdir. A concurrent
  // winner still wins (we'd get EEXIST again and re-evaluate).
  rmSync(lock, { recursive: true, force: true });
  mkdirSync(lock); // throws EEXIST if someone raced us — correct (they own it now)
  write();
}
...
export function claimResumeLock(cwd, featureCode) {
  ...
  mkdirSync(claimPath); // throws EEXIST if a concurrent run raced us — correct
  writeOwner();
}
