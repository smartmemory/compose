...
async function defaultHeadCheck(url, timeoutMs = 5000) {
  return new Promise((resolve) => {
    ...
    // https tunnels are the normal case — pick the module by protocol or the
    // probe can never complete a TLS handshake (status would false-fail).
...
async function verbStatus(subArgs, { port, token, cwd, out, distDir, headFn }) {
  ...
  if (!token) {
    throw new Error(
      'remote status incomplete: paired-device query skipped because COMPOSE_API_TOKEN is not set.'
    );
  }
}
