...
import { normalizeFullSha, validateCompletionInputShape } from './completion-writer.js';
import {
 ...
 * still runs the pure writer-shape preflight but skips guard-scoped git/test
 * evidence, while the second refuses.
 *
 * @param {object}   a
 * @param {string}   a.featureCode
 * @param {string}   [a.commitSha]        omitted ⇒ commit-less (non-git) completion
 * @param {boolean}  [a.testsPass]        optional claim; a testCommand may independently attest
...
export async function completionGate({
  ...
  // Canonicalise the caller's SHA once, before either the live or backfill path
  // can verify evidence, compute a recovery identity, write an intent, or touch
  // the guard. The writer owns this rule (Decision 9); the gate only assigns the
  // returned canonical value so every downstream artifact sees lowercase.
  if (commitSha !== undefined && commitSha !== null) {
    try {
      commitSha = normalizeFullSha(commitSha);
    } catch (e) {
      return { ok: false, guarded, refusedAt: 'evidence', reasons: [e.message] };
    }
  }

  // Hoist every caller-controlled pure shape rule enforced by recordCompletion
  // EXCEPT tests_pass. The raw claim may be undefined when a configured command
  // supplies the attestation; the writer later validates the derived boolean.
  // `set_status` is included as the literal false this gate always supplies;
  // semantic evidence, feature-state and guard-policy checks remain below.
  try {
    validateCompletionInputShape({
    ...
    }, { validateTestsPass: false });
  } catch (e) {
    return { ok: false, guarded, refusedAt: 'request', reasons: [e.message] };
  }
...
async function backfillGate({
  ...
  try {
    ...
    if (recovering) {
      ...
      // A pre-fix intent may contain values the writer would reject. Recovery
      // must reject those persisted bytes before replaying the guard or writing
      // lifecycle history, otherwise the later writer refusal strands the same
      // intent again. Unlike a live raw claim, `tests_attested` is already the
      // gate-derived value that the writer will receive, so it is strict here.
      try {
        writeContext.commitSha = normalizeFullSha(writeContext.commitSha);
        validateCompletionInputShape({
          feature_code: writeContext.featureCode,
          tests_pass: writeContext.attested,
          files_changed: writeContext.filesChanged,
          notes: writeContext.notes,
          set_status: false,
          idempotency_key: writeContext.operationId,
        });
      } catch (e) {
        return refusal('recovery',
          `the persisted intent at ${intentPath(workspaceRoot, featureCode)} is malformed: ${e.message}`,
          { guarded });
      }
