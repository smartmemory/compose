...
export function injectCertInstructions(intent, template) {
  ...
  for (const [i, section] of sections.entries()) {
    ...
    if (i === 0 && requireCitations) lines.push('Format each fact as: [P1] <fact, citing file:line>, [P2] ..., etc.');
    if (i > 0 && requireCitations) lines.push('Reference premises by their [P<n>] ID.');
    lines.push('');
  }
