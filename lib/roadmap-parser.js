...
// Canonical status enum. Longest-first is irrelevant here (none is a prefix of
// another), but kept explicit so the leading-token match is unambiguous.
 ...
 * real status cell uses before commentary: `PARKED — needs X` → `PARKED` (note
 * the space before the em-dash), `PARTIAL (1a COMPLETE)` → `PARTIAL`,
 * `COMPLETE` → `COMPLETE`. Deliberately conservative: glued forms like
 * `PLANNED-ish` or `PARKED/blocked` return null (left for the validator to flag)
 * rather than being silently coerced to a valid enum. Case-insensitive; returns
 * the canonical UPPERCASE token, or null if none.
 *
 * @param {string} text
 * @returns {string|null}
 */
...
export function parseRoadmap(text) {
  ...
  for (const line of lines) {
    ...
    if (phaseMatch) {
      ...
      currentParentPhaseId = phaseMatch[1].trim();
