...
/**
 * Extract the leading status token from cell/override text, tolerant of any
 * trailing commentary: `PARKED — needs X` → `PARKED`, `PARTIAL (1a COMPLETE)` →
 * `PARTIAL`, `COMPLETE` → `COMPLETE`. The token must be followed by end-of-string
 * or a non-word char (space, `(`, `—`, etc.) so `PLANNEDX` does not match.
...
export function parseStatusToken(text) {
  ...
  for (const t of STATUS_TOKENS) {
    if (up === t) return t;
    if (up.startsWith(t)) {
      const next = up[t.length];
      if (!/[A-Z0-9_]/.test(next)) return t; // boundary after the token
    }
  }
  return null;
}
