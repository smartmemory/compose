...
import { resolveFeaturesPath, resolveRoadmapPath, loadExternalPrefixes } from '../project-paths.js';
 ...
 */
export function collectGraphInputs(cwd, featuresDir = resolveFeaturesPath(cwd)) {
  const warnings = [];
  ...
  for (const f of features) {
    ...
    const folder = join(featuresDir, f.code);
  ...
  for (const f of features) {
    ...
    const depsPath = join(featuresDir, f.code, 'deps.yaml');
