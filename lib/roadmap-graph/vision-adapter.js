 ...
 * @param {{externalPrefixes?: string[], includeProseEdges?: boolean}} [opts]
 *   includeProseEdges (default true): the LIVE projection renders every
 *   relationship, including prose-derived `informs` edges (shipped cockpit
 *   behavior). The CANONICAL projection passes false so only structural
 *   (deps.yaml-derived blocks/supports/...) edges remain, matching the static
 *   collector which only ever drew deps.yaml edges.
 * @returns {{nodes: object[], rawEdges: object[], knownCodes: Set<string>, warnings: string[]}}
 */
export function visionToGraphInputs(items, connections, { externalPrefixes = [], includeProseEdges = true } = {}) {
  ...
  for (const c of connections || []) {
    ...
    // `informs` is the prose-derived relationship type. The canonical
    // projection excludes it so it matches the deps.yaml-only static collector.
    if (!includeProseEdges && c.type === 'informs') continue;
  ...
  // Deterministic node order (independent of readdirSync / store insertion order)
  // so the rendered HTML is byte-stable for --check. buildGraph preserves this
  // order; edges are sorted inside buildGraph.
