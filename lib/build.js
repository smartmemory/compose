...
/**
 * COMP-PLAN-GATE-LOOP: read Stratum's current round for a flow from its
 * persisted state file.
 *
 * The await_gate dispatch Stratum returns does not carry the round, but the
 * persisted flow (`~/.stratum/flows/<flowId>.json`) does — Compose already
 * reads this file elsewhere. Threading the round into the gate id makes each
 * gate re-entry after a `revise` a fresh, pending gate rather than colliding
 * with the prior resolved gate and replaying its stale outcome.
 *
