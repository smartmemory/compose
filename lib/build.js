 ...
 *
 * NOTE: Synthesis output shape is {consensus, claude_only, codex_only} — intentionally
 * outside the canonical ReviewResult contract. Full canonicalization tracked as STRAT-XMODEL-PARITY.
 *
...
async function runCrossModelReview(mergedResult, filesChanged, cwd, stratum, streamWriter, opts = {}) {
  ...
  const synthesisPrompt =
    ...
    `{\n` +
    `  "consensus": [<ReviewResult finding items from Claude, with codexNote field added>],\n` +
    `  "claude_only": [<ReviewResult finding items>],\n` +
    `  "codex_only": [{"lens":"general","file":"","line":null,"severity":"should-fix","finding":"<codex text>","confidence":6,"applied_gate":7,"source":"codex"}]\n` +
    `}\n\n` +
    `For CODEX_ONLY findings, create ReviewResult finding-shaped objects with file=null if the file is not clear.\n` +
  ...
  try {
    ...
    const { text: synthText } = await runAndNormalize(null, synthesisPrompt, syntheticStep, {
      streamWriter,
