...
import { shouldRunCrossModel } from './review-lenses.js';
...
function defaultConnectorFactory(agentString, opts) {
  const { provider, allowedTools, disallowedTools } = resolveAgentConfig(agentString);
  ...
  try {
    ...
    while (response.status !== 'complete' && response.status !== 'killed') {
      ...
      if (response.status === 'execute_step') {
        ...
        {
          const { template: stepTemplate, allowedTools: stepAllowed, disallowedTools: stepDisallowed } = resolveAgentConfig(agentType);
          if (stepTemplate) {
        ...
        if (childFlowName === 'parallel_review' && childResult?.output && (context.filesChanged?.length ?? 0) > 0) {
          ...
          if (synthesized !== mergedResult) {
            // Inject the synthesized result back into the child flow envelope
...
async function runCrossModelReview(mergedResult, filesChanged, cwd, getConnector, streamWriter, opts = {}) {
  ...
  let synthesis = { consensus: [], claude_only: claudeFindings, codex_only: [] };
  try {
      ...
      try {
        ...
        if (parsed && typeof parsed === 'object') {
          synthesis = {
            ...
            codex_only:   Array.isArray(parsed.codex_only)   ? parsed.codex_only   : [],
          };
        }
      } catch { /* keep fallback */ }
    }
