  ...
  try {
    ...
    if (streamWriter) {
      try {
        ...
        // This replaces the phase-marker checkStaleness, which read a
        // `<!-- phase: -->` marker that no production writer ever emitted (dead
        // signal). findStaleArtifacts works off the canonical chain + mtimes, so
        // it needs no marker to be written first.
        try {
