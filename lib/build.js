...
import { readFileSync, writeFileSync, existsSync, mkdirSync, unlinkSync, renameSync, symlinkSync, openSync, closeSync, fsyncSync, lstatSync, readlinkSync } from 'node:fs';
...
import { printLearnSummary } from './learn-summary.js';
...
import { probeServer, probeWorkspace } from './server-probe.js';
import { deriveId } from './discover-workspaces.js';
import { resolveGateTimeoutMs } from './gate-timeout.js';
...
import { floorProfileToStoredRequirements, isTriageStale } from './triage.js';
...
export function runPreMergeGateLocal(cwd, commands, baseCwd, timeoutMs) {
  ...
  if (baseCwd) {
    try {
      ...
      if (existsSync(baseModules) && !existsSync(worktreeModules)) {
        ...
        dependencyBridge = { path: worktreeModules, target: baseModules };
      }
  ...
  try {
    ...
    try {
      if (dependencyBridge
        && lstatSync(dependencyBridge.path).isSymbolicLink()
        && readlinkSync(dependencyBridge.path) === dependencyBridge.target) {
        unlinkSync(dependencyBridge.path);
      }
    } catch { /* bridge already removed or replaced by the gate */ }
  }
}
...
//
// COMP-MODEL-ROUTE: mode precedence is
//   --route-mode flag  >  .compose/compose.json#routing.mode  >  preset `_routing.mode`  >  'off'.
//
// The project-config term is what lets an ORDINARY build accumulate a shadow corpus. Before it,
// the only shipped config setting `_routing` was presets/team-fable-astra.profiles.json and there
// was no CLI flag, so `.compose/routing/ledger.jsonl` was never written by anything but tests --
// and S2's hindsight report and Q3's repair-floor ruling both need that corpus to exist. Shadow on
// the ordinary bug-fix/build presets is an already-tested path (test/helpers/routing-runtime-fixture.js
// drives runBuild with template 'bug-fix' at mode 'shadow' and asserts ledger rows).
//
// Refusal semantics are DELIBERATELY unchanged: a ROUTING_* integrity error still fails the run,
// including an ordinary-only one (design.md acceptance "Missing/drifted roots or issuance bindings
// refuse even ordinary-only runs"). Recording a censored sample would corrupt the very corpus this
// exists to fill. The escape hatch is the setting, surfaced in the failure message, not a downgrade.
export function routingOptionsFor(profiles = {}, opts = {}, config = {}) {
  const configured = config?.routing?.mode;
  if (configured !== undefined && !['off', 'shadow'].includes(configured)) {
    throw new Error(`.compose/compose.json routing.mode must be "off" or "shadow" (found ${JSON.stringify(configured)})`);
  }
  const mode = opts.route_mode ?? configured ?? profiles._routing?.mode ?? 'off';
...
export async function admitConsumerWave({ descriptor, descriptors = [descriptor], audit, localSpec,
  ...
  if (!findings.length) {
    const admissionStep = routing ? routingScope(routing.start.spec.effective, descriptor).step : step;
    for (const [stageIndex] of step.fanout.steps.entries()) {
      const entry = profiles[step.id] ?? admissionStep.fanout.steps[stageIndex]?.agent ?? 'claude';
      const provider = resolveConsumerProfile(entry, {}).provider;
      const result = validateWaveAdmission(entry, items,
        { provider, ownership: policy.ownership, independent: policy.independent });
...
const DEFAULT_REVIEW_PROMPT_MAX_CHARS = 120_000;
const DEFAULT_REVIEW_RATE_LIMIT_BASE_MS = 2_000;
const DEFAULT_REVIEW_RATE_LIMIT_MAX_MS = 30_000;
...
function previousProviderFailure(previousFailure) {
  if (!previousFailure) return null;
  const reason = typeof previousFailure === 'string'
    ? previousFailure
    : (typeof previousFailure.reason === 'string' ? previousFailure.reason : JSON.stringify(previousFailure));
  const match = /\[provider-failure:(prompt-too-long|rate-limited)\]/.exec(reason);
  return match ? { class: match[1], reason } : null;
}

function reviewPromptBudgetChars(context) {
  // Production configuration is environment-owned; the context value is the
  // narrow injection seam used by unit tests and embedded callers.
  return positiveInteger(
    context?.reviewPromptBudgetChars ?? process.env.COMPOSE_REVIEW_PROMPT_MAX_CHARS,
    DEFAULT_REVIEW_PROMPT_MAX_CHARS,
  );
}

function reviewRateLimitDelay(context, attempt) {
  const policy = context?.reviewRetryPolicy ?? {};
  const maxMs = positiveInteger(
    policy.maxMs ?? process.env.COMPOSE_REVIEW_RATE_LIMIT_MAX_MS,
    DEFAULT_REVIEW_RATE_LIMIT_MAX_MS,
  );
  const baseMs = Math.min(maxMs, positiveInteger(
    policy.baseMs ?? process.env.COMPOSE_REVIEW_RATE_LIMIT_BASE_MS,
    DEFAULT_REVIEW_RATE_LIMIT_BASE_MS,
  ));
  return Math.min(maxMs, baseMs * (2 ** Math.max(0, (attempt ?? 2) - 2)));
}

function sleepForReviewRetry(context, delayMs) {
  const injectedSleep = context?.reviewRetryPolicy?.sleep;
  return typeof injectedSleep === 'function'
    ? injectedSleep(delayMs)
    : sleep(delayMs);
}
...
export async function runConsumerIssuance({
  ...
  if (reviewOpts.reviewMode) {
    ...
    const promptWithoutAmbientContext = buildStepPrompt(dispatch, {
      ...worktreeContext,
      contextDir: null,
    });
    requiredPrompt = reviewScaffold + '\n\n' + promptWithoutAmbientContext;
  }
  ...
  let mainResult;
  const priorProviderFailure = previousProviderFailure(descriptor.previousFailure);
  const routingCalls = callsForRouting(evidenceContext, routingIssuance, routingIssuance ? null : 'multi-stage-consumer', descriptor);
  try {
    if (priorProviderFailure?.class === 'prompt-too-long') {
      const error = new AgentError(
        `Prompt-too-long retry suppressed before provider dispatch; `
        + `the prior deterministic provider rejection was: ${priorProviderFailure.reason}`,
      );
      error.code = 'PROVIDER_RETRY_SUPPRESSED';
      progress.warn(error.message);
      throw error;
    }
    if (priorProviderFailure?.class === 'rate-limited') {
      const delayMs = reviewRateLimitDelay(context, descriptor.attempt);
      const message = `Rate-limited review retry: waiting ${delayMs}ms before provider dispatch.`;
      progress.warn(message);
      console.warn(`[consumer] ${message}`);
      await sleepForReviewRetry(context, delayMs);
    }
    ...
    mainResult = await runAndNormalize(null, prompt, dispatch, routingCallOptions({
      ...
      ...(reviewOpts.reviewMode ? {
        promptBudget: {
          maxChars: reviewPromptBudgetChars(context),
          requiredPrompt,
        },
      } : {}),
    }));
    ...
    // A stuck halt or local pre-dispatch refusal performs no provider dispatch,
    // so neither requires confirmed provider termination. Integrity refusals
    // above still win, even if they also carry an observer-abort type/reason.
    const stuckAbort = error instanceof AgentAbortedError && error.reason?.stuck === true;
    const localPreDispatchRefusal = ['PROVIDER_RETRY_SUPPRESSED', 'REVIEW_PROMPT_BUDGET_EXCEEDED'].includes(error?.code);
    if (routingCalls && !routingCalls.terminated() && !stuckAbort && !localPreDispatchRefusal) throw error;
    ...
    if (error instanceof AgentTimeoutError) {
      ...
      const providerClass = ['prompt-too-long', 'rate-limited'].includes(error?.providerFailureClass)
        ? error.providerFailureClass
        : null;
      const originalReason = error instanceof Error ? error.message : String(error);
      const reason = providerClass
        ? `[provider-failure:${providerClass}] ${originalReason}`
        : originalReason;
      mainResult = {
...
const BUILD_ACCUMULATOR_VERSION = 5;
const BUILD_ACCUMULATOR_FIELDS = new Set([
  ...
  'tokens_total',
  'usd',
  // Provenance for the aggregate dollar total. STATED by each dispatch and folded
  // with aggregateUsdSource semantics; never inferred from the amount.
  'usd_source',
  ...
  'input_tokens',
  'output_tokens',
  // COMP-COST-OWNER Open Question 0. Cache tokens are 95%+ of billed input once
  // prompt caching is on, and the row carried no field for them at all -- so
  // `input_tokens: 0` on a real build read as "no input" when it meant "all of it
  // was cached". Canonical names: the producer's short `cache_read`/`cache_creation`
  // (result-normalizer:761-765) and the raw `cache_*_input_tokens` dialect are BOTH
  // read at the entry, and exactly these two names are written here and on the row,
  // matching the row's existing `input_tokens`/`output_tokens` convention.
  //
  // NOT part of `tokens_total`. That field means input+output, cache is billed at a
  // different rate, and the S1 ledger/accumulator reconciliation depends on it keeping
  // that meaning. Do not fold these in.
  'cache_read_tokens',
...
function validateBuildAccumulator(value, expectedFeatureCode = null) {
  ...
  if (![null, 'reported', 'estimated'].includes(value.usd_source)) {
    throw new Error('Build accumulator is corrupt: usd_source must be null, reported, or estimated');
  }
...
function migrateBuildAccumulator(value) {
  ...
  if (next.v === 3) {
    // COMP-COST-OWNER Open Question 0. A v3 record was written before cache tokens had
    // anywhere to go, so it cannot know its cache split -- and a resumed build's earlier
    // segments are unrecoverable. NULL rather than 0, for the same reason v2->v3 refused
    // to seed the input/output split: a 0 here reads downstream as a measured zero, which
    // is exactly the "no input" misreading this field exists to end.
    next = { ...next, v: 4, cache_read_tokens: null, cache_creation_tokens: null };
  }
  if (next.v === 4) {
    // COMP-COST-OWNER S1 provenance. A v4 record knows its dollar total but did not
    // record how that total was obtained. NULL is the only honest migration value:
    // never retrofit `reported` from the mere presence of `usd`.
    next = { ...next, v: 5, usd_source: null };
  }
  return next;
}
 ...
 * @param {string|null} params.flowStatus
 * @param {string|null} params.failedStepId
 * @param {boolean} params.sameMode
 * @returns {{ action: 'resume'|'fresh'|'refuse'|'error', flowId?: string, reason: string }}
 */
export function decideBuildStart({
  active,
  opts = {},
  pidAlive = false,
  flowTerminal = false,
  ...
  if (flowTerminal) {
    if (wantsResume) {
      if (flowStatus === 'failed' || (!flowStatus && active.status === 'failed')) {
        const step = failedStepId ?? active.currentStepId;
        return {
          action: 'error',
          reason: step
            ? `The run failed at step "${step}" and cannot be resumed.`
            : 'The run failed and cannot be resumed; the failed step was not recorded.',
        };
      }
      return { action: 'error', reason: 'Nothing to resume (no in-progress or failed build found)' };
    }
...
function failedStepFromAudit(audit, active) {
  const recorded = audit?.failure?.stepId ?? audit?.failure?.step_id;
  if (typeof recorded === 'string' && recorded.length > 0) return recorded;
  const steps = audit?.steps;
  if (Array.isArray(steps)) {
    const failed = steps.find(step => step?.status === 'failed');
    if (typeof failed?.id === 'string' && failed.id.length > 0) return failed.id;
  } else if (steps && typeof steps === 'object') {
    const failed = Object.entries(steps).find(([, step]) => step?.status === 'failed');
    if (failed) return failed[0];
  }
  const persisted = active?.steps?.find?.(step => step?.status === 'failed');
  return persisted?.id ?? active?.currentStepId ?? null;
}
...
export async function runBuild(featureCode, opts = {}) {
  ...
  const workspaceId = deriveId({ root: cwd }).id;
  ...
  if (cfg.runsTriage && !opts.skipTriage && !opts.template) {
    ...
    if (cachedFeature?.profile && (_escalated || !isTriageStale(cwd, featureCode, featuresDir))) {
      // Reuse the cached derivation, but apply any separately recorded human
      // override at read time too. feature.json is intentionally excluded from
      // the mtime staleness scan, so an override added to an otherwise-fresh
      // feature would never reach applyFrontTriage without this reconciliation.
      const cachedPolicy = floorProfileToStoredRequirements(
        cachedFeature.profile,
        cachedFeature.profile,
        cachedFeature.profileOverrides,
        cachedFeature.triageRationale ?? '',
      );
      buildProfile = cachedPolicy.profile;
      if (JSON.stringify(buildProfile) !== JSON.stringify(cachedFeature.profile)
          || cachedPolicy.rationale !== (cachedFeature.triageRationale ?? '')) {
        cachedFeature = await _buildProvider.putFeature(featureCode, {
          ...cachedFeature,
          profile: buildProfile,
          triageRationale: cachedPolicy.rationale,
        });
      }
  ...
  const visionWriter = opts.visionWriter ?? new VisionWriter(dataDir, { workspaceId });
  ...
  // STRAT-LEARN-INLINE-TS-1 §A5: the summary prints at every non-aborted exit —
  // once at the terminal-status line and, for a build that threw before reaching
  // it, in the catch below. The flag keeps a post-summary throw from printing a
  // second copy.
  let learnSummaryPrinted = false;
  ...
  // History aggregation: group rows by (flowId, accumulator_build_id), take the
  // LAST row within a group, SUM across groups. Auto-resume of a terminal flow
  // correctly rotates the accumulator: a new lifetime is exactly a new group.
  let lastOwnerCost = {
    accumulator_build_id: selectedAccumulator?.build_id ?? null,
    ...
    cache_read_tokens: selectedAccumulator.cache_read_tokens,
    cache_creation_tokens: selectedAccumulator.cache_creation_tokens,
    usd_unknown_count: selectedAccumulator.usd_unknown_count,
    usd_source: selectedAccumulator.usd_source,
  };
  const buildCostSnapshot = () => {
    const owner = readBuildAccumulator(cwd, featureCode);
    if (owner) {
      lastOwnerCost = {
        accumulator_build_id: owner.build_id ?? null,
        cost_usd: owner.usd,
        ...
        // COMP-COST-OWNER Open Question 0: cache tokens ride the mirror onto the history
        // row. appendBuildHistory spreads this snapshot and whitelists nothing, so the row
        // gains both fields here rather than at a fourth site.
        cache_read_tokens: owner.cache_read_tokens,
        cache_creation_tokens: owner.cache_creation_tokens,
        usd_unknown_count: owner.usd_unknown_count,
        usd_source: owner.usd_source,
      };
    }
  ...
  const rotateStaleAccumulatorForFreshStart = () => {
    ...
    // Zero, not null: a rotation installs a FRESH v5 record, whose cache totals are a
    // measured nothing rather than an unrecoverable v3 unknown.
    lastOwnerCost = { accumulator_build_id: build_id ?? null, cost_usd: 0, input_tokens: 0, output_tokens: 0,
      cache_read_tokens: 0, cache_creation_tokens: 0, usd_unknown_count: 0, usd_source: null };
  ...
  try {
    ...
    let routeOptions = recordedRunId ? { mode: routing ? 'shadow' : 'off' } : routingOptionsFor(stepProfiles, opts, composeConfig);
    ...
    const configureFreshStart = () => {
      routing = null;
      routeOptions = routingOptionsFor(stepProfiles, opts, composeConfig);
    ...
    let flowStatus = activeForDecision?.status === 'failed' ? 'failed' : null;
    let failedStepId = flowStatus === 'failed'
      ? failedStepFromAudit(null, activeForDecision)
      : null;
    ...
    if (probeFlowId && !opts.fresh && (opts.resumeFlowId || !flowTerminal)) {
      try {
        ...
        flowStatus = audit?.status ?? flowStatus;
        if (audit?.status === 'failed') failedStepId = failedStepFromAudit(audit, activeForDecision);
    ...
    if (verdict.action === 'resume') {
      ...
      if (!isFreshStart) {
        ...
        writeActiveBuild(dataDir, {
          ...
          // Preserve the durable summaries that seed stepHistory below. A resume
          // refresh must not erase context produced by already-completed steps.
          steps: Array.isArray(activeForDecision?.steps) ? activeForDecision.steps : [],
    ...
    // A new invocation normally rebuilds this local array; on resume, restore the
    // summaries already persisted by syncStepHistory. Fresh runs stay isolated.
    stepHistory = isFreshStart ? [] : rehydrateStepHistory(activeForDecision?.steps);
    ...
    context.recordBuildUsage = async (usage, meta) => {
      ...
      // The per-DISPATCH entries. Both the unpriced-step count below and the cache
      // totals read from HERE rather than from the aggregate `usage` object, for the
      // reason spelled out in each.
      const usageEntries = Array.isArray(usage)
        ? usage
        : (Array.isArray(usage?.usages) ? usage.usages : (usage ? [usage] : []));
      // A settleable observation may carry only duration. That still matters for
      // provenance: a labelled $0 is a real report, while a duration-only unlabelled
      // dispatch is an unknown cost. Keeping this predicate entry-based also preserves
      // the distinction the aggregate object loses between an omitted cost and $0.
      const isSettleableObservation = (entry) => {
        if (!entry || typeof entry !== 'object') return false;
        const cost = entry.cost_usd ?? entry.usd;
        if (typeof cost === 'number' && Number.isFinite(cost)) return true;
        return [
          entry.input_tokens, entry.output_tokens, entry.tokens,
          entry.cache_read, entry.cache_read_input_tokens,
          entry.cache_creation, entry.cache_creation_input_tokens,
          entry.duration_ms, entry.ms,
        ].some((value) => typeof value === 'number' && Number.isFinite(value) && value !== 0);
      };
      const settleableEntries = usageEntries.filter(isSettleableObservation);
      const hasSettleableObservation = settleableEntries.length > 0;
      ...
      const unknownEntries = settleableEntries.filter((entry) => {
        const cost = entry.cost_usd ?? entry.usd;
        const hasCost = typeof cost === 'number' && Number.isFinite(cost);
        const hasSource = entry.usd_source === 'reported' || entry.usd_source === 'estimated';
        return !hasCost || !hasSource;
      }).length;
      ...
      // COMP-COST-OWNER Open Question 0. Cache tokens are summed per ENTRY, selected exactly
      // as unknownEntries above -- and for the same documented reason. The aggregate `usage`
      // object is the wrong place to look: in the non-event path result-normalizer fills the
      // cache fields on the RECORD (usageRecordFromRaw, :306-310) while `usageTotals` keeps
      // the 0 it was seeded with at :409-410, so reading the aggregate silently reports no
      // caching on a fully cached build. Same trap as `usd_source` riding usages[0].
      // Both dialects are read: the short `cache_read`/`cache_creation` the normalizer emits
      // (:306-310, :761-765) and the raw `cache_*_input_tokens` a connector reports (:282-283).
      const sumEntries = (pick) => usageEntries.reduce((sum, entry) => {
        const value = entry && typeof entry === 'object' ? pick(entry) : 0;
        return sum + (typeof value === 'number' && Number.isFinite(value) ? value : 0);
      }, 0);
      const cacheReadTok = sumEntries((e) => e.cache_read ?? e.cache_read_input_tokens);
      const cacheCreationTok = sumEntries((e) => e.cache_creation ?? e.cache_creation_input_tokens);
      // Cache joins the write guard because a cache-ONLY usage is constructible by the
      // shipped producer, not merely imaginable: hasReportedUsage (result-normalizer:282-283)
      // returns true on cache tokens alone, so usageRecordFromRaw builds a record with
      // input_tokens: 0, output_tokens: 0 and a cache field. result-normalizer:521 already
      // treats that case as real when it counts an unpriced step (`inTok + outTok + ccit +
      // crit > 0`). Without this term such a usage would be dropped here in silence.
      if (tokens !== 0 || usd !== 0 || unknownEntries !== 0
        || cacheReadTok !== 0 || cacheCreationTok !== 0 || hasSettleableObservation) {
        updateBuildAccumulator(cwd, featureCode, (accumulator) => ({
          ...
          // Same null stickiness as the split above: null came from a v3 record that could
          // not know its cache totals, and a partial count presented as a total is the
          // failure this feature exists to end.
          cache_read_tokens: accumulator.cache_read_tokens === null
            ? null
            : accumulator.cache_read_tokens + cacheReadTok,
          cache_creation_tokens: accumulator.cache_creation_tokens === null
            ? null
            : accumulator.cache_creation_tokens + cacheCreationTok,
          usd_unknown_count: accumulator.usd_unknown_count === null
            ? null
            : accumulator.usd_unknown_count + unknownEntries,
          // Use aggregateUsdSource itself for the sticky reduction: unknown dominates,
          // estimated beats reported, and only a stated source can label a cost.
          // A pristine record's null means "no observation yet"; after any persisted
          // cost observation, null means the aggregate provenance is unknowable.
          usd_source: (() => {
            const hasPriorObservation = accumulator.usd_source !== null
              || accumulator.usd_unknown_count === null
              || accumulator.usd_unknown_count > 0
              || accumulator.tokens_total !== 0
              || accumulator.cache_read_tokens === null
              || accumulator.cache_creation_tokens === null
              || accumulator.cache_read_tokens !== 0
              || accumulator.cache_creation_tokens !== 0;
            const prior = hasPriorObservation
              ? [{ cost_usd: 0, usd_source: accumulator.usd_source }]
              : [];
            return aggregateUsdSource([...prior, ...settleableEntries]);
          })(),
        }));
      }
    ...
    // Last consumer-merge failure fingerprint per merge gate. The journal seeds
    // both maps on resume; these caches keep the ordinary within-process path
    // cheap while code + conflicting paths survive regenerated patch text.
    ...
    while (!isTerminalFlow(response.status) && !buildCancel.cancelled) {
      ...
      if (response.status === 'ready') {
        ...
        let requiredPrompt = basePrompt;
        if (isReviewMain) {
          requiredPrompt = buildStepPrompt(stepDispatch, {
            ...context,
            contextDir: null,
          });
        }
        if (isReviewScaffoldMain) {
          const reviewScaffold = buildReviewPrompt({
            agentType,
            lens: 'general',
            lensFocus: '',
            exclusions: '',
        ...
        const resolveGateWithConsumerMerge = async (requestedOutcome, requestedRationale, resolvedBy) => {
          ...
          const repairFor = (error) => {
            ...
            const persistedRetry = consumerMergeArtifacts?.gateRetryState(stepId);
            const previousFailure = consumerMergeFailures.has(stepId)
              ? consumerMergeFailures.get(stepId)
              : persistedRetry?.lastFailureFingerprint;
            const decision = decideMergeRepairOutcome(
              previousFailure, failure, repairOutcome,
            );
            const fingerprint = mergeFailureFingerprint(failure);
            consumerMergeFailures.set(stepId, fingerprint);
            consumerMergeArtifacts?.recordGateRetryState(stepId, {
          ...
          try {
            ...
            if (context.routing && routingIntegrityError(error)) throw error;
            const acknowledgement = disposition
              ? acknowledgeRoutingGate(context, disposition, null, await stratum.audit(flowId))
              : undefined;
            ...
            // Four arms: adopt an explicit already-resolved refusal; adopt a
            // validated engine token witness; adopt under wave profiles as a
            // SCOPE EXCEPTION; or rethrow. The first two adoption arms provide
            // evidence. The wave-profile arm does not: with no routing plane,
            // consumed-token evidence is unavailable anywhere because the engine
            // does not record it. It preserves pre-84eb338 behaviour for that
            // scope rather than satisfying the proof requirement. This guard is
            // named for its context, not its invariant. Do not delete it again as
            // "redundant"; that has happened twice. Without it,
            // test/build-wave-ship.test.js:103 fails.
            const alreadyResolved = /^(?:MCP error -32603: )?gate is not awaiting a decision$/.test(error?.message ?? '');
            if ((alreadyResolved || acknowledgement?.reconciliation === 'token-engine-witness' || waveProfilesEnabled(pipelineProfiles)) && decision) {
              outcome = decision;
        ...
        // The manager is available here for consumer merge gates even when the
        // fan-out was empty. Seed lazily because ordinary gates have no journal.
        if (!gateReentries.has(stepId) && consumerMergeArtifacts) {
          gateReentries.set(stepId, consumerMergeArtifacts.gateRetryState(stepId).roundCount);
        }
        const gateReentryCount = (gateReentries.get(stepId) ?? 0) + 1;
        gateReentries.set(stepId, gateReentryCount);
        consumerMergeArtifacts?.recordGateRetryState(stepId, { roundCount: gateReentryCount });
        ...
        if (policy.mode === 'skip') {
          ...
          const serverUp = await probeWorkspace(undefined, workspaceId);
          if (!serverUp && await probeServer()) {
            console.log(
              `Compose server http://localhost:${resolvePort()} cannot address workspace "${workspaceId}"; ` +
              'using headless gate resolution.',
            );
          }
    ...
    // STRAT-LEARN-INLINE-TS-1 §A5 / DELIVER-1 D6: at every non-aborted exit, show the
    // lessons and lesson reviews still waiting on the owner. Silent when the workspace's
    // [learn] switches are off; fail-open.
    if (!buildCancel.teardown && !buildCancel.cancelled) {
      learnSummaryPrinted = true;
      await printLearnSummary(cwd);
    }
    ...
    // COMP-COST-OWNER 0d-1: runBuild's terminal result. Before this, every non-throwing
    // path resolved `undefined`, so a terminal failure was indistinguishable from a clean
    // build and `compose build` exited 0 on it. `ok` is true ONLY for 'complete': 'killed'
    // (the gate rejected the work) and 'aborted' (the flow was cancelled) did not ship, so
    // a caller gating CI on this must see them as failures. A build that THROWS still
    // rejects — this is the contract for terminal states the loop reaches without throwing,
    // and it mirrors the `{ ok, flowId, status }` shape abortBuild already returns.
    ...
    // STRAT-LEARN-INLINE-TS-1 §A5: a thrown build is still a build exit, so the
    // summary belongs here too — unless the terminal path already printed it or
    // the build was cancelled. printLearnSummary is fail-open and never throws
    // into the rethrow path.
    if (!learnSummaryPrinted && !buildCancel.teardown && !buildCancel.cancelled) await printLearnSummary(cwd);
    throw err;
  } finally {
...
function rehydrateStepHistory(steps) {
  if (!Array.isArray(steps)) return [];
  return steps
    .filter(step => step && typeof step.id === 'string' && step.id.length > 0)
    .map(step => ({
      stepId: step.id,
      outcome: step.status === 'done' || step.status === 'succeeded' || step.status === 'completed'
        ? 'complete'
        : step.status === 'revised'
          ? 'revise'
          : step.status === 'killed'
      ...
      filesChanged: Array.isArray(step.filesChanged) ? [...step.filesChanged] : null,
      retries: step.retries ?? 0,
      violations: Array.isArray(step.violations) ? [...step.violations] : [],
...
export async function pollGateResolution(visionWriter, gateId, intervalMs = 2000, signal) {
  const timeoutMs = resolveGateTimeoutMs();
  const deadline = Date.now() + timeoutMs;
  const deadlineError = () => Object.assign(
    new Error(
      `Gate ${gateId} was not resolved before the ${timeoutMs}ms deadline. ` +
      `Resolve it headlessly with: compose gate resolve ${gateId} --approve ` +
      '(or use --revise/--kill).',
    ),
    { code: 'GATE_POLL_TIMEOUT', gateId, timeoutMs },
  );
  const beforeDeadline = async (promise) => {
    ...
    try {
      return await Promise.race([
        promise,
        new Promise((_, reject) => { timer = setTimeout(() => reject(deadlineError()), remaining); }),
      ]);
    } finally {
      clearTimeout(timer);
    }
  ...
  while (true) {
    ...
    try {
      const gate = await beforeDeadline(visionWriter.getGate(gateId, { requireServer: true }));
      signal?.throwIfAborted();
      consecutiveFailures = 0;
      if (!gate) throw new Error(`Gate ${gateId} not found (404)`);
      if (gate.status === 'expired') throw deadlineError();
...
const MERGE_FAILURE_FINGERPRINT_PREFIX = 'merge-failure-v1:';

function mergeFailureFiles(failure) {
  const files = [];
  const patterns = [
    /patch failed: (.+?):\d+(?=\r?$)/gm,
    /^error:\s+(.+?):\s+(?:patch does not apply|does not match index|does not exist in index|already exists in working directory|cannot apply binary patch).*$/gm,
    /^Applied patch to '(.+)' with conflicts\.?$/gm,
    /^U\s+(.+)$/gm,
  ];
  for (const pattern of patterns) {
    for (const match of String(failure).matchAll(pattern)) {
      const file = normalizeOwnedPath(match[1]);
...
/** Stable comparison key for one merge failure, suitable for the durable journal. */
export function mergeFailureFingerprint(failure) {
  if (typeof failure !== 'string') return String(failure);
  if (failure.startsWith(MERGE_FAILURE_FINGERPRINT_PREFIX)) return failure;
  const code = failure.match(/^([A-Z][A-Z0-9_]+):/)?.[1];
  const files = mergeFailureFiles(failure);
  return code && files.length > 0
    ? `${MERGE_FAILURE_FINGERPRINT_PREFIX}${code}:${JSON.stringify(files)}`
    : failure;
}
 ...
 * Git's message body changes with regenerated hunks, so conflicts compare by
 * error code + normalized path set when those paths are available. Raw equality
 * remains the fallback for failures without file evidence.
 *
...
export function decideMergeRepairOutcome(previousFailure, failure, repairOutcome) {
  const fingerprint = mergeFailureFingerprint(failure);
  const previousFingerprint = previousFailure === undefined
    ? undefined
    : mergeFailureFingerprint(previousFailure);
  const repeated = previousFingerprint !== undefined && previousFingerprint === fingerprint;
  if (repairOutcome === 'revise' && repeated) {
    const files = mergeFailureFiles(failure);
    const repeatReason = files.length > 0
      ? `the same merge failure (${files.join(', ')}) as the previous round`
      : "identical to the previous round's failure";
    return {
      outcome: 'kill',
      repeated,
      rationale: `${failure} — ${repeatReason} at this gate; `
