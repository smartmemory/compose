...
export async function admitConsumerWave({ descriptor, descriptors = [descriptor], audit, localSpec,
  ...
  try {
    ...
    if (root === 'input' && path.length) {
      ...
      if (source?.status !== 'succeeded' || source.epoch !== epoch) throw new Error('Wave source is not current and succeeded');
      items = access(source.output, path.slice(1));
      sourceProvenance = { reference, step: root, epoch: source.epoch,
    ...
    for (const d of descriptors.filter(d => d.step === descriptor.step)) {
      ...
      if (!recorded || !Number.isInteger(d.stage) || d.stage < 0 || d.stage >= step.fanout.steps.length
...
export async function evaluateConfiguredGate(context, { localSpec, gateStepId, gateToken, audit, costCeilingUsd }) {
  ...
  if (!decision && config) {
    ...
    decision = decideGateFromOutput(config, audit.steps, { gateStepId, gateToken, ceiling, executeProfile, executeProvider });
  }
 ...
 * literals; no restrictions). Malformed or non-object sidecars fail closed.
 *
