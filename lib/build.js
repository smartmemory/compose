...
export async function runBuild(featureCode, opts = {}) {
  ...
  // it, in the outer catch. The flag keeps a post-summary throw from printing a
  // second copy.
  ...
  try {
    ...
    if (!buildCancel.teardown && !buildCancel.cancelled) {
      ...
      await printLearnSummary(cwd, { signal: buildCancel.signal });
    }
    ...
    // Keep signal handlers installed while failed-build summary queries are pending.
    if (!learnSummaryPrinted && !buildCancel.teardown && !buildCancel.cancelled) {
      learnSummaryPrinted = true;
      await printLearnSummary(cwd, { signal: buildCancel.signal });
    }
    throw err;
    ...
    // Include setup failures before the inner try, without repeating a terminal summary.
    if (!learnSummaryPrinted && !buildCancel.teardown && !buildCancel.cancelled) {
      learnSummaryPrinted = true;
      await printLearnSummary(cwd, { signal: buildCancel.signal });
    }
