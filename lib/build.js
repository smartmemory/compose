...
export async function executeParallelDispatchServer(
  ...
  // T2-F5-COMPOSE-MIGRATE-WORKTREE: apply diffs locally when isolation:worktree + capture_diff.
  // On merge conflict the helper throws to halt the CLI — emit build_step_done FIRST so
  // downstream event consumers still see start/done pairs for the step.
  const isolation = dispatchResponse.isolation ?? 'worktree';
 ...
 */
function applyServerDispatchDiffs(
  ...
  try {
      ...
      if (streamWriter) {
        streamWriter.write({
          ...
          message:
            ...
            `Flow has already advanced server-side (merge_status reported as "clean"). ` +
    ...
    if (appliedFiles.length > 0 && context) {
      const set = new Set(context.filesChanged ?? []);
