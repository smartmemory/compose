...
export function makeAskAgent(stratum, context, gateDispatch, gateExtras) {
  ...
  return async function askAgent(question, artifactPath) {
    ...
    const text = await stratum.runAgentText('claude', qaPrompt, {
      ...
      ...(flowTag(context.flowId, gateDispatch.step_id ?? gateDispatch.id)
        ? { flow: flowTag(context.flowId, gateDispatch.step_id ?? gateDispatch.id) }
        : {}),
      telemetry: {
        site: 'gate-qa',
        project_cwd: context.projectCwd ?? context.cwd,
...
export async function runBuild(featureCode, opts = {}) {
  ...
  try {
    ...
    const buildIdentity = { featureCode, flowId: registeredFlowId, pid: process.pid,
    ...
    if (!buildCancel.teardown && response.status === 'completed') buildStatus = 'complete';
    if (!buildCancel.teardown && (response.status === 'failed' || response.status === 'budget_exhausted') && !killedByGate) {
      buildStatus = 'failed';
    ...
    if (streamWriter && !buildCancel.teardown) {
      try {
