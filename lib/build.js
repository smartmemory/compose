  ...
  try {
    ...
    signalHandler = (signal) => {
      ...
      buildCancel.teardown = runCancelTeardown({
        ...
        // §3.7: identity-claimed like every other mutation. `stampPid` is not used here —
        // this process IS the driver, so stamping its own pid is correct.
        writeTerminal: () => {
          const claim = claimActiveBuild(dataDir, { featureCode, flowId: response?.runId, pid: process.pid });
          if (!claim.ok) return;
          writeActiveBuild(dataDir, { ...claim.record, status: 'aborted', completedAt: new Date().toISOString() });
        },
        removeListeners: () => {
