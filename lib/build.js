...
async function executeChildFlow(
  ...
  while (resp.status !== 'complete' && resp.status !== 'killed') {
    if (resp.status === 'execute_step') {
      ...
      // Fix pass: dispatch a fix step before retrying. For codex steps, the fix
      // goes to claude (cross-agent). For claude steps, the fix is same-agent but
      // a distinct prompt focused on fixing the issues, not re-running the check.
