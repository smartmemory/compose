...
const BUILD_ACCUMULATOR_VERSION = 4;
...
export async function runBuild(featureCode, opts = {}) {
  ...
  const rotateStaleAccumulatorForFreshStart = () => {
    ...
    // Zero, not null: a rotation installs a FRESH v4 record, whose cache totals are a
    ...
    lastOwnerCost = { accumulator_build_id: build_id ?? null, cost_usd: 0, input_tokens: 0, output_tokens: 0,
      cache_read_tokens: 0, cache_creation_tokens: 0, usd_unknown_count: 0 };
  ...
  try {
    ...
    context.recordBuildUsage = async (usage, meta) => {
      ...
      const unknownEntries = usageEntries.filter((entry) => {
      ...
      if (tokens !== 0 || usd !== 0 || unknownEntries !== 0
        || cacheReadTok !== 0 || cacheCreationTok !== 0) {
