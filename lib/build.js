...
export async function runBuild(featureCode, opts = {}) {
  ...
  if (!isBugMode) {
    ...
    if (_feat) {
      await _bp.setStatus(featureCode, 'IN_PROGRESS', { by: 'build' });
    }
  }
  ...
  try {
    ...
    if (response.status === 'complete' && buildStatus === 'complete') {
      ...
      if (!isBugMode) {
        ...
        // Original updateFeature silently no-oped when feature was missing.
        const _featPre = await _bp.getFeature(featureCode);
        if (_featPre) {
          await _bp.setStatus(featureCode, 'COMPLETE', { by: 'build' });
          // Attach filesChanged onto the now-COMPLETE feature. Read current state back
          // so putFeature receives the full object (it overwrites, not merges).
          const _feat = await _bp.getFeature(featureCode);
          if (_feat) {
            await _bp.putFeature(featureCode, { ..._feat, filesChanged: context.filesChanged ?? [] });
          }
        }
      }
      ...
      if (!isBugMode) {
        ...
        const _feat = await _bp.getFeature(featureCode);
        if (_feat) {
          // IN_PROGRESS → PLANNED is not a standard transition; force-reset to allow
          // re-builds after a killed run. This matches prior updateFeature behavior.
          await _bp.setStatus(featureCode, 'PLANNED', { force: true, by: 'build' });
        }
      }
      const termState = readActiveBuild(dataDir);
      if (termState) {
      ...
      if (!isBugMode) {
        ...
        if (_feat) {
          // IN_PROGRESS → PLANNED is not a standard transition; force-reset to allow
          // re-builds after a failed run. This matches prior updateFeature behavior.
          await _bp.setStatus(featureCode, 'PLANNED', { force: true, by: 'build' });
        }
      }
      const termState = readActiveBuild(dataDir);
