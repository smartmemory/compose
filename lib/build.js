...
export async function reportUsageReceipts(context, usage, meta = {}) {
  if (context?.buildCancel?.cancelled || !context?.receiptsMode || !context.flowId || typeof context.stratum?.usageReport !== 'function') {
    return [];
  }
...
/** One claim protects the cancelled driver's record AND its vision projection. */
async function terminalizeCancelledBuild({ dataDir, buildIdentity, visionWriter, itemId, failureReason }) {
  ...
  try { await visionWriter.updateItemStatus(itemId, 'killed'); } catch { /* best-effort projection */ }
}
  ...
  try {
    ...
    signalHandler = (signal) => {
      ...
      buildCancel.teardown = runCancelTeardown({
        ...
        // The teardown claims once before vision AND this write (§3.7).
        ...
        const resolveGateWithConsumerMerge = async (requestedOutcome, requestedRationale, resolvedBy) => {
          ...
          if (consumerMergeArtifacts && outcome === 'approve') {
            if (consumerMergePreparationError) {
              ...
              if (mergeApplied && (buildCancel.cancelled || await isRunCancelled(stratum, flowId))) {
          ...
          let next = await callFlowWithCancellation(stratum, 'gateResolve', flowId, buildCancel,
    ...
    if (streamWriter && !buildCancel.cancelled) {
      try {
...
export async function abortBuild(dataDir, featureCode, cwd, opts = {}) {
  ...
  try {
    ...
    if (!claim.ok) return refuse(claim.reason);
    if (ABORT_TERMINAL_STATUSES.has(claim.record.status)) return refuse(`already_${claim.record.status}`);
    if (item?.id) await visionWriter.updateItemStatus(item.id, 'killed');
