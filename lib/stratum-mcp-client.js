...
export class StratumMcpClient {
  ...
  #recordAgentDispatch(dispatchId, agentType, opts, resultOrError, outcome, elapsedMs, dispatchMeta = {}) {
    try {
      ...
      const usage = resultOrError?.usage && typeof resultOrError.usage === 'object'
        ? resultOrError.usage
        : {};
      // COMP-COST-OWNER: connectors report the true input/output detail and the
      // cost's provenance BESIDE the Budget-shaped usage — the same seam
      // #recordRoutingEvidence already prefers (STRAT-USAGE-SPLIT, ~:880). This row
      // used to hardcode both token fields to null, so EVERY stratum-routed dispatch
      // recorded a bare total. Measured 2026-09-14: `tokens_in` null on 36/39 claude
      // rows and 6/6 codex rows, which is why 449,116 historical unpriced tokens can
      // never be repriced — input and output differ by 6x on terra.
      const split = resultOrError?.split && typeof resultOrError.split === 'object'
        ? resultOrError.split
        : {};
      ...
      const event = {
        ...
        tokens_in: finiteOrNull(split.input),
        tokens_out: finiteOrNull(split.output),
        // NOT recomputed from the split: `tokens` is the connector's own total and
        // the two can legitimately differ (codex keeps OpenAI's raw dialect, where
        // input_tokens INCLUDES cached_input_tokens). Deriving it here would change
        // the field's meaning silently.
        tokens_total: finiteOrNull(usage.tokens),
        usd: finiteOrNull(usage.usd),
        // An ESTIMATED dollar must never reach a consumer looking like a reported
        // one. Absent provenance stays null — unknown, not a claim.
        usd_source: nonemptyStringOrNull(resultOrError?.usdSource),
  ...
  async #dispatchAgentRun(agentType, prompt, opts, callOpts) {
    const { routingCalls, ...transportOpts } = opts;
    opts = routingCalls ? transportOpts : opts;
    const dispatchId = randomUUID();
    const intent = routingCalls?.begin({ callId: dispatchId, transport: 'mcp', provider: String(agentType ?? 'claude').split(':')[0], model: opts.modelID, effort: opts.effort });
    ...
    const dispatchMeta = { observing: Boolean(intent) };
    try {
      ...
      if (intent) await routingCalls.finish(intent, { value: result, launched: dispatchMeta.launched, returned: true });
      return result;
    } catch (error) {
      if (intent && routingIntegrityError(error)) { attachDispatchId(error, dispatchId); throw error; }
      this.#recordAgentDispatch(
        dispatchId,
        agentType,
        opts,
      ...
      if (intent) {
        const returned = !dispatchMeta.launched || Boolean(dispatchMeta.returned)
          && !['CANCELLATION_UNCONFIRMED', 'CANCELLATION_TEARDOWN_TIMEOUT'].includes(error?.code);
        await routingCalls.finish(intent, { value: error, failed: true, launched: Boolean(dispatchMeta.launched),
          returned, termination: dispatchMeta.termination });
        if (!returned && dispatchMeta.outcome) routingCalls.observeLate?.(intent, dispatchMeta.outcome, () => Boolean(dispatchMeta.returned));
      }
      throw error;
    }
  }
  ...
  async #invokeAgentRun(agentType, prompt, opts, callOpts, dispatchMeta = {}) {
    ...
    dispatchMeta.launched = true;
    const rpc = this.#callTool('stratum_agent_run', request, dispatchMeta.observing
      ? { ...callOpts, onRoutingReturn: () => { dispatchMeta.returned = true; } } : callOpts);
    ...
    const outcome = rpc.then(value => { dispatchMeta.returned = true; return { value }; }, error => {
      // A protocol agent error is a returned result. A broken transport proves no teardown.
      dispatchMeta.returned ||= error instanceof StratumError && Boolean(error.detail);
      return { error };
    });
    dispatchMeta.outcome = outcome;
    let cancellation;
    let rejectAbort;
    ...
    const cancel = () => {
      cancellation ??= (async () => {
        ...
        dispatchMeta.termination = { acknowledgement: ack, rpcSettled: true };
  ...
  async #callTool(toolName, args, opts = {}) {
    ...
    const result = await (client ?? this.#client).callTool(callArgs, undefined, requestOpts).catch(error => {
      if (error instanceof McpError && ![ErrorCode.ConnectionClosed, ErrorCode.RequestTimeout].includes(error.code)) opts.onRoutingReturn?.();
  ...
  async runAgentText(agentType, prompt, opts = {}) {
    ...
    const reportUsage = async (value) => {
      ...
      try {
        ...
        if (opts.routingCalls && routingIntegrityError(error)) throw error;
