...
export class GitHubProvider extends TrackerProvider {
  ...
  async recordCompletion(code, rec) {
    return this._lock(code, async () => {
      ...
      // Dedup by commit_sha in cache path
      if (rec.commit_sha && completions.some(c => c.commit_sha === rec.commit_sha)) {
        return this.cache.get(code);
      }
  ...
  async _applyOp(op) {
    ...
    if (op.op === 'setStatus') {
      ...
      // Post status event as a compose-event comment
      await this._postEvent(id.issueNumber, op.payload.event);
      // Best-effort Projects v2 mirror (failure does NOT fail the op)
      try {
        await this.api.graphql('mutation{updateProjectV2ItemFieldValue(input:{}){projectV2Item{id}}}', {
          nodeId: id.nodeId, status: op.payload.to,
        });
      } catch (err) {
        // Projects v2 is a mirror; log and continue
        console.warn('[github-provider] Projects v2 update failed (non-fatal):', err?.message);
      }
      return { version: updated.updated_at };
    }
    ...
    if (op.op === 'recordCompletion') {
      ...
      // Idempotent: skip if already persisted (reconcile replay safety)
      if (!sha || !existingCompletions.some(c => c.commit_sha === sha)) {
      ...
      await this._postEvent(id.issueNumber, {
        type: 'completion',
        commit_sha: op.payload.completion.commit_sha,
        ts: Date.now(),
      });
      return { version: updated.updated_at };
    }
