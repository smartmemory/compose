...
/**
 * Accepted `prior` tiers, DERIVED from the tier table rather than hand-kept, so a tier
 * added there can never be rejected here. (`budget` is accepted but not currently
 * producible: it is addressable, not laddered, so no mapping resolves to it — see
 * server/model-tiers.js.) model-tiers.js has no imports of its own, so this preserves
 * the module's purity.
 */
const PRIOR_TIERS = [null, ...Object.keys(MODEL_TIERS)];
...
export function dispatchKey({ preset, scopedStep, stage, provider, template = '', prior, fingerprint }) {
  ...
  if (typeof template !== 'string' || !PRIOR_TIERS.includes(prior)) invalid('Invalid template/prior');
