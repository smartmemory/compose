...
import { calculateEventCost } from './model-pricing.js';
  ...
  const unsub = stratum.onEvent(correlationId, subStepId, (env) => {
    ...
    switch (env.kind) {
      ...
      case 'step_usage': {
        ...
        const hasCost = m.cost_usd != null;
        const stepCost = hasCost
          ? m.cost_usd
          : calculateEventCost(m.model, inTok, outTok, ccit, crit);
        const stepSource = m.usd_source
          // No stated provenance: a supplied cost is the producer's report; a cost we had
          // to derive ourselves is an estimate, and says so.
          ?? (hasCost ? 'reported' : 'estimated');
