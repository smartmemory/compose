...
export class PromptBudgetExceededError extends AgentError {
  constructor({ maxChars, promptChars, requiredChars }) {
    super(
      `Review prompt budget exceeded before provider dispatch: required task, contract, and schema use `
      + `${requiredChars} characters, above the ${maxChars}-character limit `
      + `(assembled prompt: ${promptChars} characters).`,
    );
    this.name = 'PromptBudgetExceededError';
    this.code = 'REVIEW_PROMPT_BUDGET_EXCEEDED';
...
export async function runAndNormalize(_connectorIgnored, prompt, stepDispatch, opts = {}) {
  ...
  const promptBudget = opts.promptBudget;
  if (promptBudget && Number.isInteger(promptBudget.maxChars) && promptBudget.maxChars > 0
    && actualPrompt.length > promptBudget.maxChars) {
    let requiredPrompt = promptBudget.requiredPrompt;
    if (typeof requiredPrompt !== 'string') requiredPrompt = prompt;
    if (closureSchema) requiredPrompt = injectSchema(requiredPrompt, closureSchema);
    else if (hasSchema) requiredPrompt = injectSchema(requiredPrompt, schema);

    if (requiredPrompt.length > promptBudget.maxChars) {
      throw new PromptBudgetExceededError({
        maxChars: promptBudget.maxChars,
        promptChars: actualPrompt.length,
        requiredChars: requiredPrompt.length,
      });
    }

    const droppedChars = actualPrompt.length - requiredPrompt.length;
    const message = `Review prompt exceeded ${promptBudget.maxChars} characters; `
      + `dropped ${droppedChars} characters of ambient docs/context content before provider dispatch.`;
    progress?.warn?.(message);
    console.warn(`[review-prompt] ${message}`);
    actualPrompt = requiredPrompt;
  }
  ...
  try {
    ...
    try {
      ...
      if (typeof err?.providerFailureClass === 'string') {
        agentError.providerFailureClass = err.providerFailureClass;
      }
      if (Number.isFinite(err?.providerRetryAfterMs)) {
        agentError.providerRetryAfterMs = err.providerRetryAfterMs;
      }
