...
export class VisionWriter {
  ...
  async _restResolveGate(gateId, outcome, comment) {
    await this._fetch(`/api/vision/gates/${encodeURIComponent(gateId)}/resolve`, {
      method: 'POST',
      body: JSON.stringify({ outcome: normalizeOutcome(outcome), comment: comment || null }),
    });
  }
  ...
  _directResolveGate(gateId, outcome, comment) {
    const state = this._load();
