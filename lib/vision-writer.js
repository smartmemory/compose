/**
 * VisionWriter — Dual-dispatch read-modify-write for vision-state.json
 *
 * Routes mutations through REST when the Compose server is running,
 * falls back to direct atomic file writes when it's down.
 *
 * Provides step-to-item mapping, feature item lookup (using lifecycle.featureCode),
 * gate management with outcome normalization, and migration of legacy formats.
 */

import fs from 'node:fs';
import path from 'node:path';
import crypto from 'node:crypto';
import { resolvePort } from './resolve-port.js';
import { probeServer, probeWorkspace } from './server-probe.js';
import { migrateVisionItemFeatureCode, normalizeGateOutcome } from './state-migrations.js';
import { resolveMode } from './lifecycle-modes.js';

const EMPTY_STATE = () => ({ items: [], connections: [], gates: [] });

/**
 * Locate the vision item a featureCode refers to, tolerant of UI-created items
 * that have no lifecycle yet (#31). Priority order: lifecycle.featureCode (the
 * canonical binding), then the raw item id, then a top-level item.featureCode.
 * A lifecycle match anywhere in the list beats an id/featureCode match, so the
 * three passes are evaluated in order rather than first-item-wins.
 *
 * @param {Array<object>} items
 * @param {string} featureCode
 * @returns {object|null}
 */
function matchFeatureItem(items, featureCode) {
  return items.find(item => item.lifecycle?.featureCode === featureCode)
    || items.find(item => item.id === featureCode)
    || items.find(item => item.featureCode === featureCode)
    || null;
}

/** Canonical outcome normalization — maps legacy past-tense to imperative.
 *  Shared single implementation lives in state-migrations.js (COMP-MIGRATE-UNIFY-VISION). */
const normalizeOutcome = normalizeGateOutcome;

export class ServerUnreachableError extends Error {
  constructor(message = 'Server is unreachable') {
    super(message);
    this.name = 'ServerUnreachableError';
  }
}

export class VisionWriter {
  /**
   * @param {string} dataDir  Path to the data directory (e.g. `.compose/data/`)
   * @param {object} [opts]
   * @param {number} [opts.port]  Server port (default: resolvePort())
   * @param {string} [opts.workspaceId]  Workspace id; when set, sent as `X-Compose-Workspace-Id` on REST calls
   */
  constructor(dataDir, opts = {}) {
    this.filePath = path.join(dataDir, 'vision-state.json');
    this._dataDir = dataDir;
    this._port = opts.port ?? resolvePort();
    this.workspaceId = opts.workspaceId;
  }

  // ---------------------------------------------------------------------------
  // Internal helpers
  // ---------------------------------------------------------------------------

  /**
   * Read and parse vision-state.json, applying migration if needed.
   * Returns empty state if the file doesn't exist or contains invalid JSON.
   */
  _load() {
    try {
      const raw = fs.readFileSync(this.filePath, 'utf-8');
      const parsed = JSON.parse(raw);
      parsed.items = parsed.items || [];
      parsed.connections = parsed.connections || [];
      parsed.gates = parsed.gates || [];

      // Migration: legacy featureCode → lifecycle.featureCode (shared impl)
      let migrated = false;
      for (const item of parsed.items) {
        if (migrateVisionItemFeatureCode(item).changed) migrated = true;
      }

      // Dedup gates by ID (keep latest)
      const gateMap = new Map();
      for (const gate of parsed.gates) gateMap.set(gate.id, gate);
      if (gateMap.size < parsed.gates.length) { migrated = true; }
      parsed.gates = Array.from(gateMap.values());

      // Migration: normalize legacy gate outcomes
      for (const gate of parsed.gates) {
        if (gate.outcome) {
          const normalized = normalizeOutcome(gate.outcome);
          if (normalized !== gate.outcome) {
            gate.outcome = normalized;
            migrated = true;
          }
        }
      }

      if (migrated) {
        this._atomicWrite(parsed);
      }

      return parsed;
    } catch {
      return EMPTY_STATE();
    }
  }

  /**
   * Atomically write state by writing to a temp file then renaming.
   * @param {object} state
   */
  _atomicWrite(state) {
    const dir = path.dirname(this.filePath);
    fs.mkdirSync(dir, { recursive: true });
    const tmp = path.join(dir, `vision-state.json.tmp.${crypto.randomUUID()}`);
    fs.writeFileSync(tmp, JSON.stringify(state, null, 2) + '\n', 'utf-8');
    fs.renameSync(tmp, this.filePath);
  }

  /** Check if the Compose server can address this writer's workspace. */
  async _serverAvailable() {
    return this.workspaceId
      ? probeWorkspace(this._port, this.workspaceId)
      : probeServer(this._port);
  }

  /** Base URL for REST calls */
  get _baseUrl() {
    return `http://localhost:${this._port}`;
  }

  /** Fetch helper with timeout */
  async _fetch(urlPath, opts = {}) {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 5000);
    try {
      const res = await fetch(`${this._baseUrl}${urlPath}`, {
        ...opts,
        signal: controller.signal,
        headers: {
          'Content-Type': 'application/json',
          ...(this.workspaceId ? { 'X-Compose-Workspace-Id': this.workspaceId } : {}),
          ...opts.headers,
        },
      });
      if (!res.ok) {
        const body = await res.text().catch(() => '');
        throw new Error(`REST ${opts.method || 'GET'} ${urlPath} failed: ${res.status} ${body}`);
      }
      return res.json();
    } finally {
      clearTimeout(timer);
    }
  }

  // ---------------------------------------------------------------------------
  // REST variants
  // ---------------------------------------------------------------------------

  async _restFindFeatureItem(featureCode) {
    const state = await this._fetch('/api/vision/items');
    return matchFeatureItem(state.items || [], featureCode);
  }

  async _restEnsureFeatureItem(featureCode, title, mode = 'feature') {
    const existing = await this._restFindFeatureItem(featureCode);
    if (existing) {
      // Partial repair: item exists (e.g. UI-created, matched by id or
      // featureCode) but no lifecycle — start lifecycle so the next CLI run
      // finds it by lifecycle.featureCode (#31).
      if (!existing.lifecycle?.featureCode) {
        // COMP-ROADMAP-PLAN: forward `mode` so a repaired (e.g. UI-created) plan
        // item starts as plan, not build. store.createItem drops unknown fields,
        // so mode must travel on the start body (not item-create).
        await this._fetch(`/api/vision/items/${existing.id}/lifecycle/start`, {
          method: 'POST',
          body: JSON.stringify({ featureCode, mode }),
        });
      }
      return existing.id;
    }
    const item = await this._fetch('/api/vision/items', {
      method: 'POST',
      body: JSON.stringify({
        type: mode === 'bug' ? 'bug' : 'feature',
        title: title || featureCode,
        description: '',
        status: 'planned',
        phase: 'planning',
      }),
    });
    // COMP-ROADMAP-PLAN: forward `mode` on the post-create start so the lifecycle
    // is stamped with the right mode (the item-create body can't carry it —
    // store.createItem has no `mode` field).
    await this._fetch(`/api/vision/items/${item.id}/lifecycle/start`, {
      method: 'POST',
      body: JSON.stringify({ featureCode, mode }),
    });
    return item.id;
  }

  async _restUpdateItemStatus(itemId, status) {
    await this._fetch(`/api/vision/items/${itemId}`, {
      method: 'PATCH',
      body: JSON.stringify({ status }),
    });
  }

  async _restUpdateItemPhase(itemId, stepId) {
    await this._fetch(`/api/vision/items/${itemId}/lifecycle/advance`, {
      method: 'POST',
      body: JSON.stringify({ targetPhase: stepId }),
    }).catch(() => {
      // Lifecycle advance may reject for invalid transitions — fall through
    });
  }

  async _restCreateGate(flowId, stepId, itemId, opts = {}) {
    const round = opts.round ?? 1;
    const gate = await this._fetch('/api/vision/gates', {
      method: 'POST',
      body: JSON.stringify({
        flowId, stepId, round, itemId,
        fromPhase: opts.fromPhase || null,
        toPhase: opts.toPhase || null,
        artifact: opts.artifact || null,
        options: opts.options || null,
        summary: opts.summary || null,
        comment: opts.comment || null,
        policyMode: opts.policyMode || undefined,
      }),
    });
    return gate.id;
  }

  async _restGetGate(gateId) {
    try {
      return await this._fetch(`/api/vision/gates/${encodeURIComponent(gateId)}`);
    } catch {
      return null;
    }
  }

  async _restResolveGate(gateId, outcome, comment, resolvedBy) {
    await this._fetch(`/api/vision/gates/${encodeURIComponent(gateId)}/resolve`, {
      method: 'POST',
      body: JSON.stringify({ outcome: normalizeOutcome(outcome), comment: comment || null, resolvedBy: resolvedBy || undefined }),
    });
  }

  // ---------------------------------------------------------------------------
  // Direct (file-based) variants
  // ---------------------------------------------------------------------------

  _directFindFeatureItem(featureCode) {
    const state = this._load();
    return matchFeatureItem(state.items, featureCode);
  }

  _directEnsureFeatureItem(featureCode, title, mode = 'feature') {
    const state = this._load();
    const existing = matchFeatureItem(state.items, featureCode);
    if (existing) {
      // Partial repair: a UI-created item matched by id/featureCode but has no
      // lifecycle yet — seed lifecycle.featureCode so the next CLI run binds by
      // the canonical key (#31).
      if (!existing.lifecycle?.featureCode) {
        existing.lifecycle = {
          ...(existing.lifecycle || {}),
          featureCode,
          // currentPhase stays 'explore_design' (byte-identical to pre-MODES for
          // build AND bug; it is transient — the runner overwrites it with the
          // first stepId). Per-mode genesis-at-creation is deferred with the
          // fix/plan UI phase wiring.
          currentPhase: existing.lifecycle?.currentPhase || 'explore_design',
          mode: existing.lifecycle?.mode || resolveMode(mode),
        };
        this._atomicWrite(state);
      }
      return existing.id;
    }

    // Derive group from featureCode (same logic as vision-store.js deriveGroup)
    const groupMatch = (title || featureCode).match(/^([A-Z]+-[A-Z]+|[A-Z]+)(?=-\d)/);
    const group = groupMatch ? groupMatch[1] : featureCode;
    const item = {
      id: crypto.randomUUID(),
      type: mode === 'bug' ? 'bug' : 'feature',
      title: title || featureCode,
      description: '',
      status: 'planned',
      phase: 'planning',
      group,
      // currentPhase stays 'explore_design' (byte-identical for build AND bug;
      // transient — overwritten by the runner's first stepId). mode is the new
      // additive field; per-mode genesis-at-creation is a deferred follow-up.
      lifecycle: { featureCode, currentPhase: 'explore_design', mode: resolveMode(mode) },
      slug: featureCode.toLowerCase().replace(/[^a-z0-9]+/g, '-'),
      confidence: null,
      createdAt: new Date().toISOString(),
    };
    state.items.push(item);
    this._atomicWrite(state);
    return item.id;
  }

  _directUpdateItemStatus(itemId, status) {
    const state = this._load();
    const item = state.items.find(i => i.id === itemId);
    if (item) {
      item.status = status;
      this._atomicWrite(state);
    }
  }

  _directUpdateItemPhase(itemId, stepId) {
    const state = this._load();
    const item = state.items.find(i => i.id === itemId);
    if (item) {
      item.lifecycle = item.lifecycle || {};
      item.lifecycle.currentPhase = stepId;
      this._atomicWrite(state);
    }
  }

  _directCreateGate(flowId, stepId, itemId, opts = {}) {
    const state = this._load();
    state.gates = state.gates || [];
    const round = opts.round ?? 1;
    const gate = {
      id: `${flowId}:${stepId}:${round}`,
      flowId,
      stepId,
      round,
      itemId,
      fromPhase: opts.fromPhase || null,
      toPhase: opts.toPhase || null,
      artifact: opts.artifact || null,
      options: opts.options || null,
      summary: opts.summary || null,
      comment: opts.comment || null,
      policyMode: opts.policyMode ?? 'gate',
      status: 'pending',
      createdAt: new Date().toISOString(),
    };
    // Dedupe by full gate ID, or by a pending gate for the same item+step
    // WITHIN THE SAME FLOW. The flowId guard is essential: without it, a fresh
    // build (new flowId) would reuse a stale pending gate left behind by a prior
    // crashed/aborted run for the same feature — that gate is never resolved, so
    // the new build polls it until the 30-min server TTL expires it and then
    // errors. Scoping to flowId mints a fresh per-run gate instead.
    const existingIdx = state.gates.findIndex(g =>
      g.id === gate.id ||
      (g.flowId === gate.flowId && g.itemId === gate.itemId && g.stepId === gate.stepId && g.status === 'pending')
    );
    if (existingIdx !== -1) {
      return state.gates[existingIdx].id;
    }
    state.gates.push(gate);
    this._atomicWrite(state);
    return gate.id;
  }

  _directGetGate(gateId) {
    const state = this._load();
    return (state.gates || []).find(g => g.id === gateId) || null;
  }

  _directResolveGate(gateId, outcome, comment, resolvedBy) {
    const state = this._load();
    const gate = (state.gates || []).find(g => g.id === gateId);
    if (gate) {
      gate.status = 'resolved';
      gate.outcome = normalizeOutcome(outcome);
      gate.comment = comment || null;
      gate.resolvedBy = resolvedBy ?? 'human';
      gate.resolvedAt = new Date().toISOString();
      this._atomicWrite(state);
    }
  }

  // ---------------------------------------------------------------------------
  // Public async methods — dual dispatch
  // ---------------------------------------------------------------------------

  async findFeatureItem(featureCode) {
    if (await this._serverAvailable()) {
      return this._restFindFeatureItem(featureCode);
    }
    return this._directFindFeatureItem(featureCode);
  }

  async ensureFeatureItem(featureCode, title, mode = 'feature') {
    if (await this._serverAvailable()) {
      return this._restEnsureFeatureItem(featureCode, title, mode);
    }
    return this._directEnsureFeatureItem(featureCode, title, mode);
  }

  async updateItemStatus(itemId, status) {
    const up = await this._serverAvailable();
    // COMP-COMPLETION-GATE slice 3 (AC-16, path 14): `complete` is not a status
    // this general writer sets on a managed build-mode feature item, in EITHER
    // transport. The legitimate write is `completeItem` — the self-verifying
    // seam of §2.3b, which re-reads canonical state before writing. Fix/plan
    // items, UI items with no lifecycle, and the `compose new` kickoff item
    // (build mode, no feature.json) are not feature completions and keep this
    // path (§2.3d). In REST mode the server's PATCH refuses too (AC-10); the
    // pre-check here makes the refusal the writer's own, with the writer's message.
    if (status === 'complete') {
      const item = up ? await this._restGetItem(itemId) : this._load().items.find(i => i.id === itemId);
      const { isManagedBuildItem } = await import('../server/completion-projection.js');
      if (item && isManagedBuildItem(item, this._workspaceRoot())) {
        const e = new Error(
          `VisionWriter.updateItemStatus: refusing to mark ${itemId} (${item.lifecycle.featureCode}) ` +
          `complete — a managed build-mode feature completes through the completion gate, which ` +
          `projects it via completeItem() after verifying the commit, tests and guard ledger.`,
        );
        e.code = 'COMPLETE_VIA_GATE_ONLY';
        throw e;
      }
    }
    if (up) {
      return this._restUpdateItemStatus(itemId, status);
    }
    return this._directUpdateItemStatus(itemId, status);
  }

  /**
   * COMP-COMPLETION-GATE slice 3 (§2.3b, AC-4b): the ONLY path that writes
   * `status: 'complete'` on a managed build-mode feature item. It carries no
   * authority: in REST mode the server re-reads feature.json + the guard ledger
   * before writing (`POST …/completion-projection`); in direct mode the same
   * predicate runs here. Either way a completion that canonical state does not
   * already record is refused.
   *
   * @param {string} itemId
   * @param {object} evidence
   * @param {string} evidence.featureCode
   * @param {string} [evidence.cwd]        workspace root; derived from dataDir when omitted
   * @param {string} [evidence.commitSha]
   * @param {string} [evidence.ledgerRef]
   * @returns {Promise<{ok:true, itemId:string, verified_by:string}>}
   */
  async completeItem(itemId, evidence = {}) {
    if (!evidence.featureCode) throw new Error('VisionWriter.completeItem: evidence.featureCode is required');
    if (await this._serverAvailable()) {
      const r = await this._fetch(`/api/vision/items/${itemId}/completion-projection`, {
        method: 'POST',
        body: JSON.stringify({
          featureCode: evidence.featureCode,
          commitSha: evidence.commitSha ?? null,
          ledgerRef: evidence.ledgerRef ?? null,
        }),
      });
      return { ok: true, itemId, verified_by: r.verified_by, transport: 'rest' };
    }
    return this._directCompleteItem(itemId, { ...evidence, cwd: evidence.cwd || this._workspaceRoot() });
  }

  /** The workspace root this writer's dataDir belongs to (`<root>/.compose/data`). */
  _workspaceRoot() {
    return path.resolve(this._dataDir, '..', '..');
  }

  async _restGetItem(itemId) {
    try {
      return await this._fetch(`/api/vision/items/${itemId}`);
    } catch {
      return null;
    }
  }

  async _directCompleteItem(itemId, evidence) {
    const state = this._load();
    const item = state.items.find(i => i.id === itemId);
    if (!item) throw new Error(`VisionWriter.completeItem: item not found: ${itemId}`);
    const { verifiedCompleteProjection, projectionStamp } = await import('../server/completion-projection.js');
    const v = await verifiedCompleteProjection({
      item, featureCode: evidence.featureCode, cwd: evidence.cwd, consultGuard: true,
    });
    if (!v.ok) {
      const e = new Error(`VisionWriter.completeItem: projection refused — ${v.reasons.join('; ')}`);
      e.code = 'PROJECTION_REFUSED';
      e.reasons = v.reasons;
      throw e;
    }
    item.status = 'complete';
    item.completion_projection = projectionStamp(v, { ...evidence, source: 'direct' });
    this._atomicWrite(state);
    return { ok: true, itemId, verified_by: v.verified_by, transport: 'direct' };
  }

  async updateItemPhase(itemId, stepId) {
    if (await this._serverAvailable()) {
      return this._restUpdateItemPhase(itemId, stepId);
    }
    return this._directUpdateItemPhase(itemId, stepId);
  }

  async createGate(flowId, stepId, itemId, opts = {}) {
    if (await this._serverAvailable()) {
      return this._restCreateGate(flowId, stepId, itemId, opts);
    }
    return this._directCreateGate(flowId, stepId, itemId, opts);
  }

  async getGate(gateId, opts = {}) {
    if (opts.requireServer) {
      const up = await this._serverAvailable();
      if (!up) throw new ServerUnreachableError();
      return this._restGetGate(gateId);
    }
    if (await this._serverAvailable()) {
      return this._restGetGate(gateId);
    }
    return this._directGetGate(gateId);
  }

  async resolveGate(gateId, outcome, comment, resolvedBy) {
    if (await this._serverAvailable()) {
      return this._restResolveGate(gateId, outcome, comment, resolvedBy);
    }
    return this._directResolveGate(gateId, outcome, comment, resolvedBy);
  }
}
