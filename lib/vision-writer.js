...
import { resolveMode, genesisOf } from './lifecycle-modes.js';
...
export class VisionWriter {
  ...
  _directEnsureFeatureItem(featureCode, title, mode = 'feature') {
    ...
    if (existing) {
      ...
      if (!existing.lifecycle?.featureCode) {
        existing.lifecycle = {
          ...(existing.lifecycle || {}),
          featureCode,
          currentPhase: existing.lifecycle?.currentPhase || genesisOf(mode),
    ...
    const item = {
      ...
      phase: 'planning',
      group,
      lifecycle: { featureCode, currentPhase: genesisOf(mode), mode: resolveMode(mode) },
