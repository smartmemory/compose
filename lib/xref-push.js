 ...
 * writes the EXTERNAL tracker (a GitHub/Forgejo issue's open/closed state) to match the
 ...
 * mutates feature.json: explicit `expect` remains unchanged, while links marked
 * `derive_expect` read the owning feature's current status at push time.
 * Resolution and the external write are both injectable for testability.
 */
...
import { featureStatusToExternalExpect } from './status-projection.js';
...
/** Build a ForgejoApi for `link`, or return a degrade reason. */
async function makeForgejoClient(link, { transport = null, auth } = {}) {
  let ForgejoApi;
  try { ({ ForgejoApi } = await import('./tracker/forgejo-api.js')); }
  catch (e) { return { skipped: true, reason: `forgejo client unavailable: ${e.message}` }; }
  const a = auth ?? { token: process.env.COMPOSE_FORGEJO_TOKEN };
  try {
    return { forgejo: new ForgejoApi({ repo: link.repo, auth: a }, transport) };
  } catch (e) {
    return { skipped: true, reason: (e && e.message) || 'no Forgejo auth' };
  }
}
...
/**
 * Resolve a Forgejo issue to its current state and labels. Pull-request-backed
 * issue records are rejected before any write can be planned.
 */
export async function defaultForgejoResolve(link, opts = {}) {
  if (!link.repo || link.issue == null) return { skipped: true, reason: 'incomplete forgejo ref' };
  const c = await makeForgejoClient(link, opts);
  if (c.skipped) return c;
  try {
    const r = await c.forgejo.getIssueResult(link.issue);
    if (r.status === 404) return { skipped: true, reason: `target ${link.repo}#${link.issue} missing (404)` };
    if (r.status < 200 || r.status >= 300) return { skipped: true, reason: `HTTP ${r.status}` };
    if (r.body && r.body.pull_request) {
      return { skipped: true, reason: `${link.repo}#${link.issue} is a pull request, not an issue` };
...
/**
 * Apply Forgejo state/label plans through independent endpoints. Every planned
 * aspect is attempted even if another aspect fails. Successful 2xx writes stay
 * visible in the returned outcome and are never collapsed into a skipped row.
 */
export async function defaultForgejoWrite(link, statePlan, labelsPlan, opts = {}) {
  const outcome = { statePushed: false, labelsPushed: false, labelsAdded: [], errors: [] };
  if (!link.repo || link.issue == null) {
    outcome.errors.push('incomplete forgejo ref');
    return outcome;
  }

  const c = await makeForgejoClient(link, opts);
  if (c.skipped) {
    outcome.errors.push(c.reason);
    return outcome;
  }

  if (statePlan.action === 'write') {
    try {
      const r = await c.forgejo.updateStateResult(link.issue, { state: statePlan.to });
      if (r.status >= 200 && r.status < 300) outcome.statePushed = true;
      else outcome.errors.push(`state write HTTP ${r.status}`);
    } catch (e) {
      outcome.errors.push(`state write: ${e && e.rateLimit ? 'rate limit' : (e && e.message) || 'write error'}`);
    }
  }

  if (labelsPlan.action === 'add') {
    let allLabelsPushed = true;
    for (const label of labelsPlan.add) {
      try {
        const r = await c.forgejo.addLabelResult(link.issue, label);
        if (r.status >= 200 && r.status < 300) {
          outcome.labelsAdded.push(label);
        } else {
          allLabelsPushed = false;
          outcome.errors.push(`label "${label}" write HTTP ${r.status}`);
        }
      } catch (e) {
        allLabelsPushed = false;
        outcome.errors.push(`label "${label}" write: ${e && e.rateLimit ? 'rate limit' : (e && e.message) || 'write error'}`);
      }
    }
    outcome.labelsPushed = allLabelsPushed;
  }
 ...
 * declared or derived `expect`. Eligibility: github/forgejo provider,
 * `push === true`, and a valid issue-state and/or label intent. Dry-run unless
 * `apply` is true. NEVER mutates feature.json.
 *
 * @param {string} cwd
 * @param {object} [opts]
 ...
 * @param {(link: object) => Promise<{state?: string, labels?: string[], skipped?: boolean, reason?: string}>} [opts.forgejoResolve]
 * @param {(link: object, statePlan: object, labelsPlan: object) => Promise<{statePushed: boolean, labelsPushed: boolean, labelsAdded: string[], errors: string[]}>} [opts.forgejoWrite]
 * @param {object} [opts.forgejoTransport] injectable transport for the default forgejo resolve/write (tests)
 * @param {object} [opts.forgejoAuth]      injectable auth for the default forgejo resolve/write (tests)
...
export async function pushExternalRefs(cwd, opts = {}) {
  ...
  const forgejoClientOpts = { transport: opts.forgejoTransport ?? null, auth: opts.forgejoAuth };
  const forgejoResolve = opts.forgejoResolve ?? ((link) => defaultForgejoResolve(link, forgejoClientOpts));
  const forgejoWrite = opts.forgejoWrite
    ?? ((link, statePlan, labelsPlan) => defaultForgejoWrite(link, statePlan, labelsPlan, forgejoClientOpts));
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    for (const link of fj.links) {
      ...
      if (link.provider === 'github' || link.provider === 'forgejo') {
        const derivesExpect = link.derive_expect === true;
        const expect = derivesExpect
          ? featureStatusToExternalExpect(fj.status)
          : link.expect;
        // PARKED/unknown derived state freezes the state aspect but does not
        // suppress an independently declared label intent.
        const effectiveLink = expect === link.expect ? link : { ...link, expect };
        // Eligible if it declares ANY intent (state and/or labels).
        if (expect == null && link.expect_labels == null) continue;
        scanned++;
        // Malformed-state skip fires ONLY when a state intent is present —
        // a labels-only link is still eligible.
        if (expect != null && !isGithubState(expect)) {
          skip(fj, link, `malformed expect "${expect}" (want open|closed)`);
          continue;
        }
        const resolve = link.provider === 'github' ? ghResolve : forgejoResolve;
        const r = await resolve(link);
        if (r.skipped) { skip(fj, link, r.reason); continue; }

        const statePlan = expect != null ? planPush(effectiveLink, r.state ?? null) : { action: 'none' };
        ...
        if (link.provider === 'github') {
          ...
          const outcome = apply
            ? await forgejoWrite(link, statePlan, labelsPlan)
            : { statePushed: false, labelsPushed: false, labelsAdded: [], errors: [] };
          pushed.push(forgejoRow(fj, link, statePlan, labelsPlan, outcome, apply));
        }
...
function forgejoRow(fj, link, statePlan, labelsPlan, outcome, apply) {
  const row = {
    code: fj.code,
    provider: 'forgejo',
    target: targetLabel(link),
    statePushed: outcome.statePushed === true,
    labelsPushed: outcome.labelsPushed === true,
    labelsAdded: Array.isArray(outcome.labelsAdded) ? outcome.labelsAdded : [],
  ...
  if (statePlan.action === 'write') {
    ...
    row.to = statePlan.to;
    row.state = { from: statePlan.from, to: statePlan.to };
    parts.push(`state ${statePlan.from} → ${statePlan.to}${apply && !row.statePushed ? ' (FAILED)' : ''}`);
  }
  if (labelsPlan.action === 'add') {
    row.labels = { added: apply ? row.labelsAdded : labelsPlan.add };
    if (apply) {
      const missing = labelsPlan.add.filter((label) => !row.labelsAdded.includes(label));
      const added = row.labelsAdded.length ? ` +${row.labelsAdded.join(',')}` : '';
      parts.push(`labels${added} (${row.labelsAdded.length}/${labelsPlan.add.length} written${missing.length ? `; not written: +${missing.join(',')}` : ''})`);
    } else {
      parts.push(`labels +${labelsPlan.add.join(',')}`);
    }
  }
  if (row.errors.length) parts.push(`FAILED: ${row.errors.join('; ')}`);
  row.summary = parts.join('; ');
  return row;
}
