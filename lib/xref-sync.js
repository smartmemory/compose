...
async function defaultResolve(link, cwd, featuresDir) {
  ...
  if (link.provider === 'local') {
    ...
    const siblingFeatures = join(cwd, '..', link.repo, featuresDir, link.to_code, 'feature.json');
    if (!existsSync(siblingFeatures)) return { skipped: true, reason: `local target ${link.repo}/${link.to_code} not found` };
    try {
      const fj = JSON.parse(readFileSync(siblingFeatures, 'utf8'));
      return { state: fj.status || null };
