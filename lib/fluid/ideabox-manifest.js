...
export class IdeaboxSourceChangedDuringImport extends Error {
  constructor(ideaboxPath) {
    super(
      ...
      `before the edit are already in the store, so re-running will NOT pick the edit up: copy the ` +
      `edited text somewhere safe, restore this file to the version the migration started from, ` +
      `run the command again so the migration finishes, then re-apply the edit with ` +
      `\`compose ideabox\`.`
    );
...
export function openManifest(provider, ideaboxPath, { markdown, planned, plannedClusters = [] }) {
  ...
  if (open && open.hash === hash && sameSet(open.planned, planned)) return path;
