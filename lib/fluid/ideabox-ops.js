...
export async function resolveCluster(provider, name) {
  ...
  const byTitle = clusters.filter((c) => c.title.toLowerCase() === String(name).toLowerCase());
  if (byTitle.length === 1) return { handle: byTitle[0].handle, created: null };
  if (byTitle.length > 1) {
    throw new IdeaboxInvalid(
      `"${name}" matches ${byTitle.length} clusters (${byTitle.map((c) => c.handle).join(', ')}). ` +
      `Pass the handle instead.`,
      'cluster'
    );
  }

  const created = await provider.createRecord({ kind: KIND.CLUSTER, title: String(name) });
  return { handle: created.handle, created };
}
