...
function warnIfUnsafelyShared(provider) {
  if (provider.isShared() && !mutationScopeAtLeast(provider.mutationScope(), MUTATION_SCOPE.CLUSTER)) {
    console.warn(
      ...
      + `store offers a cross-machine reservation primitive (smart-memory-service#4).`
    );
  }
  return provider;
}
...
async function construct(cwd, opts = {}) {
  ...
  if (name === 'smartmemory') {
    ...
    // (COMP-FLUID-SEAM-GUARANTEES). The second of the two gaps it named — an
    // import that could not be restarted, because `reclaimAborted` was ignored —
    // is FIXED; the first is blocked on smart-memory-service#4.
