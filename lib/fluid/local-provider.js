...
const FLUID_STATUSES = Object.freeze(new Set(['new', 'discussing', 'promoted', 'killed']));
 ...
 *  appendDiscussion(). */
const UNPATCHABLE = Object.freeze([
...
export class LocalFluidProvider extends FluidProvider {
   ...
   *
   * Every structured field is cloned. Returning stored objects by reference let
   ...
   *
   * Absent optional fields are filled to their contract defaults rather than
   * left `undefined`, so a record written before a field existed reads back the
   * same shape as one written after it.
   */
  _normalize(record) {
    return {
      id: record.id,
