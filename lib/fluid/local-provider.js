...
export class LocalFluidProvider extends FluidProvider {
   ...
   * corpus.
   *
   * An aborted allocation is safely distinguishable from a retired handle, and
   * the difference is what makes reclaiming it sound rather than merely
   * convenient: a retired handle was once LIVE, so a citation to it can exist,
   * which is the entire reason retired handles stay retired. An aborted one was
   * never live and never rendered, so nothing can be pointing at it.
   *
   * Deliberately conservative in both directions. Structured events are read
   * rather than `issuedHandlesFromLog`, whose regex matches a handle anywhere
   * in the log — including as another record's link target. And ANY `deleted`
   * event disqualifies the handle, so a record whose file was removed by hand
   * is treated as retired, not as an aborted create.
   */
