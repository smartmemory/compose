...
export class SmartMemoryFluidProvider extends FluidProvider {
  ...
  /**
   * NONE, honestly, and this is the one declaration in the file that is a
   * confession rather than a description.
   *
   * Allocation reads the issued set and adds one across two remote calls with
   * nothing holding the gap, so N concurrent creates allocate the same handle
   * and last-writer-wins destroys N-1 records. `lib/dir-lock.js` would not fix
   * it even if wired in: it is a local mutex and this store is reachable from
   * every machine sharing the workspace, which is the entire reason to use it.
   *
   * The service exposes no primitive to build on — no caller-supplied item id,
   * no conditional create, no compare-and-swap on update, no counter. Tracked as
   * SVC-LEASE-1 in SmartMemory's roadmap (`smart-memory-docs/docs/ROADMAP.md`),
   * asking for the scoped Redis lease the service already runs internally
   * (`snapshot_sweep.py`) to be exposed and to fail closed. NOT another team's
   * queue — we own SmartMemory, so this is a task, not a dependency. When it
   * lands, this returns CLUSTER and the conformance suite's concurrency case
   * starts applying to this provider.
   *
   * Declared NONE rather than left silent because a caller that believes it is
   * serialized and is not will lose records without an error — which is exactly
   * what shipped in S3b-1 and what this feature exists to make impossible to
   * repeat.
   */
  ...
  async createRecord(input) {
    ...
    if (input.handle === undefined) {
      ...
      // event log and record set, both already readable. It is therefore fixed
      // now rather than waiting on SVC-LEASE-1, which gates only the
      // serialization half.
