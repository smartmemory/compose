...
export class SmartMemoryFluidProvider extends FluidProvider {
   ...
   * no conditional create, no compare-and-swap on update, no counter. Filed
   * upstream as smart-memory-service#4, asking for the scoped Redis lease the
   * service already runs internally (`snapshot_sweep.py`) to be exposed and to
   * fail closed. When it lands, this returns CLUSTER and the conformance
   * suite's concurrency case starts applying to this provider.
   *
  ...
  async createRecord(input) {
    ...
    if (input.handle === undefined) {
      ...
      // now rather than waiting on smart-memory-service#4, which blocks only the
      // serialization half.
