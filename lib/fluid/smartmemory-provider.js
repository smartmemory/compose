...
/**
 * Kinds `recall()` may return — architecture.md §Q3's FULL set.
 *
 * `cluster` and `decision` are INDEXED: stored, readable by handle, and never
 * surfaced by recall. A cluster is a hand-authored grouping label rather than a
 * fuzzy-recall target, and fuzzy-matching a decision risks presenting a
 * superseded one as if it were live.
 *
 ...
 * nor verify it, so relying on it would leave INDEXED kinds recallable in any
 * unconfigured deployment. Filtering the OUTPUT holds regardless of how the
 * server is configured.
 */
export const RECALLABLE_KINDS = Object.freeze(new Set([KIND.IDEA, KIND.THREAD, KIND.QUESTION]));
