...
// ---------------------------------------------------------------------------
// COMP-TEST-BOOTSTRAP-4-1: isTestFile
// ---------------------------------------------------------------------------

/**
 * Whether a repo-relative path looks like a test file. Used to scope the
 * post-coverage test-review pass to the tests the coverage step produced.
 * Conservative: matches conventional test locations/suffixes across the
 * frameworks detectTestFramework knows, and avoids substring false positives
 * (e.g. "contest", "greatest").
 *
 * @param {string} p - repo-relative file path
 * @returns {boolean}
 */
export function isTestFile(p) {
  if (typeof p !== 'string' || p.length === 0) return false;
  return (
    /(^|\/)(tests?|__tests__|spec)\//.test(p) ||  // test/ tests/ __tests__/ spec/ dirs
    /\.(test|spec)\.[mc]?[jt]sx?$/.test(p) ||      // *.test.* / *.spec.* (js/ts/jsx/tsx/mjs/cjs)
    /_test\.go$/.test(p) ||                         // go
    /(^|\/)test_[^/]*\.py$/.test(p) ||              // pytest test_*.py
    /_test\.py$/.test(p)                            // pytest *_test.py
  );
}
