 ...
 *   - `createItem`/`getItem`/`listItems`/`updateItem`/`deleteItem` — typed-record
 *     CRUD over the generic MemoryItem routes, for callers that address items
 *     individually.
 *
 ...
 * @returns {{ health(): Promise<{ok:boolean,status?:number}>, ingest(content:string,ctx:object): Promise<{status:string,unchanged:boolean,raw:object}>, search(query:string,opts?:object): Promise<object>, createItem(input:object): Promise<object>, getItem(itemId:string): Promise<object|null>, listItems(opts?:object): Promise<object>, updateItem(itemId:string,patch:object): Promise<object>, deleteItem(itemId:string,opts?:object): Promise<object> }}
 */
export function createSmartmemoryClient(cfg) {
  ...
  return { health, ingest, search, createItem, getItem, listItems, updateItem, deleteItem };
}
