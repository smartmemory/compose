...
export function normalizePipelineProfiles(raw, spec) {
  ...
  if (Object.hasOwn(raw, '_routing')) {
    keys(raw._routing, ['mode']);
    if (raw._routing.mode !== 'shadow') fail('_routing.mode must be shadow');
  }
  for (const [id, entry] of Object.entries(raw)) {
    ...
    for (const step of matches) {
      ...
      if (object(entry)) {
        keys(entry, ['default', 'tier_from', 'route']);
        if (Object.hasOwn(entry, 'route')) validateRoute(entry.route);
...
export function mergeRuntimeProfiles(normalized, runtime = {}) {
  ...
  for (const [id, override] of Object.entries(runtime)) {
    if (object(override) && Object.hasOwn(override, 'route') || id === '_routing') fail('Runtime overrides cannot supply route policy', 'ROUTING_POLICY_OVERRIDE');
...
export function preflightPipelineProfiles(raw, spec, runtime = {}, routingOptions = {}) {
  const original = normalizePipelineProfiles(raw, spec);
  const projection = routingProfileProjection(original, routingOptions);
  const normalized = normalizePipelineProfiles(mergeRuntimeProfiles(projection.staticProfiles, runtime), spec);
  const resolved = {};
  const overrides = {};
  ...
  for (const [id, winner] of Object.entries(resolved)) {
    const originalEntry = projection.staticProfiles[id];
    const origin = routingOptions.runtimeOrigins?.[id];
    const hasOverride = Object.hasOwn(runtime, id);
    const supplied = hasOverride && (origin?.supplied ?? true);
    const manualFallback = { supplied, origin: origin?.origin ?? (hasOverride ? 'explicit' : 'none'),
      profile: hasOverride ? (typeof runtime[id] === 'string' ? runtime[id] : runtime[id].default) : null,
      recordedRole: origin?.recordedRole ?? null };
    const steps = Object.values(parsed?.flows ?? {}).flatMap(flow => flow?.steps ?? []);
    const step = steps.find(s => s.id === id) ?? steps.find(s => id.startsWith(`${s.id}/`));
    const stage = step?.fanout ? step.fanout.steps[Number(id.slice(step.id.length + 1)) || 0] : step;
    const specPrior = !supplied && origin?.recordedRole ? agent(origin.recordedRole).tier : agent(stage?.agent ?? winner.provider).tier;
    const prior = originalEntry ? resolveConsumerProfile(originalEntry, {}).tier : specPrior;
    staticProvenance[id] = { prior: prior ?? null, winner: structuredClone(winner),
      source: supplied ? 'manual' : originalEntry ? 'preset' : 'default',
      via: supplied ? 'manual.fallback' : originalEntry ? 'preset.default' : 'spec.agent', manualFallback,
      itemTier: originalEntry?.tier_from ? { source: 'preset', via: 'item.tier' } : null };
  }
  const result = { ok: true, normalized, resolved, profilesDigest: profilesDigest({ normalized, resolved, overrides }) };
  Object.defineProperties(result, {
    routingPolicy: { value: projection.policy, enumerable: false },
    staticProvenance: { value: staticProvenance, enumerable: false },
  });
  return result;
}
...
function validateRoute(route) {
  keys(route, ['learn']);
  if (typeof route.learn !== 'boolean') fail('route.learn must be boolean');
}
/** A route-only wrapper projects to a legacy string; existing default-only objects stay objects.
 * Custom sidecars must retain tier_from to preserve that object representation when adding route.
 */
export function routingProfileProjection(raw, { mode = raw?._routing?.mode ?? 'off', route_trials = [], route_explore = 0, calibration_feedback = false } = {}) {
  if (!['off', 'shadow'].includes(mode) || !Array.isArray(route_trials) || route_trials.length || route_explore !== 0 || calibration_feedback !== false) {
    fail('Only static off/shadow routing is available in S1a', 'ROUTING_SLICE_UNAVAILABLE');
  }
  if (Object.hasOwn(raw, '_routing')) {
    keys(raw._routing, ['mode']);
    if (raw._routing.mode !== 'shadow') fail('_routing.mode must be shadow');
  }
  const staticProfiles = structuredClone(raw);
  delete staticProfiles._routing;
  const routes = {};
  for (const [id, entry] of Object.entries(raw)) {
    if (id.startsWith('_')) continue;
    if (object(entry) && Object.hasOwn(entry, 'route')) {
      keys(entry, ['default', 'tier_from', 'route']);
      validateRoute(entry.route);
      routes[id] = structuredClone(entry.route);
      staticProfiles[id] = Object.hasOwn(entry, 'tier_from') ? { default: entry.default, tier_from: entry.tier_from } : entry.default;
    }
  }
  return { staticProfiles, policy: { mode, routes, route_trials: [], route_explore: 0, calibration_feedback: false } };
}
