 ...
 *   - XREF_TARGET_MISSING   (error)   github/forgejo 404 / local target absent
...
import { featureStatusToExternalExpect, featureStatusToVisionStatus } from './status-projection.js';
import { GitHubApi } from './tracker/github-api.js';
import { ForgejoApi } from './tracker/forgejo-api.js';
  ...
  for (const row of ctx.citationRows || []) {
    ...
    for (const p of parsed) {
      refs.push({
        ...
        expect_labels: p.expect_labels ?? null,
        derive_expect: p.derive_expect ?? null,
      });
    }
  }
  for (const [code, folder] of ctx.foldersByCode) {
    ...
    for (const l of fj.links) {
      if (l && l.kind === 'external') {
        refs.push({
          ...
          expect_labels: l.expect_labels ?? null,
          derive_expect: l.derive_expect ?? null,
        });
      }
    }
  }
...
function issueTrackerDrift(ref, state) {
  // Promotion-created links derive their expectation from the current
  // canonical feature status. Use the same projection as xref-push so
  // validation cannot disagree with the state a push would apply.
  if (ref.derive_expect === true) {
    const expect = featureStatusToExternalExpect(ref.citing.status);
    if (expect === null) return null;
    return state !== expect
      ? `expected ${ref.repo}#${ref.issue} to be ${expect} but it is ${state}`
      : null;
  }
...
function issueTrackerLabelDrift(ref, labels) {
  if (!Array.isArray(ref.expect_labels) || ref.expect_labels.length === 0) return null;
  const current = new Set((Array.isArray(labels) ? labels : [])
    .map((label) => (label && label.name != null ? label.name : label)));
  const missing = [...new Set(ref.expect_labels)].filter((label) => !current.has(label));
  if (missing.length === 0) return null;
  const noun = missing.length === 1 ? 'label' : 'labels';
  return `missing expected ${noun} ${missing.join(', ')} on ${ref.repo}#${ref.issue}`;
}
...
async function resolveGithubRef(ref, gh, findings) {
  ...
  const drift = issueTrackerDrift(ref, state);
  if (drift) {
    findings.push(finding('warning', 'XREF_DRIFT', code, locatorDetail(ref, drift), ref.source));
  }
  const labelDrift = issueTrackerLabelDrift(ref, r.body.labels);
  if (labelDrift) {
    findings.push(finding('warning', 'XREF_DRIFT', code, locatorDetail(ref, labelDrift), ref.source));
  }
}

async function resolveForgejoRef(ref, forgejo, findings) {
  const code = ref.citing.code || undefined;
  let r;
  try {
    r = await forgejo.getIssueResult(ref.issue);
  } catch (e) {
    if (e && e.rateLimit) { const x = new Error('ratelimit'); x._rateLimit = true; throw x; }
    // offline / fetch reject / unexpected — per-ref degrade
    findings.push(finding(
      'warning', 'XREF_RESOLUTION_SKIPPED', code,
      locatorDetail(ref, `forgejo resolution skipped for ${ref.repo}#${ref.issue}: ${e && e.message ? e.message : e}`),
      ref.source,
    ));
    return;
  }
  if (r.status === 404) {
    findings.push(finding(
      'error', 'XREF_TARGET_MISSING', code,
      locatorDetail(ref, `forgejo ${ref.repo}#${ref.issue} not found (404)`),
      ref.source,
    ));
    return;
  }
  if (r.status < 200 || r.status >= 300) {
    findings.push(finding(
      'warning', 'XREF_RESOLUTION_SKIPPED', code,
      locatorDetail(ref, `forgejo ${ref.repo}#${ref.issue} unresolved (HTTP ${r.status})`),
      ref.source,
    ));
    return;
  }
  if (!r.body || (r.body.state !== 'open' && r.body.state !== 'closed')) {
    findings.push(finding(
      'warning', 'XREF_RESOLUTION_SKIPPED', code,
      locatorDetail(ref, `forgejo ${ref.repo}#${ref.issue} returned no parseable issue state (HTTP ${r.status})`),
      ref.source,
    ));
    return;
  }
  const drift = issueTrackerDrift(ref, r.body.state);
  if (drift) {
    findings.push(finding('warning', 'XREF_DRIFT', code, locatorDetail(ref, drift), ref.source));
  }
  const labelDrift = issueTrackerLabelDrift(ref, r.body.labels);
...
async function runExternalRefChecks(ctx, findings, options = {}) {
  ...
  let forgejoNoTokenAggregated = false;
  let forgejoShortCircuited = false;
  ...
  for (const ref of refs) {
    ...
    try {
      ...
      if (ref.provider === 'forgejo') {
        if (!gateOn) {
          findings.push(finding(
            'warning', 'XREF_RESOLUTION_SKIPPED', code,
            locatorDetail(ref, `forgejo ${ref.repo}#${ref.issue} not resolved (network off; pass --external / COMPOSE_XREF_ONLINE=1)`),
            ref.source,
          ));
          continue;
        }
        if (forgejoShortCircuited) {
          // An aggregate no-token or rate-limit warning already explains why
          // the remaining Forgejo refs were not attempted.
          continue;
        }
        let forgejo;
        try {
          forgejo = new ForgejoApi(
            {
              repo: ref.repo,
              baseUrl: options.forgejoBaseUrl,
              auth: options.forgejoAuth || { tokenEnv: 'COMPOSE_FORGEJO_TOKEN' },
            },
            options.forgejoTransport || null,
          );
        } catch (e) {
          if (e && e.name === 'TrackerConfigError' && e.detail && e.detail.missing === 'token') {
            if (!forgejoNoTokenAggregated) {
              forgejoNoTokenAggregated = true;
              findings.push(finding(
                'warning', 'XREF_RESOLUTION_SKIPPED', undefined,
                'forgejo external refs skipped: no Forgejo token (set COMPOSE_FORGEJO_TOKEN)',
                'xref',
              ));
            }
            forgejoShortCircuited = true;
            continue;
          }
          findings.push(finding(
            'warning', 'XREF_RESOLUTION_SKIPPED', code,
            locatorDetail(ref, `forgejo client init failed: ${e && e.message ? e.message : e}`),
            ref.source,
          ));
          continue;
        }
        try {
          await resolveForgejoRef(ref, forgejo, findings);
        } catch (e) {
          if (e && e._rateLimit) {
            forgejoShortCircuited = true;
            findings.push(finding(
              'warning', 'XREF_RESOLUTION_SKIPPED', undefined,
              'forgejo external refs skipped: Forgejo rate-limited (remaining forgejo refs not resolved this run)',
              'xref',
            ));
            continue;
          }
          findings.push(finding(
            'warning', 'XREF_RESOLUTION_SKIPPED', code,
            locatorDetail(ref, `forgejo resolution error: ${e && e.message ? e.message : e}`),
            ref.source,
          ));
        }
        continue;
