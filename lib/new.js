...
import { toEngineUsage } from './build.js';
...
function kickoffUsageFields(usage) {
  const engineUsage = toEngineUsage(usage);
  if (!engineUsage) return {};

  // Installed Stratum surface 21 accepts only usage here. Re-enable usdSource/split
  // after the dependency ships surface 22 (STRAT-STEPDONE-PROVENANCE-1).
  return { usage: engineUsage };
}
  ...
  try {
    ...
    while (!['completed', 'failed', 'budget_exhausted'].includes(response.status)) {
      if (response.status === 'ready') {
        ...
        const { result, usage } = await runAndNormalize(null, prompt, dispatch, { stratum, cwd });
