  ...
  try {
    ...
    while (response.status !== 'complete' && response.status !== 'killed') {
      ...
      if (response.status === 'execute_step') {
        ...
        // COMP-PLAN-GATE-LOOP: round-aware gate id so a revise re-entry mints a
        // fresh gate instead of reusing the prior resolved one.
        const gateId = await visionWriter.createGate(flowId, stepId, itemId, { round: readFlowRound(flowId) });
