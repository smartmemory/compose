...
export class FixChainDetector {
  ...
  static fromJSON(json) {
    ...
    const looksLegacy = (
      ('iteration' in json || 'fileHits' in json) &&
      !Object.values(json).some(v => v && typeof v === 'object' && ('iteration' in v || 'fileHits' in v))
    );
    if (looksLegacy) {
      const slot = d._slot(LEGACY_KEY);
...
export class AttemptCounter {
  ...
  static fromJSON(json) {
    ...
    const looksLegacy = (
      ('count' in json || 'isVisual' in json) &&
      !Object.values(json).some(v => v && typeof v === 'object' && ('count' in v || 'isVisual' in v))
    );
    if (looksLegacy) {
      const slot = c._slot(LEGACY_KEY);
