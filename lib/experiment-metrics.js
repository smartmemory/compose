...
export function collect({ sandbox, buildResult = {}, baselineSha = null }) {
  ...
  // The history record's own cost, or NULL. There is no token-derived fallback:
  // COMP-COST-OWNER S3 removed it as unreachable, and the trace is worth keeping because
  // it was cited as a live 2.58x overcharge before anyone checked who could call it.
  // `cost_usd` is written only by the two appendBuildHistory sites (lib/build.js:3336,
  // :6438), both spreading buildCostSnapshot(), whose amount is the accumulator's `usd` --
  // always a finite number, since S1 puts unpriced spend in `usd_unknown_count` rather than
  // in the amount. So this branch was never taken with a record present. And with NO record
  // present, tokensIn/tokensOut above read from that same absent record via `?? 0`, so the
  // fallback was only ever called as deriveUsd(model, 0, 0) -- it could not return anything
  // but 0 on any input. Evidence: docs/features/COMP-COST-OWNER/evidence/
  // s3-reachability-probes-2026-09-12.md. `usd: null` is now the honest answer, and
  // `usd_unknown_count` on the history record is where an incomplete cost announces itself.
  const usd = typeof historyRecord?.cost_usd === 'number' ? historyRecord.cost_usd : null;
