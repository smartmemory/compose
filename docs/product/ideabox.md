...
#### IDEA-27 — Conflict-first merge instead of last-writer-wins
**Status:** PROMOTED | **Priority:** P0 | **Tags:** integrity concurrency roadmap merge research-influence
**Source:** Semantica (semantica-agi/semantica) teardown, 2026-08-08
...
#### IDEA-28 — Bi-temporal judgment records (valid time vs transaction time)
**Status:** PROMOTED | **Priority:** P1 | **Tags:** judgment provenance temporal decisions research-influence
**Source:** Semantica (semantica-agi/semantica) v0.4.0 teardown, 2026-08-08
**Idea:** Semantica stamps every entity, relation, decision and provenance record with TWO timestamps: valid time (when the fact was true in the world) and transaction time (when the system learned it). This is the standard database answer to the problem the judgment layer already has — amending a position overwrites the belief, so the ledger is the only history and point-in-time reconstruction means replaying it. Two columns on judgment positions/goals/situations gives 'what do we believe now' and 'what did we believe when we made that call' as direct queries, and makes amendments non-destructive by construction. Sibling to IDEA-6 (append-only DECISIONS.md with supersession-by-ID): IDEA-6 is the append-only shape, bi-temporality is the query model that makes the append-only log answerable without a replay. Cheap to retrofit — it is a schema addition to the judgment store, not an architecture change. Prerequisite for IDEA-29 (precedent search), which needs to know what was believed AT decision time, not now.
**Promoted to:** COMP-JUDGMENT-BITEMPORAL

#### IDEA-29 — Precedent search over judgment records
**Status:** PROMOTED | **Priority:** P1 | **Tags:** judgment decisions recall smartmemory research-influence
**Source:** Semantica (semantica-agi/semantica) teardown, 2026-08-08
**Idea:** Semantica exposes decisions as first-class objects with three READ operations: trace_decision_chain (causal ancestry of a decision), find_similar_decisions (precedent search), and impact analysis (what depends on this decision). Compose's judgment layer is strong on the write side — positions, amendments, ledger, goal/situation/person stores all shipped — but has no similarity-retrieval surface. This is the operation that makes recording pay for itself: you do not write judgment records to have them, you write them so the NEXT similar call retrieves the precedent unprompted. Two slices: (a) trace — walk the ledger to reconstruct the causal chain behind a position, mechanical, no embeddings needed; (b) precedent — semantic similarity over position statements, the natural bridge to the SmartMemory coupling (project_comp_smartmemory) since recall is exactly what SmartMemory is for. Slice (a) is worth doing standalone even if (b) never lands. Depends on IDEA-28 for correctness: a precedent is only useful if you can see what was believed AT the time it was decided, not what is believed now. NOTE: SmartMemory recall is currently unreliable (project_livefire_recall_broken, 19/20 drop) — do not build slice (b) on it until that is root-caused.
**Promoted to:** COMP-JUDGMENT-PRECEDENT

#### IDEA-30 — W3C PROV-O as the artifact-lineage vocabulary
**Status:** PROMOTED | **Priority:** P2 | **Tags:** provenance lineage staleness artifacts standards research-influence
**Source:** Semantica (semantica-agi/semantica) teardown, 2026-08-08
