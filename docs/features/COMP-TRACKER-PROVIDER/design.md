...
| `build.js` triage path — creates missing `feature.json` with `status: 'PLANNED'` (`build.js:644`), then caches profile metadata (`build.js:653`) | mixed: an *initial create* (null→PLANNED) + later metadata-only updates. Today this create writes **only** `feature.json` — no event, no roadmap regen. | The create goes through a distinct **`createFeature(code, obj)`** op that is **side-effect-free by default**: it persists the record with its initial status (allowed once, on a non-existent feature; no transition-policy check — there is no prior state) and emits **no genesis event and no roadmap regen**, exactly matching today's `build.js` triage behavior (preserves byte-identical local). Genesis-event + roadmap-regen remain where they are today: the explicit MCP scaffold / `addRoadmapEntry` path, not bare creation. Subsequent profile-metadata writes use `putFeature` (metadata-only, status-delta rejected). |
| `build.js` lifecycle status flips (`build.js:755,1834,1845,1854`) | **real status transitions** done via direct `writeFeature`, bypassing transition policy/events/roadmap | **Intentionally use `persistFeatureRaw` under ALL providers** (T9 decision, commit 0b9836c). These are internal build-orchestration flips, not user-facing canonical transitions — they must not emit events or regenerate the roadmap mid-build (would break byte-identical teardown and add latency). The **user-facing canonical transition path** is the `set_feature_status` MCP tool → `setFeatureStatus`, which (after FIX B) routes events through `provider.appendEvent` and mirrors Projects v2 under GitHub. This closes the bypass for MCP-initiated transitions while preserving raw semantics for internal build coordination. |
...
createFeature(code, obj)                 side-effect-free create; initial status allowed once on a
                                         non-existent feature; NO genesis event, NO roadmap regen,
                                         no transition-policy check (matches today's triage create)
