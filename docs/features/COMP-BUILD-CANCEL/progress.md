...
- 2026-09-10: full suite (node 6606 + ui 624 + tracker 100): 4 failures. 2 tests outdated by contract (S04-3 exit 1 on refusal; C28 unknown flow writes nothing) → updated. 2 exposed an S05 over-reach: resume probe ran on locally terminal records and treated an unknown flow as over with a live pid → fixed in lib/build.js (pre-S05 conditions restored, cancelled check kept). Re-run of the 11 affected/cancel files 185/185; ui + tracker green. docs/cli.md updated.
- 2026-09-10: pushed compose main (5144e95..448c47e); published compose 0.5.0 + compose-mcp 0.5.0 (tags v0.5.0, compose-mcp-v0.5.0). Stratum fast-exit race fixed + released as stratum 0.5.1 @488f73b (tag v0.5.1).
