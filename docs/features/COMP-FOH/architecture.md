...
- **Adopted: one SmartMemory workspace per product**, keyed by the *already-shipped* `resolveProjectTag()`/`deriveId()` (`lib/smartmemory-config.js:37-39`, `lib/discover-workspaces.js`), scoped via `X-Workspace-Id` — the same header pattern Maya's `self_scoped_adapter()` already uses (`maya_self/store.py:98-117`). This is what `SmartMemoryFluidProvider` does per-instance: real container isolation, zero new identity scheme.
