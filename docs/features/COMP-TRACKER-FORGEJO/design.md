...
4. **Acceptance write-back (net-new, requested by owner):** promotion, under `--apply`, performs a one-time write to the issue at acceptance time: additively apply the `Roadmap Tracked` label (never removing any existing label) and post a comment naming the assigned feature code and pointing back at the roadmap.
...
1. ~~The label call shape is decided...~~ **RESOLVED 2026-09-19 (deployment verification):** confirmed live. `git.smartmemory.ai/api/v1/version` reports `16.0.3+gitea-1.22.0`, and its own served swagger (`/swagger.v1.json`) documents `POST /repos/{owner}/{repo}/issues/{index}/labels` (`operationId: issueAddLabel`) accepting `IssueLabelsOption` (`{labels: [...]}`, names or IDs) — matches `lib/tracker/forgejo-api.js`'s `addLabelResult` exactly. Also confirmed the transport's `Authorization: Bearer <token>` header is accepted identically to the swagger-documented `token <token>` scheme (both produced the same 401 body against a malformed token probe). One open sub-item, not previously called out: the `Roadmap Tracked` label (Open Question 2) does not yet exist on any of the three checked repos — Forgejo's additive-label-by-name endpoint has not been confirmed to auto-create an unknown label, so it should be created on each target repo before promotion runs there for real, rather than assumed.
2. ~~Acceptance label name.~~ **RESOLVED 2026-09-19 (owner):** use `Roadmap Tracked` (Title Case, matching house style). Confirmed against the live labels in `smartmemory/compose`, `smartmemory/stratum`, and `smartmemory/smart-memory`: all use Bug/Chore/Docs/Feature/Refactor/Security/Testing plus High/Medium/Low Priority, identically; no organization-level labels exist and no repository triage-label convention fits this role.
3. ~~Auth scope needed on the Forgejo PAT...~~ **RESOLVED 2026-09-19 (deployment verification):** this instance's scope grammar (from its own served swagger, `CreateAccessTokenOption.scopes`) is `read:<category>` / `write:<category>` / `all`, with no separate label scope — labels are covered by the `issue` category. The PAT needs **`write:issue`** (covers issue read/write, label read/add, comment read/post — everything `forgejo-api.js` calls) and **`read:repository`** (repo-level reads). The token can additionally be scoped to just the repos in play via `AccessToken.repositories`.
4. ~~Confirm the `PARKED → null` behavior.~~ **RESOLVED 2026-09-19 (owner):** PARKED freezes issue state (`expect` remains null and is never pushed) while expected labels continue to sync normally.
