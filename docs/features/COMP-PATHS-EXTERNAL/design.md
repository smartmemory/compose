...
- [ ] **Completion decoupled from commit (Decision 6b):** feature status flips to COMPLETE / completion is recorded even with no workspace-repo `sha` (cases ii/iii); the completion record's `commit` field is nullable.
