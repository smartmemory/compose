/**
 * mcp-tool-defs.js — the compose MCP tool definitions.
 *
 * Extracted from server/compose-mcp.js (COMP-COVERAGE-GATE slice 1) so the
 * declarations can be imported WITHOUT booting the stdio server: compose-mcp.js
 * connects a StdioServerTransport at module load, so importing it for `TOOLS`
 * hangs any consumer (lib/tool-inventory.js, its contract test, validate).
 *
 * Data only — no I/O, no imports. Each entry declares `effect` and, when
 * mutating, `writes` (canon-registry entry ids). See lib/tool-inventory.js.
 */
export const TOOLS = [
  {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        provider: { type: 'string', enum: ['github', 'forgejo', 'local', 'url', 'jira', 'linear', 'notion', 'obsidian'], description: 'Required when kind:"external". Resolvable: github|forgejo|local|url. Reserved url-class (require url, not resolved in v1): jira|linear|notion|obsidian.' },
        repo: { type: 'string', description: 'External github/forgejo: "owner/name". External local: workspace-relative repo token.' },
        issue: { type: 'integer', minimum: 1, description: 'External github/forgejo: issue/PR number.' },
        url: { type: 'string', description: 'External url-class (url|jira|linear|notion|obsidian): the pointer URL.' },
        expect: { type: 'string', description: 'Optional expected state. github/forgejo: open|closed. local: a status token. url-class: recorded, never resolved.' },
        push: { type: 'boolean', description: 'Opt this external link into roadmap_xref_push. Default false.' },
        expect_labels: { type: 'array', items: { type: 'string', minLength: 1 }, description: 'Optional labels to add without removing existing labels. Supported by github and forgejo links.' },
        derive_expect: { type: 'boolean', description: 'For github/forgejo, derive open|closed from the owning feature status at validation and push time instead of storing expect.' },
        note: { type: 'string' },
        force: { type: 'boolean' },
        idempotency_key: { type: 'string' },
      },
