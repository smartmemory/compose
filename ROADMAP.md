...
| # | Feature | Description | Status |
|---|---------|-------------|--------|
| — | COMP-BUILD-SPLIT-1 | Split lib/build.js (7845 lines) into cohesive modules so the build orchestrator fits in an agent context window and concurrent briefs can touch disjoint files | PLANNED |
...
| — | COMP-RESUME-CHECKPOINT-1 | Checkpoint the build flow at the post-merge pre-triage boundary so a failed review can be recovered by revert without losing completed implementation work | PLANNED |
...
| 7 | COMP-TRACKER-FORGEJO | Extend COMP-ROADMAP-XREF-PUSH to Forgejo: transport (forgejo-api.js), read-time lifecycle→issue-state derivation (derive_expect), 4-surface provider registration, and a promotion command (issue → feature.json with acceptance write-back). Design/blueprint/plan done (5 Codex review rounds); implementation starting. | COMPLETE |
