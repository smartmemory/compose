...
| 1 | COMP-PATHS-EXTERNAL | **Relocatable artifact paths.** ROADMAP + features (+ journal/context/ideabox) can live outside the workspace root, e.g. a shared `smart-memory-docs` repo. `.compose/` (config + state) stays at the root; only artifacts redirect. Centralizes resolution: one default table + a pure `resolvePathValue` (uses `path.resolve`, not `join`, so absolute/`../`-escaping overrides work) and sweeps ~25 hardcoded/`join`-based sites. Adds `paths.roadmap`. Byte-identical when keys unset. Sliced S1 (resolver) → S2 (sweep + `featuresDir` API migration; core ask) → S3 (ship/completion/enforcement git-awareness). v1 writes external artifacts + logs "commit it there" but does NOT auto-commit a separate repo. Codex design-gate applied. See `docs/features/COMP-PATHS-EXTERNAL/design.md`. | PLANNED |
| — | COMP-PATHS-EXTERNAL-1 | **Cross-repo auto-commit of external artifacts.** Fast-follow to COMP-PATHS-EXTERNAL: when ROADMAP/features resolve into a separate git repo, teach the build ship step to commit that repo via `git -C <external>`, staging only Compose-owned paths (never blanket `git add -A`) to spare unrelated dirty files. | PLANNED |
...
| 7 | COMP-COCKPIT-11 | ChallengeModal "Discuss" posts to /api/terminal/inject (ChallengeModal.jsx:38 via agentServerUrl) but no such route exists on ANY server — dead endpoint, 404s on localhost too (client-side counterpart of the COMP-COCKPIT-10 orphaned-route sweep). Found by the COMP-MOBILE-REMOTE review gate (flagged as a missing remote proxy; root cause is the endpoint never existing). Fix: wire a real inject path on the agent server + proxy it, or remove/disable the Discuss action. | PLANNED |
