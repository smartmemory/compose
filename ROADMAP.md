...
| — | COMP-PROV-LINEAGE | Adopt the W3C PROV-O vocabulary for artifact lineage rather than inventing a _sources schema: the Entity/Activity/Agent triad plus wasDerivedFrom / wasGeneratedBy / used already models "this artifact came from those artifacts via that phase, run by that agent". Borrow the vocabulary, not the RDF stack — name the fields after PROV-O terms in existing artifact records so staleness becomes a standard graph-reachability query and a graph export stays possible later. General form of the per-artifact-type staleness tracking already filed. Read the spec before designing anything custom. Anchor: docs/features/COMP-PROV-LINEAGE/plan.md | PLANNED |
...
| — | COMP-CONFLICT-MERGE | Conflict-first merge instead of last-writer-wins: before a write supersedes an existing record, diff the incoming value against the stored one on fields the writer does not own; on divergence emit a typed conflict and halt rather than overwrite. Targets two live data-loss paths — roadmap generate clobbering hand-authored prose, and the shared build-stream / active-build last-writer-wins race. First slice is the roadmap/prose writer (single-threaded, needs no Umbrella B concurrency primitives). Distinct from COMP-* concurrency work, which prevents the race; this makes the merge honest when the race is already lost. Anchor: docs/features/COMP-CONFLICT-MERGE/plan.md | PLANNED |
...
| — | COMP-JUDGMENT-BITEMPORAL | Bi-temporal judgment records: stamp positions/goals/situations with valid time (when true in the world) and transaction time (when recorded), so "what do we believe now" and "what did we believe when we made that call" are direct queries rather than a ledger replay. Makes amendment non-destructive by construction. Sibling to the append-only decisions-log shape; the query model that makes an append-only log answerable. Cheap now while the judgment store schema is young; cost rises with every record written. Anchor: docs/features/COMP-JUDGMENT-BITEMPORAL/plan.md | PLANNED |
| — | COMP-JUDGMENT-PRECEDENT | Precedent search over judgment records — the read side that makes recording pay for itself. Slice A: trace the causal chain behind a position by walking the ledger (mechanical, no embeddings, unblocked). Slice B: semantic similarity over position statements to surface precedent for a new call, the natural bridge to the SmartMemory coupling. BLOCKED for slice B until the SmartMemory recall failure (19/20 drop) is root-caused. Depends on COMP-JUDGMENT-BITEMPORAL: a precedent is only useful if you can see what was believed AT decision time. Anchor: docs/features/COMP-JUDGMENT-PRECEDENT/plan.md | PLANNED |
