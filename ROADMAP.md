...
| 16 | COMP-MCP-FOLLOWUP-1-1 | Eager-import lib/followup-writer.js at compose MCP startup (instead of lazily on first propose_followup call) so a genuine module-skew / missing-export break fails fast at server boot rather than surfacing only when a user first calls the tool. | PLANNED |
...
| 138 | COMP-PIPE-EDIT-3 | Dependency wiring: Drag from one node's output port to another's input port to create `depends_on` edges. Visual feedback for invalid connections (cycles, missing refs). Auto-layout via dagre/elk. | PLANNED |
| 139 | COMP-PIPE-EDIT-4 | Contract editor: Define and edit contracts (LensFinding, ReviewResult, etc.) in a schema form. Contracts available as dropdowns in step inspector. New contracts auto-added to the spec. | PLANNED |
...
| 142 | COMP-PIPE-EDIT-7 | Template save: Save the current canvas as a new pipeline template in `pipelines/`. Templates appear in the existing TemplateSelector for future builds. | PLANNED |
...
| 300 | STRAT-HERMES-CONNECTOR | Add a Nous Hermes Agent backend to Stratum so pipelines can dispatch `agent: hermes` like `claude`/`codex`. New `HermesConnector(AgentConnector)` in stratum-mcp connectors/, modeled on the opencode.py template (spawn `hermes -z` + translate). Tier A = text-return (reviewer / executor without live narration); Tier B = streaming envelope events, gated on an unverified Hermes structured event stream. DEPENDS ON T2-F5-OPENCODE-DISPATCH — the connector factory hard-restricts to {claude, codex}, so the multi-connector dispatch path must be finished first (opencode is written but likewise gated). Small compose follow-on: allow agent:hermes + tier/caps wiring. Stratum-owned code, tracked here in the unified roadmap. | PLANNED |
| 301 | COMP-CODEX-IMPL | Let Codex be the implementation agent (instead of all-Opus), keeping cross-model review. Codex is already a dispatchable backend, so a `build-codex.stratum.yaml` variant (or `--agent codex` flag, mirroring `--quick`) flips implementation steps to `agent: codex`. Must pair Codex-implements with **Claude**-reviews (not codex reviewing its own work). Frictions: role-aware fix-routing (build.js:3124 codex→claude swap would hijack codex fixes), codex Seatbelt vs the execute step's worktree isolation (spike — start non-parallel), Claude-centric caps/tool enforcement, tier→model mapping. All compose-side; no Stratum work. Shipped 2026-06-19 (v1): single-spec role interpolation via STRAT-AGENT-INTERP (implementer_agent/reviewer_agent flow inputs), `--codex` flag, role-aware fix routing (fixer=implementer), Codex self-review suppression, resume-durable roles, preflight Codex-in-worktree probe with fail-fast rollback. Caps/cert needed no change. 18 tests; design 3 + impl 4 Codex rounds CLEAN. | COMPLETE |
| 301a | COMP-CODEX-IMPL-SPIKE | Follow-up to COMP-CODEX-IMPL: a Codex-specific `isolation: none` execution path so `--codex` works in environments where the preflight worktree probe fails (Codex Seatbelt can't write inside a detached git worktree). v1 ships the probe + fail-fast abort; this adds the fallback. `isolation` is not interpolatable (STRAT-AGENT-INTERP covers only `agent`), so this needs either a producer-side change or a `build-codex` execute variant. Also bundle: `build-quick` Codex parity, `lib/new.js` swap role-awareness, a Codex/gpt tier→model map. Owner: Compose. | PLANNED |
| 302 | STRAT-AGENT-INTERP | Make Stratum's per-step `agent:` field interpolatable so a flow can select its executor at runtime. Today `Step.agent` is a literal emitted verbatim by the executor; `inputs` are rendered (`$.input.*`/`$.steps.*.output.*`) but the agent field is not. Resolve it through the same resolver before `make_agent_connector`, so `agent: "$.steps.route.output.agent"` (a router step emitting the agent for a later step) works. Stays in the data plane — the resolved value comes from recorded flow state, so audit/resume/result-cache replay identically; no control-plane hook or non-deterministic external actor. Backward-compatible (literal agents unchanged; resolved value must validate to a known agent type). The data-plane alternative to a mid-flow orchestrator override and a natural enabler for COMP-CODEX-IMPL. Owner: Stratum; compose consumes transparently. Shipped 2026-06-19 (stratum 4651933): resolve_agent/effective_agent, connector-prefix validation, conditional cache-key fold, 18 tests. | COMPLETE |
