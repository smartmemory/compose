...
| 154 | COMP-MODEL-ROUTE-1 | Root-cause the review_triage "Usage credits required for 1M context" failure (flow 05f660fe): missing-model, prompt-size, and model-eligibility hypotheses all refuted by controls; mechanism unproven; decisive test is a real review_triage run. | PLANNED |
...
| 305 | COMP-OUTCOME-ENUM-1 | Agents return outcome success/done/approved/pass where the PhaseResult contract allows only complete|skipped|failed, forcing a full extra dispatch per occurrence (measured on host B and host C in COMP-HOST-PORTABILITY-1; 23 occurrences across stratum+compose via learn harvest). Decide: fix the step instructions/schema vocabulary, or normalize equivalent completion words before the costly retry. Check the steps first; do not loosen the contract blind. Gap G6. | PLANNED |
| 306 | COMP-SUPERVISOR-OWNER-1 | compose start SIGTERMs whatever supervisor the machine-wide PID file names (server/supervisor.js:80-93 killExistingSupervisor) before binding :4001, so starting the app from any project replaces another project's running server. Took down the owner's server during COMP-HOST-PORTABILITY-1. Verify ownership (same target project) and require explicit takeover, or scope supervisor identity per project. Gap G8. | PLANNED |
| 307 | COMP-CLI-HONEST-EXIT-1 | Four standalone CLI commands exit 0 while doing less with no error-level signal (COMP-HOST-PORTABILITY-1 host C SD01-04): roadmap prints nothing for a populated roadmap; experiment reports complete with nCompleted 0/1; remote status omits server health when COMPOSE_API_TOKEN is unset; migrate-anon --non-interactive changes nothing and returns 0. Each must render its result or report the incomplete state distinctly (message + exit code). Gap G7. | PLANNED |
| 308 | COMP-VERIFY-POLICY-1 | Build triage silently downgrades a feature's requested verification: feature.json needs_verification true became false at runtime and Stratum recorded the step skipped with no warning (COMP-HOST-PORTABILITY-1 SD08, host B). Preserve explicit feature requirements, or require a visible recorded override before skipping verification. Gap G9. | PLANNED |
| 309 | COMP-EXPLORER-EVIDENCE-1 | The design phase prompt requires 2-3 explorer subagents but the accepted design ran one or none on both hosts (COMP-HOST-PORTABILITY-1 SD05) with no cardinality check or warning. Specify the required exploration evidence in the PhaseResult, validate it before accepting the phase, and make the subagent dispatch host-portable instead of Claude Agent/subagent_type semantics. Gap G10. | PLANNED |
