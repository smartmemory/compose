...
**Last updated:** 2026-05-17 (filed `COMP-MCP-XREF-SCHEMA` #15 + `COMP-MCP-XREF-VALIDATE` #16 + follow-on resolvers `COMP-MCP-XREF-JIRA` #17 + `COMP-MCP-XREF-LINEAR` #18 under `COMP-MCP-FEATURE-MGMT` — cross-project external references + read-only staleness resolution, realizing the per-project-provider roadmap model decided in forge-top `ROADMAP.md`. Spec+plan Codex-reviewed; #16 adds `getIssueResult()` to `github-api.js` since `getIssue()` discards HTTP status.)
...
| 15 | COMP-MCP-XREF-SCHEMA | **Cross-project external reference type.** Add an external-ref shape to `feature.json` `links[]` — `kind: external`, `provider` (`github`/`local`), `repo`, `issue`/`url` — distinct from the same-project `to_code` resolution every existing `kind` assumes. Schema (`feature-json.schema.json`) + `feature-writer.js` validation + `link_features` MCP tool acceptance of the external shape. No sync engine, no network calls on write. Enables forge-top (and any prose-owned roadmap) to cite a product-repo issue/PR without embedding or mirroring its status. Pairs with `COMP-WORKSPACE-ID` for canonical repo identity. | PLANNED |
| 16 | COMP-MCP-XREF-VALIDATE | **External-reference staleness resolution in `validate_project`.** Builds on `COMP-MCP-VALIDATE` (#7, COMPLETE). New finding kind: resolve `kind: external` links read-only (issue exists, not deleted, open/closed state not blatantly contradicting the citing row's status). GitHub resolution via a new read-only `github-api.js` `getIssueResult()` (status-returning sibling of `getIssue()`, modeled on `getRepo()`); local via feature.json read. Surfaced in the pre-push hook + a CI-friendly `--external` flag; emits drift, **never writes back**. Depends on `COMP-MCP-XREF-SCHEMA`. | PLANNED |
| 17 | COMP-MCP-XREF-JIRA | **Jira external-ref resolver.** Graduates the reserved `provider:jira` (parse-valid url-class from #15) to a real read-only resolver: new `jira-api.js` (REST + token, injectable transport), issue-state drift rule mirroring the github path in `runExternalRefChecks`, own degrade matrix (no-token/offline/rate-limit ⇒ `XREF_RESOLUTION_SKIPPED` warning; 404 ⇒ `XREF_TARGET_MISSING`). No write-back. No schema change (provider enum already includes `jira`). Depends on #16. | PLANNED |
| 18 | COMP-MCP-XREF-LINEAR | **Linear external-ref resolver.** As #17 but `linear-api.js` over Linear's GraphQL API; reuses the `graphql()` pattern from `github-api.js`. Graduates reserved `provider:linear`. `notion`/`obsidian` remain permanently url-class by design (no canonical open/closed state — spec §9). Depends on #16. | PLANNED |
