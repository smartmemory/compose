...
| 4 | COMP-ROADMAP-XREF-PUSH | External-write counterpart to COMP-ROADMAP-XREF-SYNC's Pull: write GitHub trackers to match the local `expect=` declared intent. Dry-run by default, per-ref `push:true` opt-in, `--apply` to mutate, degrade-skip on offline/no-token/404. github provider only in v1. | IN_PROGRESS |
