...
| 305 | COMP-HOST-PORTABILITY-1 | Host portability audit. Measure what Stratum + Compose actually do under three hosts: Claude Code (baseline), a Codex CLI host, and no host at all (`compose <cmd>` from a shell). Stratum's connector layer already has claude.ts and codex.ts as peers; Compose ships only lib/local-claude-connector.js and `compose setup` installs subagent defs to ~/.claude/agents/, so the lifecycle assumes Claude Code while 30+ CLI commands look host-independent. Headline deliverable is the silent-degradation inventory — commands that exit 0 on a non-Claude host while doing less. Audit only; remediations spawn as separate features. | PLANNED |
