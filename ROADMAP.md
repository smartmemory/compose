...
| 7 | COMP-TRACKER-FORGEJO | Extend COMP-ROADMAP-XREF-PUSH to Forgejo: transport (forgejo-api.js), read-time lifecycle→issue-state derivation (derive_expect), 4-surface provider registration, and a promotion command (issue → feature.json with acceptance write-back). Design/blueprint/plan done (5 Codex review rounds); implementation starting. | IN_PROGRESS |
