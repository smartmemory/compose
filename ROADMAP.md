...
| # | Feature | Description | Status |
|---|---------|-------------|--------|
| — | COMP-MCP-ENFORCE | **Mechanical enforcement of lifecycle/gate guarantees.** Filed 2026-06-02. Compose's `/compose` guarantees (design-before-implement, review-until-CLEAN, full-suite-before-merge) live in the *skill prompt*, not the tools — drive the MCP/REST surface directly and none apply: gates are self-approved (`vision-routes.js:782` "server only updates gate state"), `force:true` bypasses both status policy and the roundtrip guard, two uncoordinated state machines drift (lifecycle phase `vision-routes.js:155-169` vs roadmap status `feature-writer.js:48-57`), completion accepts caller booleans (`tests_pass` defaults true). REST has no auth beyond localhost CORS. Fix: stop hand-rolling a weaker state machine; consume stratum's enforcement substrate (`STRAT-GUARD` + `stratum_judge`) as a client — compose declares the phase graph + per-edge evidence predicates, stratum enforces them. **Design-gate decisions (2026-06-02):** lifecycle-phase is the single source of truth (roadmap STATUS becomes a projection, `set_feature_status` derive-only); Slice 1 guards `approve_gate` **and** `advance`/`skip`/`complete` together (no half-closed bypass). **Slices:** (1) STRAT-GUARD ships + verdict-gate the gate & lifecycle endpoints; (2) collapse the two state machines — absorbs **COMP-PARITY-7**; (3) kill `force` → authorized override + evidence-bound completion (real test exit + git-commit digest, not caller booleans); (4) phase-scoped tool capabilities — absorbs **COMP-DEBUG-1**. Depends on stratum `STRAT-GUARD`. Codex design-reviewed, 8 findings fixed (trusted-evidence binding so caller-staged artifacts can't fabricate proofs, immutable re-registration, ledger hash-chain, full graph incl. ship→complete/kill edges). `docs/features/COMP-MCP-ENFORCE/design.md`. | PLANNED |
| — | COMP-DEBUG-1 | **Debug discipline: agent capability profiles + enforcement.** Limits each agent type to a tool subset; emits capability_violation events; opt-in block mode rejects calls outside the profile. **SUPERSEDED 2026-06-02 → merged into `COMP-MCP-ENFORCE` Slice 4** — phase-scoped tool capabilities is the same mechanism. | SUPERSEDED |
