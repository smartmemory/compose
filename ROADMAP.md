...
| 17 | COMP-MCP-ARTIFACT-LINKER-1 | `link_artifact` advertises artifact_type values the feature.json schema enum rejects; align tool description, validation, and schema. | PLANNED |
...
| 131 | COMP-TUI-4 | Parallel task grid: live 2-4 row grid during `parallel_dispatch` showing per-task status (spinner/done/failed) with agent and elapsed time. | COMPLETE |
...
## COMP-TEAMS: Agent Team Presets — PARTIAL (v1 complete; Fable-led Astra Teams planned)
...
| 151 | COMP-FABLE-ASTRA | Fable-led Astra Teams — a `team-fable-astra` preset for `compose build --team`: Fable (claude-fable-5-1) plans and assesses as ordinary steps, Astra workers (gpt-6-astra) implement in bounded waves in isolated worktrees, a fresh Astra reviewer checks the integrated result, and the gate revise edge drives a repair wave covering only affected tasks; waves are the dependency mechanism. Epic with Stratum prerequisites: STRAT-LOOP-CARRY (a declared, persisted flow variable updated on gate revise, since a fanout over a later step's output is a routing cycle and the revise reset deletes it) and STRAT-FLOW-CANCEL-FG (foreground flow cancel), plus four Compose seams (output-driven gate resolution with validators, wave-checkpoint branch with reconciliation, files_owned enforcement at merge from the engine-recorded item, per-item tier resolution so Fable picks critical/standard/fast per task and a wave mixes Astra with terra or spark). Provider stays per stage: the TS engine's agent field is a literal enum, so cross-provider per-item routing is a follow-up. Slice 1 bumps the Claude tier table and closes the fail-open sidecar. Design: [design.md](docs/features/COMP-FABLE-ASTRA/design.md). **Status 2026-09-13: slices 1, 3 and 4 are SHIPPED and RELEASED** — slice 3 at 53e3710 (real-engine wave golden 6/6), slice 4 at ce3069e (the `team-fable-astra` preset, sidecar, named contracts and `--team fable-astra`), released 2026-09-10 as stratum 0.5.2 (surface 20) and compose + compose-mcp 0.5.1, both tagged and pushed. Remaining: slice 6 live-fire, which the owner HELD — it is waiting on a spend decision, not on engineering. Ledger: [progress.md](docs/features/COMP-FABLE-ASTRA/progress.md). | PARTIAL |
| 152 | COMP-BUILD-CANCEL | Compose adoption of Stratum 0.5.0 cancellation (COMP-FABLE-ASTRA D5 compose half, plus the D1 consumer tagging it needs): every consumer `stratum_agent_run` carries `flow:{runId,stepId,itemIndex}` and a `cancellationId` (client version guard bumped to surface 19), `compose build --abort` and the SIGINT/SIGTERM handler call `stratum_flow_cancel` before the existing kill/active-build/actuals writes, Compose kills its own in-process isolation:none agents, and no patch captured after a cancel is merged. Compose moves to 0.5.0 with `@smartmemory/stratum ^0.5.0` (shared minor). Preset carry/verify ordering stays in the COMP-FABLE-ASTRA preset slice. Design: [design.md](docs/features/COMP-FABLE-ASTRA/design.md) §D5. | COMPLETE |
| 153 | COMP-MODEL-ROUTE | Dynamic model routing framework for agent dispatches. Every pipeline dispatch (plan, execute items, verify, review, assess) resolves its provider tier through ONE resolver over a dispatch key (preset, step, provider, planner tier for fanout items). Each key's value comes from an ordered chain of sources: manual override, learned value (only where the preset opts the key in and cross-run evidence clears a breadth bar), preset, default; the winning source and reason are recorded per dispatch as flow state before dispatch so resume/replay see the same choice. The learned source is an outcome-based policy over STRAT-LEARN-COST receipts joined with downstream acceptance from flow state (epoch>0 / repair = not accepted), keyed by contract fingerprint, evidence recomputed from an append-only ledger, breadth-not-volume durability per STRAT-TS-LEARN. Not failure-based escalation. Ladder: S1 resolver + ledger + shadow decisions on all steps; S2 hindsight report fed into Fable's planning prompt (absorbs COMP-FABLE-CALIBRATE); S3 active learned routing on plan + execute only, bounded exploration, never below a repaired task's tier, never on ship-deciding gates, default-off per preset. Provider axis stays static until STRAT-AGENT-INTERP-TS. Design: [design.md](docs/features/COMP-MODEL-ROUTE/design.md). | PARTIAL |
| 154 | COMP-MODEL-ROUTE-1 | Root-cause the review_triage "Usage credits required for 1M context" failure (flow 05f660fe). Cause: an explicit allowedTools omitting ToolSearch disables MCP tool-schema deferral, inlining every schema on the post-connect turn (511,214 cache-creation tokens for a one-line echo) and pushing sonnet-4-6 onto the 1M tier. Fixed in stratum f250d9e. | COMPLETE |
| 155 | COMP-MODEL-ROUTE-2 | Re-price or exclude dispatch-ledger rows since stratum 603ec78, whose Claude turns carry ~500K cache-creation tokens caused by the ToolSearch deferral defect (fixed in stratum f250d9e) | PLANNED |
...
| 290 | COMP-CTXBUDGET-2 | **Context budget as a tracked recurring cost, not a one-shot snapshot.** COMP-CTXBUDGET-1/-1-2 shipped a manually-invoked audit that reports live-startup tokens for the current moment; it cannot tell you the surface grew since the last check. Three deltas on `lib/context-budget.js`: (a) **persist a baseline series** — append each run's live-startup total plus per-component breakdown to JSONL so the skill reports drift against the 2026-06-06 forge baseline (~107.8K live / ~55.5K reclaimable) instead of a bare snapshot; (b) **run unattended** — pre-push hook or cron against a configurable live-startup ceiling, so context debt surfaces before sessions feel heavy rather than when someone thinks to ask; (c) **rank reclaims by recurring cost** — tokens/session x sessions/week rather than bytes-on-disk, which materially reorders the cut list under progressive disclosure (a 400-line rarely-invoked skill is near-free at frontmatter-only load; 200 lines of always-loaded `~/.claude/rules/` is not). Secondary scope to split if it grows: **adoption auditing** — measure whether an optimization actually fires, not just whether it exists. Source case study found a trusted token-saving tool reached by only 6% of eligible calls over 30 days (tool fine, trigger broken); we have no instrumentation on whether our own hooks, gates, and guards fire at assumed rates. Read-only and advisory like -1: never auto-applies cuts. Promoted from IDEA-17 (`zwolf25/tokenminning` scan 2026-08-08). | PLANNED |
...
| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 272 | COMP-POLICY-CHECK-1 | Pattern-catalog client. Fetch SmartMemory's violation-pattern catalog via `mcp__smartmemory__memory_get_violation_patterns` once per session; cache in-memory; refresh on rule-change signal. | COMPLETE |
| 273 | COMP-POLICY-CHECK-2 | User-mode classifier. Scan recent user turn(s) for suppression-signal patterns from the SmartMemory catalog. Output user_mode ∈ {AUTONOMOUS, PACED, SKILL_GATED}. SKILL_GATED set when an active skill (e.g. Compose's plan/blueprint gate) declares deliberate-permission-asking. | COMPLETE |
| 274 | COMP-POLICY-CHECK-3 | Response scanner. Scan candidate agent responses against pattern catalog. For matched rules, evaluate suppression_signals against user_mode. Emit (rule, match, suppressed?) records. | COMPLETE |
| 275 | COMP-POLICY-CHECK-4 | Revision prompt. When unsuppressed violations detected, surface to agent: *"Your draft contains pattern X for rule Y; revise unless precedence applies."* Agent revises; check re-runs once. Does NOT hard-block. | COMPLETE |
| 276 | COMP-POLICY-CHECK-5 | Session trace. Log all pattern matches (flagged + suppressed) to the session trace for post-hoc analysis by SmartMemory's DIST-CC-INGEST-1 ensemble. Closes the measurement loop. | COMPLETE |
| 277 | COMP-POLICY-CHECK-6 | Stratum step postcondition. Stratum specs declare `ensure: ['result.unsuppressed_violations == 0']` — the policy check exposes `unsuppressed_violations` in the step result (contract-gated: attached when the step's out contract declares it or has no contract). Corrected 2026-08-17: the originally-proposed `compose.policy.*` ensure namespace does not exist; ensure is evaluated by the external Stratum engine against `result.<field>` only. | COMPLETE |
...
| 5 | COMP-LIFECYCLE-BACKFILL | **Backfill edge for out-of-band completions.** The guard enforces two claims through one gate: "did this really ship" (server-verified commit + attested tests — reality) and "did this follow the process" (server_file_exists on design/blueprint/plan — procedure). For out-of-band work the first is checkable and the second is false by definition, so failing it blocks recording a true completion — the guard forces the roadmap to lie. Already cost us once: COMP-MCP-ENFORCE-XREPO records that the STRAT-AGENT-INTERP backfill "had to set status COMPLETE via direct feature.json edit". Adds a `complete_backfilled` terminal state (a distinct node, so it carries its own edge predicates — the guard keys predicates by from->to alone) reachable from any non-terminal phase, gated on UNWEAKENED reality evidence plus a mandatory written reason, with attested backfill of intermediate phases so partial walks become representable. Phase history gains a transaction-time axis (`recordedAt`/`origin`/`confidence`, vocabulary borrowed from SmartMemory's MemoryItem) since `enteredAt`/`exitedAt` already encode valid time. Design COMPLETE @919c02e after two Codex gates, nine findings, all confirmed. **BLOCKED on stratum:** guardMigrate (transition.ts:565,592) requires STRATUM_GUARD_OVERRIDE_TOKEN and unconditionally bumps graph_version, so migrating the 31 already-registered guard resources would make this feature depend on the emergency token it exists to replace. Owner declined a Compose-only v1 covering the ~319 unregistered features (of 350) because a completion path that silently fails on the oldest 31 reintroduces the defect class being removed. Order: stratum idempotent non-emergency migrate first, then this with no carve-out. Three Compose-side pieces still to redo before implement: phaseOrder is not a valid temporal insertion order (backward loops verification->blueprint, test->fix), cross-mutation recovery between the guard and vision-state, and reader changes. **RECONCILED 2026-09-05 — stratum blocker CLOSED.** The 2026-08-17 re-diagnosis (compose c7cc848) found the real blocker was the transport, not guardMigrate: compose reaches the guard over the CLI, where the override token was caller-mintable. Stratum shipped STRAT-GUARD-AUTHZ @3647b4c the same day (option ii, signed sshsig authorization over a server-reconstructed payload; override token removed; trust root ships EMPTY) and STRAT-GUARD-UPGRADE @91a55ed (token-free idempotent per-resource upgrade; `terminal` frozen by design, so complete_backfilled still needs one signed migrate per resource). Nothing remains on stratum. Compose-side work to do: enrol a signing key in stratum's trust root, generate + review the upgrade descriptor file, then the three redo items (phaseOrder is not a temporal insertion order; guard<->vision-state cross-mutation recovery; reader changes), then implement. Status BLOCKED->PLANNED. | COMPLETE |
| 6 | COMP-LOOP-DETECT | **Retry-loop detection at the tool-call layer (PreToolUse warn, not block).** A PreToolUse hook that counts consecutive identical tool calls and, on the 3rd, returns `permissionDecision: "ask"` with a "possible retry loop, confirm to continue" reason. The load-bearing design choice is **warn, not block**: `ask` rather than exit-code-2 preserves legitimate repetition (polling a background job, watching a build) while still interrupting a blind retry loop, so it can ship enabled by default without curating an exception list. Sits one layer BELOW COMP-ITER-BUDGET, which caps *pipeline* iterations on `start_iteration_loop` with auto-abort and a structured failure report and therefore cannot see an agent burning 60 turns re-running one failing command inside a single step. Complementary to per-step `allowed_tools` gating (IDEA-7): that makes the wrong tool invisible, this catches the right tool used in circles. Targets a known, costed failure mode — `~/.claude/rules/subagent-model-routing.md` records the expensive outcome as a retry blowup (268 steps / 214K tokens for the same result a converging run reached in 95 / 86K), currently caught only by the controller happening to notice. Ships on the existing hook surface alongside `.claude/hooks/canon-guard.mjs`, installed by `compose setup` / `compose update`. Open scoping questions: what counts as "identical" (exact command string vs normalized vs same tool + same target file), whether the counter resets on any intervening different call or uses a sliding window, and whether the trip count varies per tool (3 is right for Edit, wrong for deliberate sleep-and-poll). Value is in tuning the identity predicate, not the hook. Promoted from IDEA-18 (`zwolf25/tokenminning` scan 2026-08-08, `loop-detect.sh`). | PLANNED |
| 7 | COMP-COVERAGE-GATE | **Mutating-tool authorization coverage as a validate_project check.** Compose keeps three hand-maintained lists describing which MCP tools mutate state (`TOOLS_FOR_*`/`JUDGMENT_WRITE_TOOLS` in `lib/canon-registry.js`; `IMPLEMENTER_DENY`/`REVIEWER_ALLOW` in `server/mcp-tool-policy.js`) and no list is the inventory — nothing enumerates every mutating tool or cross-checks the three against the 51 tool defs in `server/compose-mcp.js`. Same class of hole as COMP-COMPLETION-GATE (guard coverage found 0/321 by audit, not by a check). Prior art: PolicyGuide (arXiv:2608.19861) §3.3 Stage 6 makes the equivalent property a compile-time refusal. **Slice 1:** declare `effect: read\|mutating\|setup` + `writes: [canonPathKey]` on every tool def; derive `MUTATING_TOOLS` (new `lib/tool-inventory.js`, pure). **Slice 2:** `checkAuthorizationCoverage()` → four codes wired into `validate_project` — C1 MISSING_EFFECT (hard fail), C4 UNGATED_MUTATION (headline; tests `PROFILE_POLICY`, the list that is actually enforced at dispatch), C3 ORPHAN_REGISTRY_TOOL, C2 UNCOVERED_WRITE (minor). Findings carry a specific remediation, not a verdict. **Scope note (2026-08-24 trace):** the originating hunch was largely disconfirmed — `scaffold_feature` writes only the six markdown templates, not feature.json; `complete_feature`/`kill_feature` do write feature.json (`server/vision-routes.js:366,451`) without being in `TOOLS_FOR_FEATURE_JSON`, but `entry.tools` has exactly one consumer (`lib/canon-guard.js:128`, the deny-message string) and feature.json is `enforcedBy:['ship']`, so that list is help text, not a boundary. C2 demoted, C4 promoted accordingly. Pipeline-graph checks (unreachable steps, per-step `authorizes:`) split out and NOT in scope. Design: `docs/plans/2026-08-24-authorization-coverage-gate-design.md`. | COMPLETE |
| 8 | COMP-GUARD-ONE-TAP | One-tap guard authorization: compose generates/enrols the signing key, builds and signs upgrade descriptors on demand inside the gate; the human's only act is a system confirm dialog (ssh-agent confirm mode or security key). Deletes the six manual operator steps from COMP-LIFECYCLE-BACKFILL. Owner decision 2026-09-06: never an agent-held usable key. | COMPLETE |
...
| — | COMP-PROV-LINEAGE | Adopt the W3C PROV-O vocabulary for artifact lineage. VERIFIED gap: feature.json artifact entries carry only {type, path, status} (contracts/feature-json.schema.json) — no derivation edge, no upstream reference, and canonical artifacts are auto-discovered rather than registered, so there is no edge to hang staleness on at all. Borrow the vocabulary (Entity/Activity/Agent, wasDerivedFrom, wasGeneratedBy, used), not the RDF stack, so staleness becomes a reachability query and an export stays possible later. Read the spec before designing anything custom. Anchor: docs/features/COMP-PROV-LINEAGE/plan.md | COMPLETE |
...
| 14 | COMP-COMPLETION-GATE | **Route completions through the lifecycle guard.** Audit finding: the guard had never guarded a real feature — 321 managed features, 31 registered guard resources, ZERO overlap (all 31 are leaked test fixtures), 230 COMPLETE features none of which passed a guarded transition. `COMP-MCP-ENFORCE/report.md:8,52` claims "no caller can effect an unverified transition"; that has always been false (AC-14, still outstanding). Cause: the writers that actually complete features never call a guarded transition at all — registration was always possible (`guardedTransition` registers lazily), it simply never happened. Design rev 6 after five Codex rounds (8→8→7→7→7 findings, flat — the pre-registered stopping criterion fired, so the work was sliced instead of reviewed harder). Slice 1 SHIPPED @753ed47: `lib/completion-gate.js` gates `record_completion` (MCP + CLI) — evidence verification, write-ahead intent, guarded transition; `roadmap add --status COMPLETE` refused; `--tests-pass` default removed (BREAKING). Slice 2 SHIPPED @4625b2b: the build runner. Exactly one completion, at terminalization, AFTER the health gate — previously the terminal block wrote COMPLETE before the health gate that can fail the build, and ship had already written a second record. Tests hoisted above the non-git branch (which returned before running them and hard-coded tests_pass:true); `deriveTestsAttested` tri-state; accumulator v1→v2 with `tests_attested`/`evidence_root`. Slice 3 SHIPPED 2026-08-30: the gate owns the §2.3a write sequence (record → status via `persistFeatureRaw` → ROADMAP → vision → events; projection failures reported `partial:true`); `setFeatureStatus` refuses COMPLETE unconditionally; `recordCompletion` delegates its completing path to the gate (`COMPLETION_REFUSED`, nothing written); `server/completion-projection.js` is the self-verifying predicate (§2.3b) with four transports (REST endpoint, in-process `applyVerifiedProjection` for cockpit + reconciler, `VisionWriter.completeItem` direct, startup seeding) and three tiers stamped as `completion_projection.verified_by` (guarded / canonical-status-only / document-derived; only `not_found` means legacy, everything else fails closed); refusals on paths 5/7/8/14 keyed on MANAGED build items (bound + build + feature.json exists — the `compose new` kickoff item and fix/plan items keep their paths); cockpit `/lifecycle/complete` goes through the gate; `roadmap migrate` exemption named + logged on both branches (AC-17); AC-19 two-sided allowlist scan test; AC-14 report correction. Correct sentence now: build-mode completions of managed features are evidence-checked, ledgered and single-doored — NOT lifecycle-enforced. Deferred: paths 9 (GitHub half)/15 → COMP-COMPLETION-GATE-REMOTE; fix/plan modes → COMP-COMPLETION-GATE-MODES. | COMPLETE |
| 15 | COMP-GUARD-CLAIM-1 | **Correct COMP-MCP-ENFORCE's false guard-coverage claim (COMP-COMPLETION-GATE AC-14).** `docs/features/COMP-MCP-ENFORCE/report.md:8` states "No caller — skill, human cockpit, or rogue MCP/REST client — can effect a transition the guard refuses", and the Slice 3 section repeats it. This has never been true: the COMP-COMPLETION-GATE audit measured 321 managed features against 31 registered guard resources with ZERO overlap (all 31 leaked test fixtures), and 230 COMPLETE features none of which passed a guarded transition. The report describes the MCP boundary it genuinely closed and then generalises that to ALL callers — the CLI and build runner ran in-process, never called a guarded transition, and were never covered. Replace the absolute claim with what was actually delivered (the MCP/REST boundary), state the measured coverage, and forward-reference COMP-COMPLETION-GATE for the paths that remained open. Doc-only, no code. Scope note: state slice-1+2 reality only — after those slices `setFeatureStatus`, the vision PATCH endpoint, stratum-sync and direct `updateItemStatus` are STILL unguarded, so the correction must not swap one overclaim for a smaller one. | COMPLETE |
| 16 | COMP-COMPLETION-GATE-1 | record_completion ledgers the guard transition before the writer rejects a short commit_sha, leaving an unrecoverable intent; validate the full-SHA rule before the guard. | COMPLETE |
...
| # | Feature | Description | Status |
|---|---------|-------------|--------|
| — | COMP-CLI-HONEST-EXIT-1 | Four standalone CLI commands exit 0 while doing less with no error-level signal (COMP-HOST-PORTABILITY-1 host C SD01-04): roadmap prints nothing for a populated roadmap; experiment reports complete with nCompleted 0/1; remote status omits server health when COMPOSE_API_TOKEN is unset; migrate-anon --non-interactive changes nothing and returns 0. Each must render its result or report the incomplete state distinctly (message + exit code). Gap G7. | COMPLETE |
| — | COMP-EXPLORER-EVIDENCE-1 | The design phase prompt requires 2-3 explorer subagents but the accepted design ran one or none on both hosts (COMP-HOST-PORTABILITY-1 SD05) with no cardinality check or warning. Specify the required exploration evidence in the PhaseResult, validate it before accepting the phase, and make the subagent dispatch host-portable instead of Claude Agent/subagent_type semantics. Gap G10. | COMPLETE |
| — | COMP-FABLE-CALIBRATE | Receipt-calibrated task estimation for Fable-led teams. COMP-FABLE-ASTRA has Fable assign a tier per task (critical / standard / fast) from its own judgment. Close the loop: after each wave compare every task's planned tier with its receipt (STRAT-USAGE-SPLIT token split, cost, retries, review findings against that task), record the miss, and feed a compact calibration table into Fable's planning prompt for the next wave and the next run. Consumer of STRAT-LEARN-COST; the E3 complexity-triage idea in the ideabox is the shape. Depends on COMP-FABLE-ASTRA shipping. COMP-TEAMS: Agent Team Presets M medium KILLED 2026-09-11: superseded by COMP-MODEL-ROUTE (receipt join lands in slice S1b, the planning-prompt calibration table in slice S2 behind `calibration_feedback`). See docs/features/COMP-FABLE-CALIBRATE/killed.md | KILLED |
| — | COMP-HOST-PORTABILITY-1 | Host portability audit across three hosts (Claude Code baseline, Codex CLI host, standalone compose CLI). Classify every CLI command as works/degrades-loudly/degrades-silently/fails per host; inventory Claude-coupling and silent fallbacks with file:line; verdict on whether the Stratum connector layer is genuinely host-agnostic; ranked gap list. Audit only, no porting. | COMPLETE |
| — | COMP-OUTCOME-ENUM-1 | Agents return outcome success/done/approved/pass where the PhaseResult contract allows only complete\|skipped\|failed, forcing a full extra dispatch per occurrence (measured on host B and host C in COMP-HOST-PORTABILITY-1; 23 occurrences across stratum+compose via learn harvest). Decide: fix the step instructions/schema vocabulary, or normalize equivalent completion words before the costly retry. Check the steps first; do not loosen the contract blind. Gap G6. | COMPLETE |
| — | COMP-ROADMAP-ARCHIVE-GH | Two-document transactional publication for the GitHub tracker provider. COMP-ROADMAP-ARCHIVE v1 is local-provider only: GitHubProvider regenerates and publishes one roadmap document with a Contents API SHA check, and two independent Contents writes cannot claim atomicity. Publish the active/archive document set as one Git tree and one commit with a guarded ref update and conflict retry; an issue status transition may already be committed when publication fails, so report and repair that projection without repeating the transition. Provider parity for archival is complete only when this ships. Features M medium | PLANNED |
| — | COMP-ROADMAP-ARCHIVE-LINKS | Incoming-link rewriting across registered consumer roots after archival. COMP-ROADMAP-ARCHIVE v1 rewrites links inside the two roadmap documents and leaves redirect anchors for everyone else. Extend relocation to explicitly registered consumer roots (feature folders, docs, journal) using the relocation map, confined to configured roots, excluding generated and vendor trees, never sweeping other repositories or external issue bodies. Features S low | PLANNED |
| — | COMP-ROADMAP-ARCHIVE-NARRATIVE | Automatic row moves for narrative-owned roadmaps. COMP-ROADMAP-ARCHIVE v1 leaves narrative-owned workspaces (forge-top) to the manual split because the narrative guard exists so no writer edits hand-authored prose. Design identity-based, lossless row and owned-note moves against explicit narrative status tokens, with an ambiguous identity blocking the move with an actionable diagnostic and the existing regeneration guard untouched. Fixture: the forge-top ROADMAP.md / ROADMAP-ARCHIVE.md split of 2026-09-09. Features M medium | PLANNED |
| — | COMP-SUPERVISOR-OWNER-1 | compose start SIGTERMs whatever supervisor the machine-wide PID file names (server/supervisor.js:80-93 killExistingSupervisor) before binding :4001, so starting the app from any project replaces another project's running server. Took down the owner's server during COMP-HOST-PORTABILITY-1. Verify ownership (same target project) and require explicit takeover, or scope supervisor identity per project. Gap G8. | COMPLETE |
| — | COMP-VERIFY-POLICY-1 | Build triage silently downgrades a feature's requested verification: feature.json needs_verification true became false at runtime and Stratum recorded the step skipped with no warning (COMP-HOST-PORTABILITY-1 SD08, host B). Preserve explicit feature requirements, or require a visible recorded override before skipping verification. Gap G9. | COMPLETE |
...
| 304 | COMP-ROADMAP-ARCHIVE | Automatic roadmap archival during normal feature processing (generated-mode workspaces, local provider, v1). One shared projection service replaces the three renderRoadmap() call sites so completion (live and backfill), park/kill/supersede and reactivation move rows between ROADMAP.md and ROADMAP-ARCHIVE.md with stable anchors, redirect anchors, archive-first recoverable writes and archive-aware reads/validation. No new status: PARKED plus the recorded reason covers deferral. Supersedes COMP-ROADMAP-SHARD's active/archived shape; GitHub two-document publish, narrative-owned automation and cross-root link rewriting are filed follow-ups. [Design](docs/features/COMP-ROADMAP-ARCHIVE/design.md) · [Plan](docs/features/COMP-ROADMAP-ARCHIVE/plan.md) · [Blueprint](docs/features/COMP-ROADMAP-ARCHIVE/blueprint.md). Blueprint verified 2026-09-09 (72 corrections, 3 Codex rounds); implementation has not started. | PLANNED |
...
| 6 | COMP-ROADMAP-SILENT | No-op operations must fail loudly. A roadmap operation that matches nothing currently reports success, so a caller cannot distinguish 'applied' from 'affected nothing' — this is why SmartMemory's migration ticket sat mis-scoped from 2026-06-19 until 2026-09-11. Three instances found in one afternoon: (a) `roadmap_graph_check` returned matches:false plus 'stale … run compose roadmap graph' when the truth was that canon is 3% populated (19 feature.json across 642 folders, 0 deps.yaml) and regenerating would replace a 2,779-line 370-node hand-maintained graph with a 4-node 0-edge one — staleness and not-populated are different facts and it cannot tell them apart. (b) server/feature-scan.js:186-192 asserts the vision store 'is a managed projection of feature.json, not a thing that drifts from it (COMP-ROADMAP-GRAPH-2)', yet .compose/data/vision-state.json holds 644 features and 130 status conflicts with ROADMAP.md across 515 overlapping features, with records self-contradicting internally (BENCH-APP-BREADTH-1: status:complete, phase:verification, lifecycle.currentPhase:explore_design) — nothing detected it. (c) `migrateRoadmap` wrote 0 files and exited 0 (see COMP-ROADMAP-PROSE-READ). Precedent to follow: protectResidue() (lib/roadmap-residue.js:261) turns silent loss into a one-command fix. Source of record: /Users/ruze/reg/my/SmartMemory/smart-memory-docs/docs/features/ROADMAP-MIGRATE-1/design.md. | PLANNED |
...
| — | COMP-CONFLICT-MERGE | Close two verified silent-loss paths in roadmap regeneration. (1) An unbalanced `preserved-section` open marker is discarded on purpose (roadmap-preservers.js:233) so a typo'd close marker deletes exactly the content it was meant to protect. (2) readPhaseBlocks keys blocks by phase id in a Map (roadmap-preservers.js:268), so two `##` headings with the same text collapse to the last one and the earlier curated block is dropped — reachable via the second generation pass the CLI writes (bin/compose.js:1256). Guard by computing residue against the FINAL canonical bytes, not the first pass, using an occurrence-aware multiset keyed by line plus containing block. NOTE: the original justification (roadmap generate clobbers hand-authored prose generally) was DISPROVED — spliceTableIntoBlock preserves typed-phase prose and roadmap-roundtrip.test.js:83 asserts it. Providers bypass the CLI (local-provider.js:101, github-provider.js:524), so the guarantee is CLI-scoped unless it moves into the render paths. Anchor: docs/features/COMP-CONFLICT-MERGE/design.md | COMPLETE |
...
| 5 | COMP-ROADMAP-MIGRATE | Migrate forge-top narrative→structured (precondition for retiring narrative). One-time standalone script. Foreign-owned (compose/stratum) rows leave the live ROADMAP.md (option C — ROADMAP.md becomes forge-OWNED-only; foreign codes declared via externalPrefixes so validate/roundtrip skip them); the live cross-product view is the roadmap dependency graph (COMP-ROADMAP-GRAPH-3). Owned-here rows get a real feature.json. Curated changelog/Roadmap-Model prose extracted to docs/preserved-sections (never dropped). Order: classify → scaffold owned-here + set externalPrefixes → preserve prose → dry-run checkRoundtrip → flip narrative:false → validate clean. NOT link-carriers (option B dropped: xref-sync reconciles links[].expect not top-level status, so they'd render placeholders). Depends on COMP-ROADMAP-GRAPH-3. Design: docs/features/COMP-ROADMAP-MIGRATE/design.md. ROUND-TRIP SAFETY VERIFIED (2026-09-11, SmartMemory ROADMAP-MIGRATE-1): lib/roadmap-gen.js:56 generateRoadmapFromBase was called directly on a frozen snapshot with the narrative guard (:214, :519) and the CLI residue guard (bin/compose.js:1329) bypassed. The unprotected write went 6,915 -> 6,995 lines: 80 additions, ZERO deletions. It patches base text rather than regenerating, so preservation is structural, not merely guard-enforced. This removes the main data-loss risk from the migration. | PLANNED |
| 6 | COMP-ROADMAP-PROSE-READ | Prose-roadmap reader: parse a hand-authored narrative ROADMAP.md into structured feature rows. `parseRoadmap` (lib/roadmap-parser.js:54) recognizes only `## ` headings, `### ` headings and `\|…\|` table rows; every other line hits `continue` at :125-131. A `### CODE — Title` section therefore becomes a phaseId STRING and never a feature. Measured on SmartMemory's docs/ROADMAP.md (6,917 lines): 114 entries parsed, 0 real feature codes, all `_anon_`, 0 files written, exit 0 — and it scraped prose tables out of feature bodies, one yielding {"code":"_anon_0","status":"enqueued `{}`"}. Scope: handle the 421 `### CODE — Title` sections whose bodies carry bold-label lines (`**Status:**`, `**Priority:**`, `**Depends On:**`, `**Blocks:**` — e.g. ROADMAP.md:2294-2299) and ~289 list-only entries (313,827 B) with irregular ordinals, some 3.6 KB single lines holding full implementation reports (ROADMAP.md:350, :627); never a silent zero-row parse. Bonus: those bodies hold 387 `Depends on:` + 130 `Blocks:` — the only place 228 graph edges exist outside the hand-built HTML, so the same reader also generates deps.yaml. forge-top never hit this because its 99 rows were already tables with real codes. Secondary compose-side finding (measured here 2026-09-11): compose/ROADMAP.md itself mixes table headers — 74 `Feature`, 7 `Item`, 1 `Layer`, 6 `Feature \| Items` — so header-synonym handling belongs in the same reader. Source of record: /Users/ruze/reg/my/SmartMemory/smart-memory-docs/docs/features/ROADMAP-MIGRATE-1/design.md. | PLANNED |
...
| 7 | COMP-ROADMAP-ORDINAL | Preserve human force-rank ordinals through migration. feature.json has `position` (within-parent index 0-5) and `phase` (string), with no initiative/nesting type. SmartMemory's docs/ROADMAP.md carries 336 ordinals / ~170 distinct labels encoding a hand-maintained force-rank that exists in NO other artifact (roadmap-graph.html has no ordinal field; FEATURES.md is unordered). Sharp edge: the ordinals are used as IDENTIFIERS by documents that survive migration — CORE-VEC-DOMAINS-1/plan.md:9 and :174, and design-1b.md:11 and :292, all cite `25f6` by name (verified 2026-09-11). Dropping them breaks inbound references, not merely sort order. A flat `position` carries total order but cannot express sub-ranking under a parent (`19a`–`19v`), post-hoc insertion (`9b'''''`, `25a2`) or cluster tags (`3o-C3`). Minimum viable: a preserved `legacyOrdinal` string field. Source of record: /Users/ruze/reg/my/SmartMemory/smart-memory-docs/docs/features/ROADMAP-MIGRATE-1/design.md. | PLANNED |
...
| 13 | COMP-AUDIT-13 | docs/cli.md and `compose --help` omit shipped subcommands (plan, gsd, validate, experiment, remote, tracker, items, context, smartmemory) — installed capability is invisible unless users read source. Regenerate the reference and help from the real command table. | COMPLETE |
...
| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 1 | COMP-PLAN-IDEA-UNIFY | Unify ideation storage behind the fluid-store provider seam (owner ruling 2026-07-21, PROVIDER-SEAM in what-to-build 8k): ideabox records become provider records on a zero-install local floor implemented over vision idea-type items; SmartMemory is the capability-rich reference provider backing the same records later. ideabox.md becomes a read-only generated view (ROADMAP.md<-feature.json pattern). Ideabox stays the funnel/UI/CLI. Pilot workload of the provider seam positions/joints/decisions adopt next. Honors COMP-ROADMAP Decision 4 (funnel interface pinned, not storage). One-time markdown->store migration; no bidirectional sync. Rung 1 (keystone) of COMP-PLAN-RIGOR. Anchor: docs/design/2026-07-20-front-funnel-rigor-design.md; ruling: docs/product/2026-07-20-what-to-build-vision.md 8k. | COMPLETE |
| 2 | COMP-FLUID-SEAM-GUARANTEES | Lift the fluid store's safety guarantees out of the floor provider and into the seam. S3b-1 put serialized mutation (lib/dir-lock.js) and restartable-import handle reclaim (reclaimAborted) in local-provider.js only, so smartmemory-provider.js has NEITHER while fully satisfying the seam interface: it takes no lock at all (unserialized allocation, and dir-lock is a local mutex that could never serialize the shared case anyway) and its issued-handle guard throws unconditionally, so an import interrupted by a network blip burns that handle permanently and can never be re-run. Latent today (no fluid block in compose.json, so the floor is used) and unsafe to enable until closed. Fix the seam, not the second implementation: state both obligations on FluidProvider and add a conformance suite every provider must pass, so a third implementation cannot miss them silently. Carries F6-1 (concurrent --cluster adds can duplicate a cluster) and F7-1 (a discussion author containing a space does not survive the markdown grammar). NON-GOAL: the SmartMemory service issues (reindex, PATCH-cannot-clear, _embed, supersession) are filed and owned upstream. | COMPLETE |
...
| 9 | COMP-CANON-OVERRIDE | RETIRED 2026-09-08 — the canon override grant and its ledger/token machinery were removed after audit found no production caller, no recorded incident needing it, and no implementation of its promised ship-time or guard-verify consumers. The surviving canon-guard PreToolUse hook denies direct writes unconditionally. Exceptional repair uses an authorized write outside the hook's reach; a restricted agent escalates. See docs/decisions/2026-09-08-canon-override-grant-retired.md. | KILLED |
| — | COMP-JUDGMENT-BITEMPORAL | KILLED 2026-08-08 — premise disproved. Filed on the belief that amending a judgment position overwrites the belief and the ledger is the only history. Both are false: judgment_position_amend writes a NEW revision (judgment-writer.js:2931+), and the store already carries rev numbering, a `supersedes: <slug>#r<N>` chain, and retraction tombstones. Amendment is already non-destructive by construction. The only true residual — position_revision carries transaction time (provenance.written_at) but no valid-time axis, while facts already have one (fact_at) — is a design question, not a data-loss fix, and is re-filed in the ideabox. See docs/features/COMP-JUDGMENT-BITEMPORAL/killed.md | KILLED |
| — | COMP-JUDGMENT-PRECEDENT | Surface the judgment history that already exists on disk but is invisible. Revisions (`rev`), the `supersedes: <slug>#r<N>` chain, and retraction tombstones are all persisted, but the only read surface — get_judgment_state (judgment-writer.js:3354) — returns the LATEST revision per position and nothing else. Slice A: a trace op that walks a position back through its revisions and supersession chain, exposing what it replaced and why. Purely additive, no schema change, no external dependency. Slice B (semantic precedent search) remains BLOCKED on the unresolved SmartMemory recall failure. Anchor: docs/features/COMP-JUDGMENT-PRECEDENT/plan.md | COMPLETE |
...
## COMP-AGENT-LANES: Per-subagent lanes for parallel fan-outs — COMPLETE

| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 1 | COMP-AGENT-LANES | Per-subagent lane visibility for parallel fan-outs in the cockpit. When the lifecycle runs 2-3 agents in parallel (Phase-1 research, Phase-3 competing mandates), the cockpit shows only an aggregate '3 running' counter instead of each worker's mandate label, live status, and streamed output. Completes ITEM-25a's explicitly-deferred agent_run/parallel-step tracking. Cross-repo: Stratum stamps each parallel worker's stream events with a stable scoped step id + human label; Compose renders one lane per worker, reusing the existing AgentPanel per-agent renderer. | COMPLETE |

---

## COMP-AGENTOUT: Dense Agent-Facing Tool Output — PLANNED

| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 1 | COMP-AGENTOUT-1 | Enforced per-tool output budget. `server/compose-mcp.js:19` declares a 2,000-token soft cap (mcp-connector design Decision 6) as a COMMENT, and per-tool `limit` params implement it ad hoc and inconsistently; nothing fails when a tool blows it. COMP-AUDIT-19 fixed exactly one offender (add_roadmap_entry, 193,821 chars) by hand, which is a treadmill: the next writer that injects a rendered document into its result is undetectable until an agent hits the MCP token cap mid-task. Slice: declare a cap per tool, render every tool's result against a realistic large fixture (compose's own ~190KB roadmap, 350 features) in a test, and FAIL the suite when a result exceeds its declared cap or when a tool has no declared cap at all (an undeclared tool is a failure, not a skip). Prior art: Axiom's `scripts/check-doc-drift.sh` — a claim the gate cannot find is a FAILURE, not a skip. | PLANNED |
| 2 | COMP-AGENTOUT-2 | One greppable line per finding for validate_project/validate_feature/roadmap check/doctor. Today every diagnostic surface renders prose an agent must re-parse, and findings carry no stable identity, so tooling cannot filter by kind and an agent re-derives severity by reading. Adopt Axiom's AXDL shape: all findings flow through ONE structured type with three renderers (human / agent / json-lines) off the same record, so the formats can never disagree about what went wrong. Each finding gets a stable code namespaced by producer plus a wording-independent kebab-case slug, an exact location, and machine-applicable fixes inline (`loc:"msg"~>"replacement"`) rather than as English. One line per finding, colorless, no re-rendered source: `grep -c '^E '` counts errors. A consumer meeting an unknown field must FAIL, not skip (see COMP-SHIP-CONTRACT). Enforced tail: a code with a construction site but no explanation entry, or an explanation entry with no construction site, fails the gate in BOTH directions. Prior art: Axiom docs/diagnostics.md, scripts/check-doc-drift.sh check 1. | PLANNED |
| 3 | COMP-AGENTOUT-3 | Success-path notation: `compose symbols` / a dense read tool answering "what does this feature already declare" in one line per fact. AGENTOUT-2 only fires when something is WRONG; the other constant agent question is what already exists, and today answering it means re-reading feature.json, the folder, the gate log and the links, then re-deriving facts compose already computed. Emit one line per fact — status, phase, complexity, artifacts present/absent, gate state, links and their expect/actual, lifecycle position — with a kind letter deliberately disjoint from AGENTOUT-2's severity letters so a merged stream is never ambiguous. Include the stable feature code as the coordinate (compose already has content identity where Axiom needed to invent NID). Payoff claim to test, not assume: Axiom reports that making the success path greppable immediately exposed a parser bug that pretty-printed output had been absorbing for the life of the project. Prior art: Axiom AXSYM, docs/diagnostics.md. | PLANNED |

---

## COMP-DIST-EXEC: Distributed Execution Across Machines — PLANNED

| # | Feature | Description | Status |
|---|---------|-------------|--------|
| — | COMP-DIST-EXEC | Run a build's parallel agent tasks across multiple machines. Today every task in a parallel dispatch is a `child_process` on the orchestrator host, isolated by a git worktree (`server/agent-spawn.js`, `lib/build.js`), so a build's width is capped by one machine's cores and one machine's rate limits, and the host is a single point of failure mid-build. COMP-AGT-COORD deliberately scoped itself to a single host (design.md, Not-in-scope: "Multi-machine agent distribution"); this feature lifts that limit and inherits its debts. Three known dependencies, not hand-waving: (1) coordination currently goes through the parent or a shared blackboard in process memory, so a remote executor needs that channel over a network with the AGT-7 ordering and delivery guarantees restated for a partitionable link; (2) merge back into the shared build-stream is last-writer-wins today (batch builds share one build-stream), which is a tolerable gap on one host and a correctness bug the moment two machines finish concurrently — the merge queue (COMP-PAR-MERGE-QUEUE) has to become the only door; (3) worktree isolation assumes a shared filesystem, so task dispatch must ship a content-addressed workspace and collect a diff rather than assume a path. Open questions for design: executor discovery and enrolment (reuse the guard trust root, or a separate executor identity?), whether a remote executor is trusted to run lifecycle writes at all or only returns diffs for the orchestrator to apply, and what a partial-host failure does to an in-flight batch. | PLANNED |

---

## COMP-DEPS-AUTOINSTALL: Auto-install required plugin deps — COMPLETE

| # | Feature | Description | Status |
|---|---------|-------------|--------|
| — | COMP-DEPS-AUTOINSTALL | Install the plugins behind missing REQUIRED external deps during compose setup/init/update instead of printing a hint and running degraded. A clean install reported '6 of 12 deps missing (4 required)' and warned the lifecycle would run degraded, so reaching a working state depended on the user reading the report. Manifest entries gain a structured `plugin` spec and `marketplace_source`; the human `install` string is never executed (several entries are prose, and shelling manifest text is an injection surface). Required only, deduped by plugin, marketplace registered and install retried once on a pristine machine, post-install rescan reported. Opt out via --no-install-deps / COMPOSE_NO_PLUGIN_INSTALL=1; doctor stays read-only. Also fixes a scanner bug found while building it: the plugin cache outlives `claude plugin uninstall`, so an uninstalled plugin read as present and auto-install would never have fired. | COMPLETE |

---

## COMP-PRICING: One effective-dated cost model — PARKED

| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 1 | COMP-PRICING | Collapse three hand-maintained price tables into one effective-dated rates contract. `compose/lib/model-pricing.js`, `compose/lib/experiment-pricing.js` and `stratum/ts/src/judge/pricing.ts` are not three copies of one table but three different functions over two token dialects (OpenAI reports input_tokens INCLUSIVE of cached, Anthropic exclusive) and three cache models (derived 1.25x/0.1x multipliers; explicit per-model cacheRead with no write rate; no cache concept at all). That mismatch produced a measured 2.76x overcharge on a real call, caught only because the producer began stating its own amount (see COMP-MODEL-ROUTE `usd_source`). Ships an effective-dated `model -> [{from, until, input, output, cacheRead, cacheWrite, dialect, source}]` contract under `contracts/` with the existing `_source`/`_changelog`/`_consumers` convention, and one `priceCall(model, usage, at)` that takes dialect as DATA rather than branching on a model-ID regex; a receipt prices at its own timestamp, so the terra/sol table divergence becomes two query dates against one history rather than an owner ruling. Pricing stays REPRODUCIBLE, never live: receipts are tamper-evident evidence and a runtime feed would silently defuse the COMP-MODEL-ROUTE gate-5 falsifier. The only external touch is a scheduled CI diff of the pinned snapshot against the LiteLLM community registry, which opens an item and never reaches runtime. Stratum keeps its own copy behind a drift test (no new npm package, no stratum-as-library: its package.json exports nothing and compose consumes it by spawning its bin over MCP). Also folds in the prefix-precedence defect, where the legacy `gpt-5` key prefixes every gpt-5.x model and silently mispriced luna by 35.7x. Out of scope: spark (subscription-billed, no external source exists, stays flagged unverified) and Q3. | PARKED |

---

## COMP-COST-OWNER: One owner for the cost number — PARTIAL

| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 1 | COMP-COST-OWNER | One owner for "what did this cost", which every sink READS rather than re-derives. Supersedes COMP-PRICING: that feature was filed as a pricing problem, and tracing proved pricing is the small part. One measurement at the connector currently fans out to FIVE independent running totals over different event populations with nothing reconciling them — `usageTotals` (result-normalizer, per step), `accumulator.usd` (persisted sidecar, all nine `recordBuildUsage` sites), `buildCostTotals` (in-memory, ONE site: `build.js:5273-5275`, main steps only), routing receipts (per dispatch), and build-stream events. MEASURED on the retained live-fire run: the ledger and the accumulators agree to the cent at $1.4257732 (delta 0.0000000000) while `build-history.jsonl` reports $1.38900475 and 589,373 tokens against the accumulator's 808,002 — short $0.0367685 (2.6% of spend) and 218,629 tokens (27.1%). Evidence: `docs/features/COMP-COST-OWNER/evidence/three-totals-2026-09-12.md`. Cause: fix/revise/gate-fix/error-carried spend reaches the accumulator and the ledger but never `buildCostTotals`, and `buildCostTotals` is SEEDED from the accumulator at `:3746-3749` on every invocation including resume, so the history cost depends on whether the build was resumed (read-verified, not yet measured). Each sink arrived with a different feature (COMP-OBS-COST, COMP-MODEL-AB, COMP-MODEL-ROUTE, resume support), none found an owner to read from, and each added a defensive `?? 0` that silently converts "unknown" to "free" — five times between measurement and display, ending at `<$0.001` on the cockpit for a call of unknown cost. The receipt path (`build.js:2280-2283`) already does this correctly, stripping an unprovenanced dollar rather than inventing provenance, and is the model. Carries forward from COMP-PRICING: the dialect-as-data pricer, the pinned-registry freshness check (ccusage ships exactly this shape), and deleting the unreachable consumer-side fallback. | COMPLETE |
| 2 | COMP-COST-OWNER-1 | Every ordinary dispatch settles with stratum via an acknowledged receipt OR a usage-bearing stepDone; kickoff (lib/new.js) stops discarding usage | BLOCKED |
| 305 | COMP-CODEX-PROVIDER-1 | Compose has no first-class Codex lifecycle provider path: COMP-HOST-PORTABILITY-1 measured BOTH lifecycle hosts spawning only Claude via @anthropic-ai/claude-agent-sdk (lib/local-claude-connector.js), with Codex present only as the OUTER driver on host B, never as an implementation child. There is no lib/local-codex-connector.js; existing Stratum Codex support alone is insufficient, and COMP-CODEX-IMPL (COMPLETE) flips build-step agents in a spec variant rather than supplying a Compose-side provider. Implement or choose a first-class Compose Codex connector, route every phase, repair, gate and review through it, and supply host-neutral prompts and real install targets (Codex currently shares Claude skill paths; custom agents and plugin management stay Claude-oriented). Sized L and BLOCKS a genuinely Codex-executed lifecycle. Prerequisites G1-G4 (gate addressing, failed-phase resume, review prompt size and bounded provider recovery, authoritative lifecycle state bridge) are all M, all BLOCKS-level, and all still unfiled. Acceptance is the report's pilot test: design through ship, real Codex dispatch evidence per required phase, gates controlled without a web UI, verification and dependencies preserved, and resume from a deliberately failed review with recorded cost and provenance. Gap G5. | PLANNED |
| 306 | COMP-GATE-HEADLESS-1 | A gate delegated to the web UI has no headless resolution path and no application timeout: COMP-HOST-PORTABILITY-1 host C waited after 'Gate delegated to web UI. Waiting for resolution...' until an operator killed it, while gate queries returned 'HTTP 400: Unknown workspaceId: proj' because that server could not resolve the scratch workspace ('forever' means no built-in termination was observed, not an infinite-duration measurement). Host B resolved the same gate externally through Stratum, after which the foreground Compose runner tried to resolve an already-resolved gate and failed loudly. Every headless or external gate client is affected. Needs stable workspace, run and gate IDs, an explicit headless resolution path, a single wake-and-reconcile step, and a visible failure when the UI is unavailable instead of an unbounded wait. Sized M and BLOCKS. Gap G1. | PLANNED |
| 307 | COMP-RESUME-FAILED-PHASE-1 | Terminal review failure strands completed implementation work with no recovery path: COMP-HOST-PORTABILITY-1 host B lost a 18m51s, 208080-token, $14.04 flow at review and could not resume it by any measured route. Compose MCP resume failed; CLI --resume exited 1 with 'Nothing to resume'; stratum_resume completed at the Stratum layer but did not restore Compose resumability, so the Stratum call itself did not fail; --fresh restarted from design rather than review. Earlier post-gate CLI resume DID work, so this is specific to a terminal failed phase and is NOT covered by COMP-RESUME or COMP-BUILD-RESUME (both COMPLETE, both about crash and environment resume). Preserve the failed cursor and phase history and reconcile Compose against Stratum resume rather than forcing a fresh design. Sized M and BLOCKS recovery. Gap G2. | PLANNED |
| 308 | COMP-REVIEW-BUDGET-1 | Review lenses send unbounded prompts and treat a bounded provider rate limit as terminal: COMP-HOST-PORTABILITY-1 host B selected three Claude lenses (claude-sonnet-5); two failed with 'Prompt is too long', their automatic retries then failed with a visible provider rate-limit error, and because the fanout is require:all the whole review failed loudly and terminated the first lifecycle flow after $14.04 of completed work. The third diff-quality lens returned normally, so this is a prompt-size and retry-policy defect, not a lens defect. Every review using the require:all path is affected. Budget lens context against a real prompt ceiling, and handle a bounded provider retry with resumable state instead of assuming the rate limit is permanent. Loud, not silent. Sized M and BLOCKS the measured host B run. Gap G3. | PLANNED |
| 309 | COMP-LIFECYCLE-BRIDGE-1 | Two lifecycle state planes disagree, so a host cannot discover or control its own running build: COMP-HOST-PORTABILITY-1 host B saw the Compose tracker return count:0 gates and 'Item not found: HOSTB-HELLO' during a LIVE design_gate, while Stratum flow poll and audit exposed the real active flow (SD06). Process-local workspace selection succeeded without making session binding usable. An outer driver can connect both MCP servers and still not rely on Compose to find its own CLI lifecycle. This is SD06 plus the loud bind, reconciliation and recovery failures, and is NOT a claim that every tracker tool fails. Establish one source of truth, or a run-ID bridge, across tracker, flow, session and active-build state. Sized M and BLOCKS reliable host control. Gap G4. | PLANNED |
| 310 | COMP-BUILD-SPLIT-1 | Split lib/build.js (7845 lines) into cohesive modules so the build orchestrator fits in an agent context window and concurrent briefs can touch disjoint files | PLANNED |
| 311 | COMP-RESUME-CHECKPOINT-1 | Checkpoint the build flow at the post-merge pre-triage boundary so a failed review can be recovered by revert without losing completed implementation work | PLANNED |
| 312 | COMP-PROMPT-SECRETS-1 | Scan and redact assembled prompt bytes for credentials before any provider dispatch. lib/step-prompt.js concatenates every docs/context/*.md in full into the prompt and ships it to a paid provider; nothing in compose scans or redacts prompt CONTENT (stratum scrubs env vars only, ts/src/connectors/base.ts:16). A credential in any context doc silently leaves the machine. Needs a pre-dispatch scan of the final assembled bytes, redaction with a visible marker, and a loud local failure rather than a silent send. Sized S. | PLANNED |
