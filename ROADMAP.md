...
| 10 | COMP-MCP-MIGRATION-1 | **Audit-log correlated auto-rollback for `enforcement.mcpForFeatureMgmt`.** Surfaced by `COMP-MCP-MIGRATION`. Add per-build correlation IDs to `feature-events.jsonl` rows so the build runner can pre-stage scan dirty `ROADMAP.md` / `CHANGELOG.md` / `feature.json` files and reject those without a matching typed-tool event in the same build window. Promotes v1 prompt-only enforcement to true block mode. | PLANNED |
| 11 | COMP-MCP-MIGRATION-2 | **Honor `paths.features` in `.compose/compose.json` across all writers.** Surfaced by `COMP-MCP-MIGRATION`. `feature-writer.js`, `feature-json.js`, and `ArtifactManager` all default to `docs/features` and ignore the configured override. Threading the override through every writer + ArtifactManager constructor unblocks repos that need a non-default feature root. | PLANNED |
