...
| 288 | COMP-CTXBUDGET-1-1 | Add `--test-timeout` to the `npm test` node-suite script so a starved/flaky integration test (e.g. proof-run) fails loudly instead of hanging the suite and the pre-push hook indefinitely. (Independent test-infra fix; surfaced by COMP-CTXBUDGET-1, not part of the context-budget skill.) | PLANNED |
