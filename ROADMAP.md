...
| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 1 | COMP-GSD | Autonomous Long-Run Mode initiative — `/compose gsd <feature>` as a third lifecycle mode (alongside build/fix) for long autonomous runs across many context windows. Parity pass against gsd-build/gsd-2 — adopt the capabilities Compose lacks (per-task fresh context, worktree isolation, budget ceilings, stuck detection, milestone reports), reuse Compose's existing lifecycle/journal/gates as the substrate. Umbrella for COMP-GSD-1 through COMP-GSD-7. **COMPLETE 2026-06-03** — all 7 sub-tickets shipped (GSD-7 milestone reports closed it). The lone residual is GSD-3's per-task pre-merge gating + conflict-bounce, which is **PARTIAL and carried by COMP-PAR-MERGE-QUEUE** (a separate feature — not part of this umbrella's build). Follow-ups: COMP-GSD-6-WATCHDOG (row 9) + COMP-GSD-7-EVENTLOG (row 10) **shipped 2026-06-03**; COMP-GSD-4-OPSSTRIP-LIVE, COMP-GSD-4-PERTASK-TOKENS still tracked. | COMPLETE |
...
| 4 | COMP-GSD-3 | Worktree-per-task isolation + merge-back. **PARTIAL (2026-06-02, verified vs disk):** the core shipped via the Stratum substrate GSD-2 wired — the gsd `execute` step (`pipelines/gsd.stratum.yaml`) runs `isolation: worktree` + `capture_diff: true` + `merge: sequential_apply` + `retries: 2`, and `ship_gsd` runs the project test suite + build pre-commit. So worktree-per-task isolation, diff capture, sequential merge-to-base, teardown, and conflict-detect-with-retry are DONE (Stratum `worktree.py`/`parallel_exec.py`; worktrees at `~/.stratum/worktrees/<flow>/<task>`, not `.compose/gsd/<task-id>/`). **Residual carried by COMP-PAR-MERGE-QUEUE:** (1) per-task *pre-merge* lint/build/test gating on each task's diff (today the project gates run once at `ship_gsd`, post-all-merges; per-task is only TaskResult cert/ensure); (2) conflict→bounce that feeds merge-conflict context to the re-dispatched agent (today `retries` re-runs blind). See ROADMAP.md 2026-06-02 reconciliation log. | PARTIAL |
| 5 | COMP-GSD-4 | Budget ceilings + stop conditions. **Shipped 2026-06-03** by ADOPTING the stratum flow budget (STRAT-WORKFLOW-BUDGET) — not rebuilding (verify-first; same shape as GSD-3). Opt-in `gsd.budget.*` injects a flow `budget:{ms,max_agent_dispatches,max_tokens,usd}` block (absent ⇒ byte-identical); the `budget_exhausted` terminal halts the run with `budget.{md,json}` + `pause.json(kind:budget)` → `--resume`; cumulative cross-session token/cost in `budget-ledger.js` refuses a spent run pre-dispatch (`--reset-budget` clears). Per-task wall-clock via `task_timeout`. Codex gate at every phase → CLEAN. **Deferred follow-ups (documented, not gaps):** `COMP-GSD-4-OPSSTRIP-LIVE` (live cockpit burn pill — gsd emits no build-stream telemetry; v1 surfaces burn via `budget.json`+ledger), `COMP-GSD-4-PERTASK-TOKENS` (per-task token hard cutoff). `docs/features/COMP-GSD-4/`. | COMPLETE |
...
| 7 | COMP-GSD-6 | Headless CLI + crash recovery. **Shipped 2026-06-03.** `compose gsd <feature> --headless` = an outer supervisor that auto-resumes crashes/stuck with backoff under a per-pause-kind policy (`gsd.headless.*`; crash+stuck on, budget off by default). A continuously-flushed `.compose/gsd/<f>/state.json` (plain JSON, no SQLite) makes a hard crash recoverable (running+dead-pid ⇒ `crashed`; `loadResumeTaskGraph` synthesizes a resume graph when `pause.json` is absent). `compose gsd query <f>` returns an instant (~ms) JSON snapshot (precedence state→pause→budget→absent). Also closed two pre-existing gaps: atomic `run.lock` live-run exclusion + the deferred stale `pause.lock` takeover. Spec corrected (verify-first): `gsd` was already non-interactive so `--headless`=supervision not prompt-suppression; `state.json` is standalone (gsd never journaled). 48 tests; Codex CLEAN at every gate. **Deferred:** `COMP-GSD-6-WATCHDOG` (hung-child heartbeat watchdog). `docs/features/COMP-GSD-6/`. | COMPLETE |
| 8 | COMP-GSD-7 | Milestone report generator. **Shipped 2026-06-03.** A clean `compose gsd` completion writes a self-contained HTML report to **`docs/gsd-reports/<feature>.html`** (auto-discovered by the cockpit `DocsView` — zero server change; the spec's `.compose/gsd/reports/` + "cockpit asset pipeline" assumed a pipeline that doesn't exist). Per-task summary (status/attempts/files/**elapsed**), **budget actuals-vs-caps**, snapshot-derived timeline, **inline per-task diffs**. "Full v1" capture is compose-side (no Stratum change): per-task diffs snapshotted at the worktree-merge poll site (`diffs/<id>.diff`), per-task elapsed derived from compose's own poll loop into a `timing.json` sidecar, `completedAt`+`budget-final.json` persisted on complete for retroactive `compose gsd report <feature>`. All report-side writes best-effort (never demote a successful run). Both capture writes gate on `context.gsd` so build mode is byte-identical. 37 tests; Codex impl gate → CLEAN after one fix. **Follow-up shipped:** `COMP-GSD-7-EVENTLOG` (row 10) replaced the snapshot-derived timeline with a real event stream. `docs/features/COMP-GSD-7/`. | COMPLETE |
| 9 | COMP-GSD-6-WATCHDOG | Hung-child watchdog for the headless supervisor. **Shipped 2026-06-03.** The `--headless` supervisor detects a **hung** child (state.json heartbeat frozen while its pid is alive) and kills+resumes it — v1 only reacted to child *exit*. Each attempt **races child-exit against a confirm-polling heartbeat watchdog** (`defaultWatch`: two consecutive stale polls with unchanged `heartbeatAt`, surviving suspend/clock-jump); a hung child is killed by pid (SIGTERM→grace→SIGKILL), `pause.json` cleared so the crash-bridge recovers from the current `state.json`, and reclassified as a new `autoResume.hung` recovery (resumes like a crash). The load-bearing fix: an **independent wall-clock heartbeat timer** in the child (`gsd.js`, gated on `GSD_HEADLESS_ATTEMPT`) makes a frozen heartbeat a *sound* hang signal rather than the event-driven advisory it was — so a quiet-but-healthy task isn't false-killed; interactive `compose gsd` stays byte-identical. 19 tests; Codex gate: 3 design + 3 impl + 1 follow-up finding → CLEAN. `docs/features/COMP-GSD-6-WATCHDOG/`. | COMPLETE |
| 10 | COMP-GSD-7-EVENTLOG | Append-only GSD run-event log. **Shipped 2026-06-03.** GSD persisted only snapshots, so COMP-GSD-7's milestone-report timeline was reconstructed from whatever halt artifacts existed. Now a `.compose/gsd/<f>/events.jsonl` (`lib/gsd-events.js`, one `{ts,kind,...detail}` line, best-effort append, reader skips torn/non-object lines) records the run's story: `run_started` (fresh truncates the log + clears stale halt artifacts **after** preconditions; resume appends), `phase` (decompose/execute via `emitPhaseOnce` — `runState.phase` can't gate it), `task_completed` (`emitCompletionDeltas` at the execute-merge **and** both halts; deduped via a set seeded from the initial completed snapshot so resume doesn't re-fire), `paused` (`pauseKind` not `kind` — the spread would clobber it), `completed`, `failed`. The report's `buildTimeline` prefers the event stream and falls back to the snapshot timeline on **zero usable events**. 13 tests + a real-`runGsd` integration assertion; Codex gate: 5 design + 2 impl findings → CLEAN. `docs/features/COMP-GSD-7-EVENTLOG/`. | COMPLETE |
